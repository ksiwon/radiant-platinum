...
function parity() {
  ...
  for (const line of read('PARITY.md').split(/\r?\n/)) {
    // ⚠️ **꼬리 글자를 빠뜨리면 안 된다.** `8.4b 승강기`·`8.6b 깨어진 세계의
    // 기라티나`처럼 번호에 글자가 붙은 줄이 실제로 있고, 숫자만 받으면 그 둘이
    // **조용히 빠진다** — 생성물로 만든 까닭이 「잊혀서 비워지지 않게」인데
    // 정규식이 대신 잊어버리는 자리였다
    const m = /^\|\s*([0-9]+\.[0-9]+[a-z]?)\s+([^|]*?)\s*\|\s*(◐|✖)\s*\|(.*)$/.exec(line)
    if (!m) continue
