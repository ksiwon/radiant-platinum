...
const CENTERS = {
   ...
   * 장막 센터(134)와 들판 센터(123)다.
   *
   * ⚠️ **들판시티(120)도 123이다.** 214·213번도로 사이에 센터가 없어 134로 적었는데,
   * 그 자리의 앞 걸음(214 → 진실호수 호숫가 → 213 → 들판)이 회복보다 먼저 돌아 이미
   * 들판시티에 서 있다 — 장막까지 되걷다 예산을 태웠다(2026-09-24 이어 달린 판).
   *
   ...
   */
  433: 101, 132: 134, 133: 134, 143: 134, 120: 123, 122: 123,
}
 ...
 *
 * ⚠️ **레벨이 모자라서 지는 것이 아닌데도 올린다.** 롬의 멜리사는 헤이데스 L24 ·
 * 고우스트 L24 · 무우마직 L26인데, 우리가 L34·L31·L30으로 들어가 두 판(2026-09-24
 * 대표 구간 9판 · 그 세이브로 이어 달린 판)이 첫 도전에서 졌다 — 최면술·이상한빛·
 * 도깨비불이 쌓인다. 사탕은 공짜고, 레벨로 시간을 쓰지 않는 것이 규칙이다
 * (`RARE_CANDY_20260917`). 판마다 재도전으로 배지는 땄다
 */
const HEARTHOME_LEAD_LEVEL = 38
const STARAVIA = 397
/** 찌르꼬 → 찌르버드 → 찌르호크. 사탕은 어느 단계에서도 같은 자리를 가리킨다 */
const STARLY_LINE = [396, 397, 398]
const HEARTHOME_STARAVIA_LEVEL = 36
/**
...
/**
 * 빌딩 앞 **셋째** 레벨. 셋째 사탕은 연고 체육관 앞(`THIRD_LEVEL`)뿐이었다 —
 * 실측(2026-09-24 대표 구간 9판): 쥬피터를 이기고 208번도로 트레이너 (420,722)에게
 * 전멸했다. 파티가 수풀부기 L30 · 찌르버드 L30 · **비버니 L4**, 싸울 몸이 둘이었다
 * (연고 체육관이 가르쳐 준 것과 같다 — REPAIR §58). 연고 앞의 L30까지 여기서 당긴다
 */
const JUPITER_THIRD_LEVEL = 30
...
async function openTitle() {
   ...
   */
  /**
   * ⚠️ **두드리는 예산도 아래 `goto`와 같은 편차 위에 있다.** 기본값은 20초 × 3회
   * = 60초인데, 바로 아래 주석이 근거로 적은 실측은 154~300초가 넘는다 — vite가
   * 「준비됐다」를 찍은 뒤에도 모듈 그래프를 계속 변환하고 그동안 첫 요청을 붙잡고
   * 있기 때문이다. 짧은 예산으로 두드리면 **살아서 굽고 있는 서버를 「죽었다」로
   * 적고** 판을 통째로 BLOCKED로 끝낸다 — 실측(2026-09-21): 서버가 69초에
   * 「준비됐다」까지 찍었는데 두드림은 62초에 `AbortError`로 떨어졌다.
   * 아래 `goto`와 같은 10분으로 맞춘다 — `devServer.mjs`가 스스로에게 주는 예산도
   * 그것이다. 이 예산은 판정을 대신하지 않는다 — 서버가 정말 죽었으면 `fetch failed`가
   * 상한을 안 쓰고 곧바로 돌아온다
   */
  const alive = await knock(url, { timeout: 200_000, tries: 3 })
  if (!alive.ok) {
    ...
    after: async (api) => {
      ...
      for (const stop of AFTER_STOPS) {
        ...
        if (stop.id === '23') {
          ...
          const third = await candyUp(2, null, JUPITER_THIRD_LEVEL)
          log(`  ${stop.what} 앞 셋째 사탕 (L${String(JUPITER_THIRD_LEVEL)}) → ${candyLine(third)}`)
          story.candySteps = [...(story.candySteps ?? []), { what: '쥬피터 앞 셋째', ...third }]
        }
        /**
