    ...
    await withDev(async (dev, ensure) => {
      await run('29', '설치본으로 확인 지점과 화면을 연다 (개발 서버 + OPFS)',
        async ({ page, errors }) => {
           ...
           */
          /**
           * ⚠️ **두드리는 예산은 바로 아래 `warmDev`와 같은 편차 위에 있다.**
           * 기본값 20초 × 3회 = 60초로는 **살아서 굽고 있는 서버를 「죽었다」로
           * 적는다** — vite는 「준비됐다」 뒤에도 모듈 그래프를 미리 변환하고,
           * 이 항목은 제 개발 서버를 새로 띄운다. 실측(2026-09-21): 같은 판에서
           * 서버가 준비되기까지 **149초**가 걸렸는데 두드림은 62.1초에
           * `AbortError`로 떨어져 ㉙이 통째로 BLOCKED가 됐다. `journey`의 같은
           * 자리도 같은 날 62.1초로 떨어졌다 — 한 값이 두 번 같은 모양으로 나왔다.
           * `warmDev`·`devServer`와 같은 10분으로 맞춘다. 판정은 안 무뎌진다 —
           * 서버가 정말 죽었으면 `fetch failed`가 상한을 안 쓰고 곧바로 돌아온다
           */
          const alive = await knock(`${dev}/`, { timeout: 200_000, tries: 3 })
          if (!alive.ok) {
