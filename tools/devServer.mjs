...
${log}`)
  ...
  try {
    /**
     * ⚠️ **`fetch` 한 번은 300초에 끊긴다.** Node의 `fetch`(undici)는 응답 머리를
     * 기본 300초(`headersTimeout`)까지만 기다리고 「fetch failed」를 던진다 —
     * 위 `AbortSignal`의 예산과 상관없이다. 첫 요청이 170~330초인 이 프로젝트에서
     * 2026-09-23~24 밤에 두 번 그렇게 죽었다. vite는 끊긴 뒤에도 변환을 이어 가므로
     * **예산 안에서 다시 요청한다.** (`node:http`로 바꿔 봤다가 윈도우의
     * `localhost` 두 갈래에서 빈 오류로 곧바로 죽었다 — 그 길은 안 쓴다)
     */
    let r = null
    for (;;) {
      try {
        r = await Promise.race([fetch(url, { signal: AbortSignal.timeout(Math.max(1_000, until - Date.now())) }), died])
        break
      } catch (e) {
        if (dead !== null || Date.now() > until || !/fetch failed/.test(String(e?.message ?? e))) throw e
        console.log(`  개발 서버(${port}) 첫 요청이 끊겼다 (${String(e?.cause?.code ?? e?.cause?.name ?? '?')}) — 다시 요청한다`)
