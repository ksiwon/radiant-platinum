...
export const GROUP_FORMAT: Readonly<Record<string, number>> = {
  ...
  marts: 1,
  /**
   * 2 — 크레딧 **배치표를 사용자 롬에서** 읽는다 (PARITY §8.12).
   *
   * ⚠️ **이미 깔린 것을 그냥 두면 안 된다.** 지금까지 두루마리의 y 좌표는
   * 미국 오버레이에서 구운 237줄 하나였고, 화면이 그것을 대사 뱅크 길이로
   * 잘라 썼다. 한국판·일본판 롬으로 설치한 사람에게는 그 잣대가 틀린다 —
   * 일본판은 목록이 127번째 줄부터 갈려 y 간격이 열네 자리 어긋나고, 한국판은
   * 뱅크 237칸 중 뒤 28칸이 비어 있어 빈 줄이 스물여덟 개 흐른다. 설치본에
   * `data/credits.<판>.json`이 새로 생기므로 이 그룹(총 1MB가 안 된다)만
   * 다시 만들게 한다
   */
  credits: 2,
  /**
   ...
   * 가진 것이 주인공뿐이라 그 두 벌을 **화면에서** 다른 몸으로 옮긴다.
   * 실측으로 117벌이 166.2 → **169.1MB**다. 안 올리면 이미 깔린 사람은 그
   * 클립이 없어 계속 절차형 사인파로 걷는다.
   *
