...
export const GROUP_FORMAT: Readonly<Record<string, number>> = {
   ...
   * 남의 몸의 **레이어 색을 갈아 끼워** 세운다 (`NPC_RECOLOR`) — 덧칠이 아니라
   * 셰이더가 원래 곱하는 값을 바꾸는 것이라 음영이 살아 있다. 꼬리 붙은 이름으로
   * 따로 굽고(`fc2033_01-looker`·`fc1041_00-charon`) 원래 번들은 그대로 남는다 —
   * `fc2033_01`은 게임디렉터가 쓰고 있다. 번들 114 → 116벌, 147.1MiB.
   * **이걸로 사람 그림 판때기가 0이 된다.**
   *
