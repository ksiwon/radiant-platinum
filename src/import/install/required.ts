...
export const REQUIRED_PLATINUM_GROUPS = [
  ...
  // ⚠️ **없으면 기술이 아무것도 안 보인다.** 배틀은 돌지만 471개 기술의 연출이
  // 통째로 빈다 — 대본(`moveAnimTable`)이 부르는 `.spa`가 없기 때문이다.
  // 그리고 이 자리가 **이미 깔린 사람에게 다시 굽게 하는 유일한 길**이다:
  // 새 그룹은 설치 기록에 아예 없어서 `planAssets`가 못 보고, 필수 목록에
  // 없으면 「준비 완료」인 채로 영영 안 생긴다 (「굽는 쪽이 둘이다」)
  'particles',
] as const
