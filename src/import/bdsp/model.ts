...
export async function exportModel(
  ...
  for (const { v } of smrs) {
    ...
    // 27,420바이트로 줄었다 — 실측(`.audit/probe/monFail.mjs`)으로 종·판 557 중
    ...
    // 서고, `convert.ts`의 `bake`가 같은 번호의 치비로 떨어진다. ⚠️ **한때 그
    // 길을 타던 셋(`tr1026_00`·`tr1078_00`·`tr1085_00`)은 지금 다 등신으로
    // 선다** — 앞의 둘은 재질이 든 의존 번들을 같이 열어서(`bundleDeps`),
    // `tr1078_00`은 빈 뼈 칸을 뿌리로 돌려서다. 되돌림 자체는 그대로 둔다.
    //
    ...
    //
    // ⚠️ **빈 칸과 못 푼 뼈를 가른다 — 굽는 쪽 둘이 여기서 갈렸었다.**
    //
    // `m_PathID == 0`은 **뼈를 못 찾은 것이 아니라 칸이 비어 있는 것**이다.
    // `tr1078_00`의 껍데기가 그런 칸을 하나 들고 있다. 그것까지 세우면 그 사람이
    // `convert.ts`의 `bake`에서 **같은 번호의 치비**로 떨어지는데, 노드 쪽
    // (`bdspGlb.py`의 `_joint`)은 그 칸을 뿌리로 돌려 **등신**을 굽는다 — 그래서
    // 개발 서버에는 등신이 서고 설치본에만 치비가 서 있었다. 실측으로 `run.mjs`
    // ⑮가 `data/npcModels.json`에서 **딱 한 줄**을 잡았다
    // (`131="fc1078_00"` ≠ 노드의 `tr1078_00`). 여기서도 뿌리로 돌린다 —
    // 그 칸을 가리키는 정점은 가중치가 안 실리고, 없는 관절보다 안 움직이는
    // 관절이 낫다.
    //
    // ⚠️ **그렇다고 못 찾은 뼈까지 0번으로 떨어뜨리면 안 된다.** 그 자리에 실린
    // 정점이 **뿌리에 묶여** 몸에서 떨어져 나가는데, 화면에서는 "저 사람 팔이 좀
    // 이상하다"로만 보인다. 빈 칸이 아닌데 못 찾은 것은 그대로 세운다 — 그러면
    // `bake`의 되돌림이 걸려 치비로나마 온전한 몸이 선다. ⚠️ 노드 쪽은 이 둘을
    // 안 가르고 **아무 실패나** 뿌리로 돌린다. 지금 덤프에서는 답이 같지만
    // (갈리면 ⑮가 선다) 갈라지면 이쪽이 맞는 쪽이다
    const remap: number[] = []
    for (const b of (v.m_Bones as Props[] | undefined) ?? []) {
      const pid = num(b.m_PathID)
      if (pid === 0) { remap.push(roots[0] ?? 0); continue }
      const at = boneAt.get(pid)
      if (at === undefined) throw new ModelError(`뼈를 못 찾았다 (PathID ${String(pid)})`)
      remap.push(at)
    }
