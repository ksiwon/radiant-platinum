...
maybe('대사', () => {
  ...
  it('맵이 쓰는 뱅크 404개 + 공용을 담는다', () => {
    ...
    // 그 둘도 배틀이 제 뱅크를 직접 여는 자리다 (PARITY §2.26)
    expect(index.banks.length).toBe(498)
...
// ⚠️ 배틀 네 뱅크가 이 수의 절반이 넘는다 — 배틀 글 1,269줄은 거의 다
    // 이름·수를 빈칸으로 받는 문장 틀이고, 기술 줄 1,404개는 전부 이름 빈칸이 하나다
    expect(counted).toEqual({ en: 8623, ko: 6727, ja: 6948 })
  })
