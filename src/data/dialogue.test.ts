...
maybe('대사', () => {
  ...
  it('맵이 쓰는 뱅크 404개 + 공용을 담는다', () => {
    ...
    // 랭크 이름 아홉(551)
    expect(index.banks.length).toBe(496)
  ...
  /**
   * 일본 롬에 아예 없는 뱅크. 그 맵만 글이 빈다.
   *
   * 414(`month_names`)는 일본어가 달을 이름으로 안 부르기 때문이다 — 문장 틀이
   * 숫자 뒤에 `がつ`를 직접 쓴다. 미국판만 `Jan.`을 쓴다.
   *
   * ⚠️ **147(`game_corner`)은 여기 있었지만 일본 롬에도 있다.** 열쇠 표가
   * "없다"고 한 것은 그 뱅크를 일본판이 26줄로 다시 썼기 때문이다(미국판 28줄) —
   * (키, 엔트리 수)로 짝을 지으니 못 알아본 것이고, 자리로 메우면 나온다
   */
  const MISSING = {
    ja: [414, 393, 394, 413, 620],
  ...
  const SHORTER: Record<string, Record<number, number> | undefined> = {
    ...
    ja: { 147: 26, 397: 35, 534: 5, 697: 125, 548: 184 },
  }
  ...
  it('짧은 일본어 뱅크는 앞이 그대로 맞는다 — 번호가 밀리지 않는다', () => {
    ...
    for (const [at, short] of Object.entries(SHORTER.ja!)) {
      ...
      // ⚠️ **게임코너(147)도 「짧은」 것이 아니라 다시 쓴 뱅크다.** 앞 스무 줄은
      // 한국어판과 같은 순서인데 그 뒤가 일본판 슬롯머신 얘기로 갈라지고
      // (일본판만 나오는 슬롯머신 연속 당첨 얘기다) 마지막 두 줄이 없다. 뱅크가 밀린 것이
      // 아니라 롬이 그렇게 만든 것이다 — 일본판 스크립트는 자기 번호로 부른다
      if (Number(at) === 147) continue
  ...
  it('제어 코드가 온전하다', () => {
    ...
    // ⚠️ 배틀 뱅크 둘을 실으면서 셋 다 3,400쯤 늘었다 — 배틀 글 1,269줄은 거의
    // 다 이름·수를 빈칸으로 받는 문장 틀이고, 기술 줄 1,404개는 전부 이름 빈칸이 하나다
    expect(counted).toEqual({ en: 8539, ko: 6643, ja: 6860 })
  })
