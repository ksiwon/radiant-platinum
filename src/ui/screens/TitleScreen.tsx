...
// 세이브 파일 불러오기. 설정이 그 뒤에 작게 붙는다 — 빼면 게임을 열기 전에
// 설정을 볼 길이 사라진다.
//
// ⚠️ **설치 화면으로 가는 길은 평소에 안 세운다.** 여기까지 왔다는 것이 곧
// 설치가 온전하다는 뜻이라 다시 설치할 일이 없다. 뒤에서 훑다가 어긋난 것을
// 만났을 때만(`watchIntegrity`) 그 경고를 고치는 길로 선다.
//
...
export function TitleScreen() {
  ...
  /**
   * 설치 화면으로 가는 길 — **평소에는 안 세운다.**
   *
   * ⚠️ 여기까지 왔다는 것이 곧 설치가 온전하다는 뜻이라(부팅이 `installReady`를
   * 통과해야 타이틀이 열린다) 다시 설치할 일이 없다. 늘 세워 두면 쓸 일 없는
   * 단추가 하나 서 있고, 하필 그 끝이 에셋을 지우는 화면이다.
   *
   * ⚠️ **그런데 딱 한 자리에서 필요하다.** 위 `watchIntegrity`가 뒤에서 훑다가
   * 어긋난 파일을 만나는 때다 — persist가 거부된 채 공간이 모자라면 브라우저가
   * 설치본을 조용히 되찾아 간다 (e2e ㉔). 그때는 기록이 `ready`라 부팅은
   * 통과하는데 실제 파일이 깨져 있고, 이 길이 없으면 사이트 데이터를 통째로
   * 지우는 수밖에 없다 — 그러면 리포트까지 같이 날아간다.
   *
