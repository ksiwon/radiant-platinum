 ...
 *
 * ⚠️ **`unicodeRange`가 곧 다운로드 조건이다.** 화면에 그 코드포인트가 나와야
 * 그때 받는다. 그러니 **Pretendard가 이미 가진 글자를 여기 적으면 안 된다** —
 * 적으면 한국어 화면이 그 글자 하나 때문에 일본어 글꼴을 받고, 그 글자를
 * 일본 자형으로 그린다.
 *
 * ⚠️ **`U+2026`과 `U+3008-300F`가 그 자리였다.** 타이틀·설치 화면의 한국어
 * 안내문이 「시작」·「세이브 파일 불러오기」처럼 낫표를 쓰는데(`TitleScreen`
 * ·`ImportWizard`), 그 두 글자가 이 목록에 들어 있어서 **첫 화면에서
 * NotoSansJP 33.8kB를 받아 갔다** (`.audit/probe/wire.mjs` 실측 — 한국어로 열었는데도
 * 나갔다). 게다가 일본어 낫표는 전각이라 한국어 문장에 큰 빈칸이 생겼다 —
 * 같은 문장을 두 글꼴로 나란히 그려 놓고 봤다. 두 글꼴의 cmap을
 * fontTools로 세어 보면 겹치는 것은 정확히 이 아홉 자
 * (… 〈〉《》「」『』)뿐이고, 나머지는 Pretendard에 없어서 여기 있어야 한다.
 *
 * 대신 일본어 화면의 그 아홉 자는 Pretendard가 그린다 — 전각이 아니라 좁게
 * 붙는다. 한국어가 이 프로젝트의 기본 표시 언어라(`index.html`의 `lang="ko"`)
 * 그쪽을 맞춘다.
 */
const JP_RANGE = [
  'U+22EF', 'U+2640', 'U+2642', 'U+266B',
  // U+3008-300F(〈〉《》「」『』)는 뺀다 — Pretendard에 있다
  'U+3000-3007', 'U+3010-303F', 'U+3040-309F', 'U+30A0-30FF',
