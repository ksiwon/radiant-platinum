...
export const worldState = {
  player: {
    ...
    cycling: false,
    /**
     * 페달을 밟기 시작한 뒤 **지나온 거리**(타일). 속도 단이 여기서 오른다.
     *
     * ⚠️ **시간이 아니라 거리다.** 원작은 한 걸음에 한 단씩 올리고 한 걸음이
     * 곧 한 칸이다 (`PlayerAvatar_AccelerateBike`). 시간으로 세면 빠를수록
     * 더 빨리 오르는 되먹임이 생긴다
     */
    pedalling: 0,
    /**
     * 자전거의 모드 (`PlayerData.cyclingGear`) — 0이 3단, 1이 4단이다.
     *
     * ⚠️ **속도 단과 다른 것이다.** 3단은 늘 2.67배 한 속도고 4단은 걸음마다
     * 올라 4배까지 간다. **진흙 비탈은 4배로만 오르므로 3단으로는 못 오른다**
     * (`actor/bikeTerrain`). 원작의 초기값이 3단이라 그대로 둔다
     */
    bikeGear: 0 as 0 | 1,
    /**
