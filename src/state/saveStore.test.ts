...
describe('박스에 맡기고 꺼내기', () => {
  const mon = (species: number, hp = 20): PokemonInstance => ({
    ...
    friendship: 70, isEgg: false, otId: 0, otSecretId: 0, ball: 0, form: 0, pokerus: 0, mail: null,
  })
...
describe('스크립트가 주는 것', () => {
  const mon = (species: number, friendship = 70, ball = 0): PokemonInstance => ({
    ...
    origin: noOrigin({ name: '', gender: 'male' }),
    friendship, isEgg: false, otId: 0, otSecretId: 0, ball, form: 0, pokerus: 0, mail: null,
  })
