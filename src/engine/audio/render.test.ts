...
maybe('악기표와 파형 창고', () => {
  /**
   * ⚠️ **문턱을 명시한다 — 이것이 REPAIR §14의 그 빨간 줄이다.**
   *
   * 이 시험 하나가 `bnk` 521개와 `war` 521개(7.5MB)를 전부 열어 파싱한다.
   * 혼자 돌면 **0.6초**인데, 전체 판에서는 시험 파일 300개가 같이 디스크를
   * 두드려서 **6.4초**가 걸린 판을 잡았다 (2026-08-27 실측) — 기본 문턱 5초를
   * 넘어 「시간 초과」로 떨어진다. 값이 틀려서 난 것이 아니고 실제로 늦어서 난다.
   *
   * §14가 "시간 초과가 아니다"라고 적어 둔 것은 **혼자 돌 때의 0.6초**를 보고
   * 내린 판단이었다. 떨어지는 판은 언제나 **같이 도는 판**이라 그 수는 근거가
   * 못 된다. 같은 갈래를 `unityfs.test.ts`에서 이미 한 번 밟았고(파이썬을 띄우느라
   * 5.8초) 거기서도 답은 문턱이었다.
   *
   * 실측 6.4초의 열 배로 잡는다. 여유를 크게 두는 쪽이 옳다 — 이 문턱이 재는
   * 것은 성능이 아니라 **표본 번호가 맞는가**이고, 성능은 헤드리스에서 못 잰다
   */
  const SLOW = 60_000
