 ...
 * 가운데까지는 많아야 0.5이고, **칸 안이면 어디서든** 끈다 — 칸 번호는 `floor`라
 * 168.875는 게임에게도(`data-tile`) 원작으로 쳐도 168번 칸에 선 사람이다.
 *
 * ⚠️ **한때 0.35였다.** 「그보다 멀면 옆 칸에 선 사람」이라고 적었는데 틀렸다 —
 * 실측(2026-09-24 `_school43`): 트레이너 스쿨 문 (168,776)은 밟고 드는 칸이고,
 * 동쪽에서 걸어온 주인공이 168.875(가운데에서 0.375)에 서서 북쪽을 밀자 문이 안
 * 열렸다. 대표 구간 1판·8판이 거기서 판 전체를 잃었다 (REPAIR §72)
 */
const CORNER_ASSIST = 0.5
