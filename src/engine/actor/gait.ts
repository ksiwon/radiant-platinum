...
const AMP = {
   ...
   *
   * dawn.glb의 스킨 가중치로 잰 값: 팔꿈치 높이의 몸통 반폭 0.167, 팔 반지름 0.056.
   * 1.30rad(74°, 수직에서 16°)이면 팔꿈치가 몸통 표면에서 팔 반지름의 0.95배 —
   * 팔 겉면이 옆구리에 막 닿는 자리다. 1.20이면 1.54배로 날개처럼 뜨고,
   * 1.38이면 0.46배로 옆구리를 파고든다.
   * `locomotion.test.ts`의 "팔이 몸통을 파고들지도 뜨지도 않는다"가 양쪽을 못박는다.
   *
   * 달리기 추가분이 작은 이유도 같다 — 이미 몸에 닿아 있어서 더 붙일 데가 없다
   */
  armDrop: 1.30, armDropRun: 0.04,
  /**
