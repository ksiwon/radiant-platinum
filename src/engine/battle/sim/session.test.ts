...
import { speciesFileSchema, moveFileSchema, type Species } from '../../../data/schema'
...
function spawn(id: number, level: number, seed: number): SideMon {
  ...
  for (const slot of mon.moves) slot.pp = moves.get(slot.move)?.pp ?? 5
  return { mon, species: sp, label: names[id]! }
}
...
function attackSlot(side: SideMon): number {
  ...
  if (i < 0) throw new Error(`${side.label}에게 공격 기술이 없다`)
  return i + 1
}
