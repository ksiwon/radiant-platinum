...
import { BATTLE_BANK, MOVE_BANK, STAT_BANK } from '../../../ui/battle/romText'
...
const MOVE_LINES = read<string[]>('dialogue/ko/' + String(MOVE_BANK) + '.json')
...
const NAMES = {
  ...
  stats: read<string[]>('dialogue/ko/' + String(STAT_BANK) + '.json'),
}
...
async function soak(plan: Plan): Promise<Broken> {
  ...
  const beats = buildBeats(all, (e) => battleText(e, { names: NAMES, lines: LINES, moveLines: MOVE_LINES, label }))
