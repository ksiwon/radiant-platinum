...
/**
 * 우리 도구 이름 → sim 도구 이름. 배틀에서 아무 일도 안 하는 도구면 null.
 *
 * ⚠️ **번호로는 못 잇는다.** 롬의 도구 번호와 sim의 `num`이 서로 다른 체계라
 * 실측하면 467개 중 **한 개도 안 맞는다.** 이름은 밑줄만 빼면 그대로 sim의
 * id다 — 홀드 효과가 있는 도구 158개 중 **149개**가 그렇게 찾힌다.
 *
 * ⚠️ **못 찾는 아홉은 배틀 도구가 아니다** — 학습장치·평온의방울·부적금화·
 * 정화의부적·연막탄·변함없는돌·럭키에그·행운의향로·청정의향로. sim의 4세대
 * 덱스에 배틀 동작이 없는 것이 맞고, 그래서 안 넘기는 것이 맞다
 */
export function simItem(name: string): string | null {
  const it = gen4.items.get(name.replace(/_/g, ''))
  return it.exists ? it.id : null
}
