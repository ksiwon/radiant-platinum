...
maybe('코퍼스 전량', () => {
  ...
  it('제어 부호가 하나도 글자로 새지 않는다', () => {
    ...
    expect(leaked).toEqual([
      ...
      // 배틀 글 뱅크의 843~845번. **미국·한국 롬에서는 통째로 지워진 줄이고**
      // (`Dummy843`~`Dummy845` — 빈칸만 남아 있다) 일본 롬에만 옛 글이 남아 있다.
      // 게임이 여는 자리가 없으므로 화면에 뜰 일도 없다
      'ja/368: は　{STRVAR_1 9}に',
      'ja/368: は　{STRVAR_1 9}に',
      'ja/368: は　{STRVAR_1 9}に',
    ])
 ...
 *
 * ⚠️ 배틀 네 뱅크를 실으면서 11,756에서 여기까지 뛰었다 (PARITY §2.24 · §2.26) — 배틀 글
 * 1,269줄은 대부분이 이름·수·기술을 빈칸으로 받는 문장 틀이고, 기술을 쓰는 줄
 * 1,404개는 전부 이름 빈칸이 하나씩이다
 */
const MARKER_COUNT = 22_293
