...
export interface FieldServices {
  ...
  fieldMoves?: {
     ...
     *
     * ⚠️ **여기가 몸을 옮기는 밑바닥이다.** 앞 칸에 대고 A를 누른 길이든 기술
     * 창에서 고른 길이든 `FIELD_MOVES` 스크립트를 거쳐 들어오고, 그 안의
     * `UseSurf`·`UseWaterfall`·`UseRockClimb`가 이 자리로 온다. 자격도 "쓰겠습니까"도
     * 스크립트가 이미 봤으므로 여기서는 지형만 본다 — **다시 스크립트를 걸면
     * 고리가 된다**
     *
