 ...
 * **고정 시간을 기다린 뒤** 자리를 읽고 있었다 — 느린 기계에서는 아직
 * 갈아 끼우기 전을 읽고 「엉뚱한 자리로 돌아왔다」로 적었다. 기다릴 것은
 * 시간이 아니라 상태다. 지원 문의에도 같은 값이 필요하다("불러오면 딴 데서
 * 시작해요"). 값을 써서 게임을 움직일 수 있는 길은 없다 — 이 파일의 다른
 * 표식과 같다.
 *
 * ⚠️ **값이 둘이다** (`state/restoreStore`). `loading`과 `failed`는 밖에서
 * 똑같이 「안 움직이는 화면」으로 보이는데, 앞은 기다리면 되는 것이고 뒤는
 * 사람이 골라야 끝난다 — `data-renderer`가 셋을 가르는 것과 같은 까닭이다
 */
export function markRestoring(phase: 'loading' | 'failed' | null): void {
  put('restoring', phase)
}
