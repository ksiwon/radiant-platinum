...
import { DIR, type Movable } from '../engine/script/movement'
import {
...
// ── 기라티나 그림자 (`DistWorldGiratinaShadowProp`) ───────────────────────────
...
/** 지금 그림자가 지나가는 중인가 */
export function distortionShadowRunning(): boolean {
  return shadow !== null
}
...
// ── 기라티나가 내려온다 (`EventCmdPlayGiratinaArrival`) ───────────────────────
...
/** 내려오는 중인가 */
export function distortionArrivalRunning(): boolean {
  return arrival !== null
}

function beginArrival(): boolean {
...
/** 한 프레임. 다 끝났으면 true */
function tickArrival(dt: number): boolean {
  const run = arrival
