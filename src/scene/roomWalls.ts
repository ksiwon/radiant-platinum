...
// `gapsOn`이 가른다.
//
// ⚠️ **출입구에는 문 높이만큼 구멍을 남긴다 — 비우지도, 메우지도 않는다.**
// 양 극단을 다 해 봤고 둘 다 틀렸다:
//
// - **통째로 비웠더니** 문간 칸에 선 사람이 정면을 볼 때 화면이 통째로 검었다.
//   원작이 그쪽 벽을 아예 안 만들었기 때문이다.
// - **통째로 메웠더니** 문이 사라졌다. 파일럿 보고(2026-09-22): 「건물 내부 문
//   쪽에는 벽이 없이 뚫려 있어야 하는데 어색하게 문 쪽 벽이 메워져 있다」.
//   실측으로 집 1층(맵 414)의 문간 (8,12)에서 남쪽을 보면 줄무늬 벽 한 장뿐이고
//   (`pnpm shot door --eye=8.5,3,7 --gaze=8.5,1.2,13`), 그 판을 숨기면 뒤가
//   `#000001`이다 — **원작에는 거기 아무것도 없다** (`--blame=400,350`).
//
// 그래서 문간 칸에는 **인방(lintel)만** 세운다. 바닥에서 `DOOR_OPEN`까지는 비고
// 그 위로만 벽이 이어지므로, 옆 칸의 온전한 벽이 문설주가 되어 **문 모양 구멍**이
// 남는다. 그 구멍 너머가 검은 것은 고장이 아니라 어두운 문간이다.
//
// ⚠️ **통행과는 상관이 없다.** 이 판은 `<mesh name="방 벽">` 한 장으로 그려질 뿐
// 충돌에 안 들어가고, 워프는 그 칸에 올라서는 순간 터진다 — 비우든 메우든
// 드나드는 것은 격자가 정한다.
//
// 여기서 세우는 것은 원작에 없는 벽이고, 그림은 그 방의 벽에서 베낀다.
//
 ...
 * 문간 칸에 남기는 **구멍의 높이** (타일).
 *
 * 원작 문짝 19종의 윗변을 재면 제일 높은 것이 2.75타일(`gym_door`·`card_door`)
 * 이고 아홉이 1.938이다 (`.audit/tmp/probe/doorHeight.probe.ts`). 그 사이에서
 * 사람 키(눈높이 1.5타일)보다 넉넉히 높은 값을 잡는다 — 낮으면 문이 개구멍이
 * 되고, 방 벽(3~4타일)만큼 높이면 인방이 안 남아 구멍이 벽의 끝으로 보인다
 */
const DOOR_OPEN = 2.25
...
export function roomWalls(
  ...
  /**
   * 문간 칸들 (`plates.cellKey` · 청크 로컬).
   *
   * 여기 든 칸에는 `DOOR_OPEN` 위로만 세운다 — 머리말의 「문 모양 구멍」이다.
   * 안 주면 예전처럼 다 메운다(시험과 실외가 그 길이다)
   */
  ...
  for (const [key, y] of floor) {
    ...
    for (const [dx, dz] of SIDES) {
      if (floor.has(cellKey(tx + dx, tz + dz))) continue
      /**
       * 메운 구멍의 바깥쪽은 **옆이 안 서도 세운다** — 그러나 **원작이 덮는 띠는
       * 비운다.**
       *
       * 안 세워 봤더니 1인칭 광선이 9발에서 66발로 늘었다 (계단 자리). 그래서
       * 세우는 것은 맞다. 그런데 한동안 **덮인 띠까지 통째로** 세웠고, 그러면
       * 원작 벽 **앞에** 판이 한 장 더 선다 — 파일럿 보고(2026-09-22):
       ...
       * ```
       *
       * 0.4칸 앞에 서 있고, `nearestWall`이 고른 띠가 **어두운 기둥**이라 올리브
       * 벽지 앞에 짙은 판때기로 보인다. 그래서 여기서도 `gapsOn`으로 **안 덮인
       * 띠만** 메운다 — 옆이 서든 말든(`propped` 참) 세우는 것만 다르다.
       *
       * 같은 평면이 되는 자리는 방 안쪽으로 `SHIFT`만큼 물려 깊이 다툼을 피한다
       */
      ...
      const full = gapsOn(
        cover.get(edge), y, top, filled || besideKeys(edge).some(stands))
      // ⚠️ **문간은 발치를 비운다** (머리말). 인방만 남으므로 옆 칸의 온전한
      // 벽이 문설주가 되고, 그 사이가 문 모양 구멍이다
      const gaps = doors?.has(key) === true
        ? full.map(([lo, hi]): Band => [Math.max(lo, y + DOOR_OPEN), hi])
          .filter(([lo, hi]) => hi - lo > SEAM)
        : full
