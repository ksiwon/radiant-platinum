...
| 대사 | `msgdata/pl_msg.narc` | ✅ 뱅크 482개 — 미국 685개가 디컴프 원문과 완전 일치 (§2.11) |
| 아이템 | `itemtool/itemdata/pl_item_data.narc` | ✅ 468종 — 디컴프 446개와 필드 대조, 불일치 0 (§2.12) |
| 장식 이름 | `msgdata/pl_msg.narc` 뱅크 386 | ✅ 100줄 — 61번부터가 하나뿐인 장식이다 (PARITY §7.16) |
...
| **트레이너 배틀 그림** | `poketool/trgra/trfgra.narc` | ✅ 갈래 105종 전부 (§2.17.4). 명예의 전당이 주인공을 여기서 꺼낸다 |
| **깨어진 세계** | `fielddata/tornworld/tw_arc{,_attr}.narc` | ✅ 맵 10 · 떠 있는 판 10 · 통행 격자 12벌 · 사건 45자리 · 승강 발판 18/34 · 경로 22 · 소품 212자리 · 방아쇠 144 (§2.22) |
| **깨어진 세계 소품 모델** | `data/mmodel/fldeff.narc` 0x7C~0x94 | ✅ 25종 NSBMD · 삼각형 3,390 · 183KB (§2.22). 같은 길로 장막 체육관 셋과 **나무열매 밭의 흙**(17번)도 굽는다 |
| **기술 연출 대본** | `res/moves/<이름>/anim.s` (디컴프) | ✅ 468개 12,514줄 → 색·박자·흔들림·자리 (§2.23) |
...
⚠️ **통행표는 방 밖을 안 막는다.** 32×32를 다 담는데 방은 그중 일부만 쓰고, 남는
칸은 `0x0000` — 걸을 수 있음이다. 원작도 그 비트 하나만 보므로 안 막는다
(`terrain_collision_manager.c`의 `CheckCollision`). 위에서 내려다보는 화면에서는
갈 일이 없어서 문제가 안 됐지만 3인칭에서는 **바닥이 안 그려진 자리에 선다** —
걸어서 닿는 칸 110,809개 중 4,170개가 그랬다. 그래서 굽는 쪽이 청크마다
「그 칸 밑에 삼각형이 있는가」를 32×32 비트로 적고(`data/chunks/cover.bin`,
청크당 128B · 666개 85KB) 소품은 XZ 상자를 싣는다(`data/props/index.json`의
`boxes`, `posScale` 단위 정수 넷). 막는 것은 `engine/map/floorSeal`이고 자세한
것은 [REPAIR.md](REPAIR.md) §22에 있다.
...
⚠️ **텍스처가 없는 재질은 확산색(`diffAmb`의 아래 15비트)이 유일한 색이다.**
정점색이 흰색 하나뿐이라 이 값을 안 실으면 화면에 **하얗게** 뜬다 — 그림자
재질(`kage`·`shade`·`lm*`, 확산 (0,0,0)~(66,82,107))이 어두운 그림자가 아니라
흰 안개로 깔렸던 자리다. 실측:

| 어디 | 재질 | 텍스처 없는 것 | 그 파일 |
|---|---:|---:|---:|
| `data/chunks` (맵) | 7,346 | 170 | 140 |
| `data/props` (건물 소품) | 1,333 | 117 | 109 |
| `data/distortionProps` | 118 | 4 | 3 |
| `data/starter` | 37 | 0 | 0 |
...
⚠️ **떡잎마을 땅에 있는 흰 얼룩은 고장이 아니다.** 그림을 보고 「물이 하얗게
샜다」고 읽기 쉬운 자리라 근거를 남긴다 — 그 자리를 찍어 물으면
(`pnpm shot twinleaf --hour=12 --hit=700,470`) **지형 재질 `s_snow04`**가
모래(`nsandp`) 면 **위 0.1**에 덧깔려 나온다. 그 이름은 떡잎마을이 쓰는 묶음
(영역 6)에 실제로 들어 있고(`s_snow` · `s_snow02` · `s_snow04`, 74벌 중 6·61번만
갖고 있다), 팔레트까지 먹여 펴 보면 **뭉게뭉게한 흰 얼룩**이다. 201번도로 모랫길
에도 같은 것이 흩어져 있다. 플래티넘이 초반 지역을 추운 쪽으로 다시 칠하면서
넣은 눈 얼룩이고, **우리가 만든 것이 아니라 롬 청크가 이름으로 가리키는 것**이다.
...
#### 깨어진 세계에는 세 칸을 가는 뜀이 따로 있다

거동값 `0x5A`~`0x5D`(`TILE_BEHAVIOR_JUMP_NORTH_TWICE`·`SOUTH`·`WEST`·`EAST`)는
**깨어진 세계에만** 있다. 열 층에 224칸이고 오버월드와 나머지 실내에는 하나도
없다 — 세어 보면 그렇다.

| 층 | 걷는 칸 | 두 칸 뛰기 | 층 | 걷는 칸 | 두 칸 뛰기 |
|---|---|---|---|---|---|
| 1F (573) | 92 | 16 | B4F (577) | 493 | 34 |
| B1F (574) | 102 | 20 | B5F (579) | 408 | 30 |
| B2F (575) | 196 | 10 | B6F (580) | 251 | 42 |
| B3F (576) | 3,161 | 34 | B7F (581) | 92 | 10 |
| 기라티나 방 (582) | 19 | 6 | 귀혼동굴 방 (583) | 272 | 22 |

**이 뜀이 없으면 그 세계는 통째로 끊긴 섬이다.** 발판 사이가 두 칸씩 비어 있고
그 두 칸은 통행 불가라, 걸어서는 다음 발판에 못 간다. 1F가 제일 뚜렷하다 —
승강 발판이 있는 우묵한 자리(지역 x 17~21 · z 42~46)는 사방이 막혀 있고, 들어가는
길은 북쪽 통로 (19, 39)에서 남쪽으로 뛰는 것 하나뿐이다.

**거리는 세 칸이다.** 원작이 프레임마다 `FX32_CONST(2)`씩 `8 * 3`프레임을 옮기고
(`MovementAction_JumpDistortionWorldNorth`), 한 타일이 `FX32_ONE * 16`이라
2 × 24 ÷ 16 = 3칸이다. 턱은 같은 식으로 2 × 16 ÷ 16 = 2칸이다.

자료가 그 값을 뒷받침한다 — 224칸 **전부**에서 「한 칸 뒤」(출발)와 「세 칸 앞」
(착지)이 둘 다 걸을 수 있는 자리다. 두 칸이면 착지가 나머지 한 짝 위(통행 불가)에
떨어지고, 네 칸이면 허공이다.

값이 확정되는 이유는 이름에 16진수를 담은 항목들이다: `UNUSED_x55` 다음이
`PASTORIA_GYM_H_GROUND`(0x56)이고 `UNKNOWN_x60` 앞이 `WARP_STAIRS_WEST`(0x5F)라,
그 사이 열 개가 한 칸씩 맞아떨어진다.

#### 그 세계에서는 지면을 안 따라간다 — 서 있는 높이가 한 값이다

원작은 깨어진 세계에 들어서면서 주인공의 높이 계산을 **꺼 버린다** —
`InitPlayer`의 `MapObject_SetHeightCalculationDisabled(playerMapObj, TRUE)`이고,
서쪽 벽에 붙어 있을 때만 푼다. 그래서 그 세계의 y는 지형에서 읽는 값이 아니라
**들고 다니는 상태**고, 바꾸는 것은 승강 발판·뛰는 자리·움직이는 발판·벽 걷기뿐이다.

처음 서는 높이는 `LoadFloor`가 한 줄로 적어 둔다 —
`playerPos.y = mapOffset.y + MAP_OBJECT_TILE_SIZE`, 즉 **층 오프셋 + 한 칸**.
층을 오르내려도 그대로다: 승강 경로의 y 변화(−32 · −14 · −50)가 층 오프셋의
차이와 정확히 같다.

| 층 | 오프셋 y | 다음 층까지 | 층 | 오프셋 y | 다음 층까지 |
|---|---|---|---|---|---|
| 1F | 288 | −32 | B4F | 160 | −32 |
| B1F | 256 | −32 | B5F | 128 | −14 |
| B2F | 224 | −32 | B6F | 114 | −50 |
| B3F | 192 | −32 | B7F | 64 | — |

⚠️ **판의 bounds를 높이로 쓰면 안 된다.** B2F의 바닥 판 넷은 세계 y=233,
지역 9에 있는데 그건 서쪽 벽(지역 y 1~9)을 타고 올라가야 닿는 위층이다.
그것을 서 있는 높이로 쓰면 걷는 바닥에서 **여덟 칸이 뜬다** — 실측으로 걸을 수
있는 196칸 중 170칸의 그림이 지역 1에 있고 지역 9에는 셋뿐이다.

⚠️ **BDHC는 있는데 성기다.** 1F는 걸을 수 있는 92칸 중 47칸에만 판이 있고
(나머지 40칸은 판이 없는데도 그림이 지역 1에 있다), B3F는 3,161칸 중 252칸뿐이다.
그래서 BDHC로도 이 세계의 높이를 못 정한다 — 애초에 원작이 안 읽는다.

⚠️ **B3F의 넓은 어두운 바닥은 지역 0에 그려져 있다** (걸을 수 있는 3,161칸 중
2,700칸). 그런데 그 층의 방아쇠 74개와 카메라 각 여섯이 **전부 지역 y=1**이라
(`bounds.y == bounds.y + sizeY == 193`), 걷는 높이는 1이 맞다. 즉 그 위를 지날
때는 한 칸 떠서 간다 — 롬 자료가 그렇게 되어 있고, 손으로 메우면 방아쇠 74개가
통째로 안 걸린다.
...
⚠️ **그 면들이 검은 그림자를 던지고 있었다.** three가 그림자를 그릴 때 쓰는
깊이 재질이 원래 재질에서 가져가는 것은 `alphaTest`와 `map`뿐이라, 섞으려고
`alphaTest: 0`을 준 면은 **텍셀 알파와 상관없이 꽉 찬 실루엣**으로 찍힌다 —
천관산 빛기둥이 바닥에 검은 자국을 남겼다. `castShadow`는 오브젝트마다라 재질
무리별로 끌 수가 없어서, 무리를 **두 메시에 나눠 담는다**(`chunkMesh`의
`splitShadow`). 정점과 색인은 그대로 나눠 쓰고 three가 무리마다 콜 하나를
내므로 **드로우콜이 안 는다** — 실측으로 천관산에서 68로 같고, 그림자 메시에
남은 섞는 무리가 1개 → 0개다. ⚠️ **거기서 나온 기하는 버리면 안 된다**:
정점 버퍼가 원본 것이다.
...
⚠️ **사람을 품은 상자는 가리는 것이 아니다.** 배틀타워의 상자가
17.3 × 21.6 × 20.4타일이라 광장을 통째로 덮는다 — 카메라도 사람도 그 안에
서므로 잰 거리가 0이 되어 불투명도가 0.013까지 내려가고 **건물이 통째로 안
그려졌다.** 화면에 남은 것은 따로 배치된 문짝(`gym_door00`) 하나라 파란 판이
허공에 떠 있었다. 배틀파크도 같은 자리였다. 사람이 그 안에 서 있으면 그것은
둘러싼 배경이지 사이에 든 것이 아니다 (REPAIR §19).
...
arm9 안 절대 오프셋 **`0xea01c`**(미국판), 24바이트 × 593개. NARC 파일이 아니라 코드 영역이라 주소로 찍어야 한다. 한국판은 `0xeaaa4`, 일본판은 `0xe96f0`이고, 브라우저 쪽은 주소를 박는 대신 표의 성질로 찾는다(`findHeaderTable`). **세 판의 표는 `msg` 열 하나만 다르다.**
...
+6  u16 initScripts
+8  u16 msgArchive      pl_msg 뱅크 번호 (⚠️ **그 롬의 번호**다 — §2.11)
+10 u16 musicDay
+12 u16 musicNight
...
전국도감이 493종이고 494·495가 알과 나쁜알인데 파일은 508개다. 남는 열둘이
폼마다 종족값이 다른 자리다 (`Pokemon_GetFormNarcIndex`, PARITY §3.4):

| 칸 | 폼 |
|---|---|
| 496~498 | 테오키스 어택 · 디펜스 · 스피드 |
| 499~500 | 도롱마담 모래 · 쓰레기 |
| 501 | 기라티나 오리진 |
| 502 | 쉐이미 스카이 |
| 503~507 | 로토무 히트 · 워시 · 프로스트 · 팬 · 모우 |

⚠️ **폼이 열셋인데 칸은 다섯 종뿐이다.** 안농 스물여덟도, 캐스퐁 넷도, 아르세우스
열여덟도 종족값이 하나라 기본형 칸을 그대로 쓴다 — 그 갈림이 안 맞으면 도롱마담
쓰레기가 방어 85(풀)로 싸운다. `evo`·`wotbl`도 같은 508칸이라 폼 기술표가 여기서
같이 온다 (`Pokemon_LoadLevelUpMovesOf`).

⚠️ **크기 표(`zukan_data.narc`)는 494칸뿐이다.** 폼 자리가 비어 있어서 그대로 두면
저울질과 풀묶기가 도롱마담 쓰레기에게 **0kg**으로 걸린다. 원작은 그 표를 종족
번호로만 읽으므로(`Pokedex_HeightWeightData_Weight(data, species)`) 폼은 기본형과
같은 크기이고, 추출기가 그렇게 채운다. **기라티나만 예외다** — 기본 표의 487번이
오리진(6.9m·650kg)이고 어나더(4.5m·750kg)가 `zukan_data_gira.narc`에 따로 있다.
...
+100  u32 무리[2] · 낮[2] · 밤[2] · 탐지기[4] · 폼[5] · 안농표 · 듀얼슬롯[10]  104 → 204
...
**+140의 다섯 칸이 폼이다** (`encounterRatesForms`, PARITY §3.4). 이름은 "출현률"인데
쓰이는 것은 앞의 둘뿐이고 그것도 **참/거짓으로만** 쓴다 — 0이면 서쪽 바다, 아니면
동쪽이다(`AddWildMonToParty`). 실측으로 값이 0 아니면 100 둘뿐이고, 조개무지·
트리토돈이 실제로 나오는 표 아홉 개가 서쪽 다섯(운하시티·골짜기발전소·골풀무제철소·
205·218번도로)과 동쪽 넷(들판시티·212·213·224번도로)으로 깨끗이 갈린다.

**+160이 안농 글자 표 번호**다. 0이면 안농이 안 나오고, 1을 빼서 표 여덟 중 하나를
고른다(`InitEncounterFieldParams`). 실측으로 표를 가진 맵이 열여덟 개고 전부
유적마을 유적이다 — 막다른 방 열하나가 0번 표(F·R·I·E·N·D를 뺀 스무 글자), 「올바른
길」 여섯이 F-R-I-E-N-D를 하나씩, 비밀방 하나가 「!」와 「?」다.
...
**분류별 상금 배수는 NARC에 없다.** 105바이트 표가 배틀 오버레이 **#16** 안에 그대로 박혀 있고, ⚠️ **자리가 지역판마다 다르다** — 미국판·일본판 `+0x359E0`, 한국판 `+0x35A00`이다(한국판 오버레이가 0x20 크다: 0x35AA0 대 0x35A80). 알맹이 105바이트는 세 판이 **바이트로 같다**. 자리는 `supported.json`의 `prizeOffset`에 판별로 적혀 있고 두 굽는 쪽이 그 한 줄을 같이 읽는다.

⚠️ **"오버레이 안에서 딱 한 군데"로는 자리가 안 정해진다.** 미국판 자리를 한국판에 쓰면 32바이트 앞을 읽는데, 그 창은 앞이 0 패딩이라 「앞 두 칸이 0」을 통과하고 안에 포인터 한 워드가 들어와 「유일」까지 통과한다 — 105칸 중 98칸이 조용히 틀리고 28분류는 상금이 0이 된다(챔피언 배수 50이 16으로, 캠프보이 4가 245로 읽혔다). 그래서 검사가 셋이다: 앞 두 칸 · 유일성 · **0이 이어지는 길이**. 마지막 것이 실측으로 갈린다 — 참 표는 6칸(끝의 안 쓰는 분류)이고 밀린 창은 26칸이라, 그 사이 12로 끊는다.
...
갈아 끼우는 것보다 **내려놓는 것**이 더 자주 틀린다. 셋을 실측으로 못 박았다
(`script/field.ts` · `script/azureFlute.test.ts`):

⚠️ **끝난 스크립트의 뱅크를 창에 남기지 않는다.** 부른 공용 스크립트는 돌아오면서
부모 뱅크로 되돌아가지만, 도구나 좌표 사건처럼 공용 스크립트를 **바로** 시작한
판은 되돌릴 부모가 없다. 스크립트가 끝나는 자리에서 맵 뱅크로 내려놓는다 —
안 그러면 **다음** 스크립트가 남의 뱅크에서 같은 번호를 읽는다. 실측: 천계의피리로
시작의 방에 올라가니 아르세우스의 「구콰-쾅!!」 자리에 **포켓몬센터 접수원의
인사**가 떴다(뱅크 213의 0번).

⚠️ **워프는 도는 스크립트의 뱅크를 뺏으면 안 된다.** `Warp`는 스크립트 한복판에서
걸리고(천계의피리가 그렇다), 워프 뒤에도 **같은 스크립트가 제 뱅크로** 말을 잇는다.
맵을 갈아 끼우면서 걷어 버리면 「빛의 계단이 나타났다!」가 맵 뱅크의 엉뚱한 문장이
된다. 맵을 옮길 때 걷는 것은 **도는 스크립트가 없을 때뿐**이다.

⚠️ **장부만 비우면 더 나쁘다.** 「지금 무슨 뱅크인가」를 들고 있는 칸과 창에 실제로
얹힌 뱅크는 늘 같이 움직여야 한다. 칸만 비우면 다음 되돌리기가 「이미 맵 뱅크다」로
보고 그냥 지나가서, 남의 뱅크가 창에 **더 오래** 눌어붙는다.
...
"몇 개를 만들었나"(360/840)는 눈금이 못 된다. 안 쓰이는 명령이 태반이라서다.
...
| 닿는 명령 자리 | 55,778 |
| 그중 도는 것 | **54,990 (98.6%)** |
| 안 도는 것 | 788 (서로 다른 명령 223종) |

⚠️ **닿는 자리 수가 한 번 315만큼 늘었다 — 세는 법이 틀려 있었다.** 여섯 명령은
길이가 첫 인자 값에 달렸는데(`DoStrengthFunc`·`DoFlashFunc`·`DoDefogFunc`·
`DoGroupConnectionAction`·`CallTVBroadcast`·`MysteryGiftGive`) 세는 쪽이 그것을
안 보고 `args`만 더하고 있었다. 엔진의 건너뛰기는 제대로 보고 있었으므로 **도구만
어긋난 것**이고, 지금은 둘이 같은 함수를 쓴다 (`script/context`의 `operandWidth`).
...
| ~~명예의 전당 (`ClearGame`·`OpenPCHallOfFameScreen` 등)~~ | 3 | ✅ 붙였다 (PARITY §7.11) |
...
배치표를 고치는데, 그건 **사람을 세우기 전에** 돌아야 한다. 예지호수의 마박사가
...
걸린다 — 방에서 일어나 라이벌이 뛰어드는 장면, 예지호수에서 마박사가 돌아보는
...
⚠️ **없는 뱅크와 다시 쓴 뱅크는 다르다.** 일본판 `game_corner`(us #147)는 키도
항목 수도 안 맞아서 한동안 "일본판에 없음"으로 세고 있었다. 열어 보면 슬롯머신
얘기가 든 26줄짜리 게임코너 뱅크다 (미국판 28줄). 없다고 적힌 동안 트바리
게임코너 맵(#136)이 가리키는 뱅크에 us 번호가 없어서 **그 맵의 대사가 통째로
빠졌다.** 순서는 보존되므로, 앞뒤로 확정된 뱅크 사이에 빈자리가 딱 하나면 그것이
답이다 — `textbanks.js`의 `fillByPosition`이 그 자리를 메우고, 브라우저 쪽은
`textBanks.ts`가 같은 규칙을 이름 순서로 계산한다.
...
#### 맵 헤더의 `msg`는 그 롬의 번호다 — 되돌려야 한다

⚠️ **여기가 배포된 게임의 대사를 통째로 어긋나게 했던 자리다.** 산출물의 대사
파일 이름은 us 번호인데(`data/dialogue/<로케일>/<us 번호>.json`), 맵 헤더 표는
지역판마다 따로 있고 그 `msg` 열은 **자기 롬의 뱅크 자리**를 가리킨다.

헤더 24바이트를 세 판에서 견주면 다른 자리가 **8·9번(`msg`) 하나뿐**이고 593줄이
전부 다르다. 첫 줄부터 그렇다 — 떡잎마을은 us 23인데 ko 22다(us #13
`ball_seal_names_plural`이 한국 롬에 없어서 그 뒤가 한 칸씩 당겨진다).

되돌리지 않으면 맵마다 **이웃 뱅크**를 읽는다. 파일이 있으니 화면은 멀쩡히 뜨고
글만 딴 것이 나온다 — 간판을 읽으면 배틀타워 성적표가 뜨고, 상점 점원이 마을
사람 대사를 말한다. 개발 서버는 미국 롬으로 구운 `public/data`를 쓰기 때문에 이
길을 안 밟는다. **설치한 사람에게만 보인다.**

로케일 번호 → us 번호는 `usBankIndex`가 되돌리고(`textBanks.ts`), 짝이 없으면
던진다. 되돌린 뒤에는 세 판의 `maps.json`이 **바이트로 같다** — `maps.test.ts`가
그것을 잰다.
...
#### 글자는 한 자씩 안 찍는다 — **버튼 없이는 아무것도 안 넘어간다**

원작 인쇄기(`src/render_text.c`)는 프레임마다 한 자씩 내고, 설정의 글자 속도가
그 사이 프레임 수를 정한다(느림 8 · 보통 4 · 빠름 1). 그 길에 **누르고 있으면
빨라지는 갈래**(`RenderControlFlags_SetCanABSpeedUpPrint`)와 **스스로 넘어가는
갈래**(`autoScroll`)가 붙어 있다.

⚠️ **셋을 다 걷어냈다.** 우리 인쇄기(`engine/script/printer`)는 **쪽을 통째로
올리고 거기서 버튼을 기다린다.** 넘어가는 자리가 `tick()`의 `input.pressed`
하나뿐이라, 「누르지 않았는데 다음이 진행된다」가 원리적으로 안 생긴다.

| 자리 | 원작 | 우리 |
|---|---|---|
| 글자 사이 | 속도 프레임만큼 쉼 | 없다 — 쪽이 한 번에 뜬다 |
| 쪽 넘김 `
`·`` | 버튼 | 버튼 (같다) |
| **글의 끝** | 인쇄가 끝나면 곧바로 `Message`가 돌아온다 | **버튼을 기다린다** |
| 빨리 감기 · 자동 넘김 | 있다 | 없앴다 |
| 설정의 글자 속도 | 셋 | 없앴다 |

**끝에서 기다리는 것이 핵심이다.** 원작은 글 끝에 `
`이 붙어 있어 거기서
멎는데, `
` 없이 끝나는 글도 많다 — 떡잎마을 주인공 집 2층 뱅크(558)를 세면
열한 줄 중 여섯이 그렇다. 그런 글은 창이 뜨자마자 사라져서, 컷신에서는 대사
다섯이 한 호흡에 지나갔다.

⚠️ **`Message` + `WaitButton`이 붙은 자리는 겹친다.** 원작은 `Message`가 인쇄만
끝내고 돌아오고 누름은 `WaitButton`이 받는데, 우리는 `Message`가 직접 받으므로
그대로 두면 같은 창을 두 번 눌러야 한다. 한 번짜리 표(`FieldWorld.takeAck`)로
겹침을 없앤다.

⚠️ **배틀 글만 다르다.** 거기서는 버튼이 아니라 원작이 정한 프레임 수
(`WaitButtonABTime 30`)가 넘긴다 — 대사처럼 누르게 하면 한 턴에 예닐곱 번을
눌러야 한다. `useBattlePlayback`이 `finish()`로 쪽 넘김까지 풀고 `beat.hold`가
박자를 잡는다.
...
⚠️ **468 뒤의 번호 둘은 롬에서 온 것이 아니다.** 연결의 끈(468)과 고운 비늘(469)은
플래티넘에 없는 물건이라 이름·설명·아이콘이 롬에 없고, 우리가 붙인다
(PARITY §12가 왜 더하는지를 적는다). **`items.json`에는 안 넣는다** — 그 파일은
롬에서 굽는 것이라 손으로 두 줄을 넣으면 다음 추출에서 사라지고, 어느 줄이 롬
것인지도 흐려진다. 대신 `engine/bag/extraItems.ts`가 들고 있고 받는 자리에서
표 뒤에 잇는다 (`withExtraItems`). 이름·설명 세 로케일도 거기 있다.

⚠️ **롬 도구 열하나는 「밖에서 못 쓴다」인 채로 둔다.** 금속코트처럼 지닌 채
교환하던 것들인데, `fieldUseFunc`를 손으로 20으로 고치면 그 줄이 롬에서 온
것인지가 흐려진다. 대신 **쓰는 쪽**(`bag/fieldUse`)이 종족표에서 뽑은 목록을
받아 연다 (PARITY §12.2).
...
**470칸**을 24×20 격자로 굽고(768×640, **88.9KB**) 자료 없는 22칸은 `none`
아이콘으로 채운다. 빈칸을 남기면 잘못 그렸을 때 "원래 빈 칸"과 구분이 안 된다.

⚠️ **뒤의 두 칸은 롬 도구가 아니다.** 연결의 끈(468)·고운 비늘(469)이고, 격자가
480칸이라 자리가 이미 비어 있었다 — 그림 크기는 안 바뀐다. 아이콘 번호는
`tools/extract/extraItemsModule.cjs`에 있고 `engine/bag/extraItems.ts`와 같아야
한다 (어긋나면 `extraItems.test.ts`가 잡는다). 연결의 끈은 **어느 도구도 안 쓰는
NCGR 381번**을 쓴다 — 롬에 남은 3세대 잔재 33장 중 하나다. 남의 칸을 빌리지
...
단단함·수확량·자라는 시간·마르는 속도, 맛 다섯과 매끄러움. 도구 번호와의 관계는
뺄셈 하나다 (`itemID - FIRST_BERRY_IDX`, 체리열매가 149).

밭 쪽 세 값만 골라 쓴다: `stageDuration`(한 단계에 걸리는 **시간**)·
`moistureDrainRate`(한 시간에 마르는 물기)·`baseYield`(수확 등급 1당 열리는 개수).

⚠️ **「어느 밭에 무엇이 심겨 있는가」는 롬에 없다.** 배치표에는 밭 객체 118개와
저마다의 밭 번호(`data[0]`)만 있고, 새 게임에서 그 118곳에 무엇을 심을지는
**코드 안의 표**다 (`include/data/berry_init.h`의 `sBerryInitTable`). 그래서
디컴프에서 굽는다 — `pnpm gen:berryInit` → `engine/world/berryInit.ts`
(열매 26가지가 118곳에 흩어져 있다).

⚠️ **성장 값 세 벌을 그 모듈에 같이 굽는다.** 리포트를 처음 만드는 순간
(`newBerryPatches`) 밭마다의 남은 분이 `stageDuration`에서 나오는데, 그때는
`berries.json`이 아직 안 와 있다 — 원작은 롬에서 바로 읽지만 우리 자료는
비동기다. 디컴프의 도구 자료(`res/items/data/*_berry.json`의 `berryData`)를
쓰고, 두 벌이 어긋나지 않는 것은 `berryPatches.test.ts`가 64종 전부 대조해
지킨다 (불일치 0).

#### 2.12.2 상호교류광장의 표 둘 — 롬에 없다

연고시티 옆 상호교류광장에서 따라다니는 마리가 장식을 주워 온다. **무엇을 주워
오는지는 데려간 마리가 정한다** — 여섯 무리에 열 자리씩이고 자리가 곧 확률이다
(15·15·15·15·10·10·8·5·5·2, 합 100). 표에 없는 종은 0번 무리를 쓴다.

열매와 장식 아저씨가 주는 열여섯도 같은 자리에 있다 — **앞의 아홉이 열매고
뒤의 일곱이 장식**이라, 원작이 `< 9` 하나로 갈래를 가른다.

⚠️ **둘 다 NARC이 아니라 코드 안의 표다** (`scrcmd_amity_square.c`). 그래서
디컴프에서 굽는다 — `pnpm gen:amity` → `engine/world/amityTables.ts`.

⚠️ **이름은 롬 지명표를 따른다.** 한국어 롬의 지명 #56이 「상호교류광장」이고
그 맵(`D11R0101`)이 이 명령들을 쓰는 바로 그 맵이다. 팬 번역 이름을 쓰지 않는다.
...
**기술 연출이 그 사이에 든다.** 원작은 글을 찍은 뒤 `PlayMoveAnimation`이 도는
동안 게이지가 기다린다. 우리도 그 자리를 내주고, 기술 이름을 **띄운 채로** 연출이
돈 뒤에 게이지가 움직인다.

**그 길이는 기술마다 다르다** (`engine/battle/moveLength`). 원작 대본이 `Delay`로
쉬고 `WaitForAnimTasks`로 태스크를, `WaitForAllEmitters`로 입자가 사그라지기를
기다리므로 길이는 둘의 합이다:

    대본이 서는 시간   `MoveAnim.frames` — `Delay` + 태스크 다섯의 실제 프레임
    입자가 죽는 시간   `splLifeFrames` — `.spa` 헤더가 정하는 위끝

실측 468개: 중앙값 60 · 최대 194프레임(3.2초) · **340개가 40프레임을 넘는다.**

⚠️ **박자와 무대가 같은 자리에 물어봐야 한다.** 셋이 `moveFramesOf` 하나를 본다
(`vfx.ts`) — 어긋나면 연출이 끝나기도 전에 다음 글이 뜨거나, 다 끝나고도 화면이
멈춰 있다. 무대의 나감(`LUNGE`)만은 **둘 중 짧은 쪽**이다: 3초짜리 연출 내내 몸이
나가 있으면 안 되고, 20프레임짜리 연출에서 몸이 40프레임 동안 돌아오면 안 된다.
...
그래서 손잡이를 뒀다. `playback.ts`가 내는 프레임 수는 **원작 그대로 두고**,
화면에 접을 때만 곱한다 — `BATTLE_PACE`가 1 · 0.5 · 0.25이고 기본이 가운데다.

⚠️ **곱은 머무름·게이지·기절에만 건다. 연출에는 안 건다** — 연출 길이는 무대가
도는 시간이라 여기서 줄이면 화면과 어긋난다. 잰 값은 아래에 있다.

⚠️ **찍는 시간을 없앴으면 그만큼 머물러야 한다.** 글자를 한 자씩 안 찍으므로
(§2.11) 원작에서 「찍는 시간」이던 몫이 0이 되는데, 그것을 그냥 두면 「효과가
굉장했다!」가 **250ms**만 떠 있어 읽을 수가 없다. 그래서 찍지는 않되 **글자수 ×
4프레임**만큼 머문다 — 4는 원작 기본 문자속도다 (`Options_TextFrameDelay` 보통).

되재 본 값이다 — 모부기 ↔ 이상해씨가 서로 효과가 굉장한 기술을 하나씩, 최대 HP
22에 피해 7:

| | 원작대로 | 빠르게(기본) | 아주 빠르게 |
|---|---:|---:|---:|
| 「○○의 몸통박치기!」 | 1,900ms | 1,283ms | 983ms |
| 「효과가 굉장했다!」 | 1,100ms | 550ms | 283ms |
| 한 턴 | 6.00초 | 3.67초 | 2.53초 |

⚠️ **기술 줄만 빠르기를 덜 탄다.** 그 줄이 떠 있는 동안 **연출이 돌기 때문**이고,
연출 길이는 안 줄이기 때문이다. 다른 줄은 그대로 절반·사분의 일이 된다.
...
연출이 667ms이므로 아래쪽만 안 겹친다.

⚠️ **읽는 시간은 게이지 전환 길이에 안 더한다.** 그 값(`setHoldMs`)은 원작
프레임 수 그대로여야 한다 — 더하면 체력바가 느려진다. 더하는 곳은 박자의
길이뿐이다 (`useBattlePlayback`).
...
배치에 실제로 쓰이는 **161벌**과 나무열매 밭의 **193벌**을 아틀라스로 낸다
(354벌 · PNG 290KB · 표 63KB). 한 벌이 가로로 이어 붙인 한 줄이고 씬은 UV만 옮긴다.

⚠️ **밭 193벌은 배치표에 안 나온다.** 배치되는 것은 흙 하나뿐이고 그 위에 서는
...
사람들이 들어 있다. 문제는 **어느 사람이 누구인가**인데, 한동안 이름이 같은 것만
이었다 — 그림 이름(`BUG_CATCHER`)과 번들 안 텍스처 이름
(`tr1006_00_bugcatcher_body_col`)이 같은 낱말을 쓰는 자리다. 그러면 배치 3,555개
중 777개(21.9%)만 붙고, 「일본어 갈래 이름을 짚어야 아는 것들」이 통째로 남는다.
...
두 갈래 차례(`generated/trainer_classes.txt`와 BDSP의 `TrainerType`)는 같은 원본에서
나온 같은 순서다. BDSP가 짝으로 다니는 사람을 두 줄로 나눠 적은 자리
(쌍둥이·러브러브·더블팀·목장부부·취재진) 다섯만 한 줄로 접으면 1:1로 맞는다.
그래도 「그러니까 맞다」로 안 적는다. 줄마다 근거를 요구한다:

| 근거 | 줄 | 보기 |
|---|---:|---|
| ① 갈래 이름이 같다 | 58 | `Youngster` ↔ `YOUNGSTER` |
| ② 사람 이름이 같다 | 21 | `Gardenia` ↔ `LEADER_GARDENIA` |
| ③ ①②로 확인된 줄 **둘 사이에** 끼어 있다 | 19 | `GENTLEMAN`~`BEAUTY` 사이가 `SOCIALITE`다 |
| ④ 같은 사람의 다른 갈래 번호 | 4 | `PLAYER_MALE` = `DP_PLAYER_MALE` |

셋 다 못 대면 안 적는다 — 배틀 프론티어 다섯(메이드·홀 마돈나·팩토리헤드·아케이드
스타·캐슬 발렛)이 그렇게 떨어진다. BDSP에는 프론티어가 없으니 맞는 결과다.
누나와 남동생도 뺀다: 그 갈래로 싸우는 사람도 글자도 없는데 `ModelID`에는
`tr2018_00`이 적혀 있고, 그 번들을 열면 **자스민**이다.

⚠️ **손으로 적어 둔 백 줄과 대조했다.** 배틀 트레이너 쪽에 사람이 적어 둔 갈래→몸
표가 따로 있었는데, 견줄 수 있는 93줄 중 **91줄이 같았다.** 어긋난 둘은 손 쪽이
틀렸다 — 더블팀이 쌍둥이(진짜는 에이스 트레이너 둘)로, 벨과 아빠가 목장 아저씨
(BDSP의 첫 줄은 카우걸)로 적혀 있었다.

그림 → 갈래는 **플래티넘이 자기 배치표에 적어 둔다.** 트레이너 NPC는 `script`가
`TRAINER_*`이고 그 이름의 자료 파일에 갈래가 있다. 이름만 봐서는 못 잇는 자리를
그 표가 이어 준다 — `EXPERT_M`으로 선 여덟이 전부 베테랑이고, `GRUNT_M` 스물하나가
전부 갤럭시단이다. 한 그림에 갈래가 여럿 걸리면(`ACE_TRAINER_M` 하나로 에이스
트레이너·용사냥꾼·포켓몬레인저가 다 선다) 제일 많이 선 것을 쓰고, 과반이 아니면
안 적는다.

⚠️ **이름이 같은 갈래가 배치 통계보다 먼저다.** `RANCHER` 그림으로 선 트레이너는
「벨과 아빠」가 2:1로 더 많은데 그 갈래의 몸은 카우걸이라, 통계만 보면 목장 아저씨
자리에 여자가 선다.

⚠️ **포켓몬 그림에는 사람이 안 붙는다.** `PIKACHU`로 선 트레이너 셋이 전부
포켓몬키즈라 배치표만 보면 그 그림에 여자아이가 붙는다 — 야생 피카츄가 서야 할
자리다 (`engine/actor/overworldMon`).

#### 갈래가 없는 사람은 롬의 **두 번째 이름**으로 잇는다

트레이너가 아닌 마을 사람은 갈래가 없어서 위의 두 길이 다 막힌다. 그런데
플래티넘의 그림 파일에는 **이름이 두 개** 붙어 있다 — 그림 이름
(`OBJ_EVENT_GFX_TEALA`)과 빌드가 텍스처에 넣는 이름표
(`res/graphics/field_sprites/meson.build`의 `basename`, `pcwoman2`)다. 뒤엣것이
그 사람이 **누구인지**를 말한다:

    pcwoman1  pokecenter_nurse    pcwoman2  teala      pcwoman3  unused_receptionist
    oldwoman1 expert_f            oldwoman2 old_woman  oldman2   old_man
    shopw1    cashier_f           shopm1    cashier_m  woman6    receptionist
    assistantw scientist_f        mama      mom        baby      baby_in_pram

⚠️ **`pc`는 포켓몬센터다.** 그래서 `TEALA`는 센터 판매대의 점원이고, 신오 전체
101곳에 서 있는 **사람 그림 중 제일 많이 놓인 것**이다. 그림 이름만 봐서는
「테알라」라는 고유명으로 읽혀 영영 안 이어진다.

⚠️ **`EXPERT_F`도 트레이너가 아니다.** 이름표가 `oldwoman1`이고, 그 짝인
`oldwoman2`가 `OLD_WOMAN`이다 — BDSP의 `grandmother01`·`grandmother02`와
**번호까지 맞는다.**
...
**다섯째 길 — 다시 칠하기.** 핸섬(`icpo`)과 플루토(`gkanbu4`)는 플래티넘에만
나오는 사람이라 **BDSP에 몸이 아예 없다**(필드 161벌·배틀 124벌을 다 셌다).
그 둘은 남의 몸의 **레이어 색을 갈아 끼워** 세운다 — BDSP 인물은
`albedo = _MainTex(음영) × 레이어색[_MaskTex 채널]`이라 색만 바꾸면 음영이
살아 있는 채로 부위 색이 바뀐다. 넣는 값은 **롬 그림에서 잰 목표 ÷ 그 채널의
음영 평균**을 푼 것이고, 표는 `NPC_RECOLOR`다 (3D_GAP_AUDIT §3.1에 실측표).

이렇게 이어서 배치 3,555개 중 **1,895개(53.3%)**가 모델로 선다. 남은 1,660개에
**사람은 하나도 없다** — 물건이 1,118개(`ROCK_SMASH` 591 · `POKEBALL` 331 ·
환풍구 65 · 괴력바위 50 · 베어야 할 나무 49), 포켓몬이 115개(실물 3D로 따로
선다), 자리표시자가 427개(`OBJ_EVENT_GFX_VAR_*` — 대개 주인공 둘이라 이미
모델이다)다. **사람 그림만 보면 1,895 / 1,895 = 100%다.**

⚠️ **파일 이름이 번들 이름이다.** 한동안 번들 안 텍스처 이름표(`eliteM`)로 지었는데,
그 낱말은 **번들 하나를 안 가리킨다** — `tr1024_00`(에이스 트레이너)과
`tr1053_00`(눈 지방 에이스 트레이너)이 둘 다 `eliteM`이라 먼저 구운 것이 나중 것을
덮었다. 눈 지방 사람이 평지 사람으로 서 있었다.

⚠️ **배틀에 서는 몸도 같이 굽는다.** 오버월드 배치만 보고 구우면 「PI」처럼 그림을
다른 갈래와 나눠 쓰는 사람이 배틀에서 몸 없이 선다. 근거표에 있는 갈래는 전부 굽고,
안 쓰는 glb는 굽는 쪽이 치운다 — 이름 규칙을 한 번 바꿨을 때 87벌이 그냥 남아
에셋 목차에 들어갔다.

⚠️ **번들 하나에 걸지 않는다.** 어떤 배틀 번들은 같은 폴더에 없는 CAB을 가리켜
열다가 끊긴다 — 웨이트리스(`tr1026_00`) · 눈송이(`tr1078_00`) · 아이돌
(`tr1085_00`) 셋이 그렇다. 그때 쓸 다음 후보가 **같은 번호의 필드 번들**이다
(`tr1085_00` ↔ `fc1085_00`): 앞 두 글자만 다르고 번호가 같으면 같은 사람이다.
그 셋은 치비로 선다. 드래곤조련사(`tr1029_00`·`fc1029_00`)는 둘 다 끊겨서
절차형 몸으로 대신한다.

⚠️ **치비로 서는 사람은 머리를 줄이고 몸을 늘여서 세운다.** BDSP는 필드에
세울 사람을 **머리 큰 치비**로 따로 만들어 두었다 — 엄마(`fc2005_00`)는 목이
키의 44%라 머리가 키의 56%다(주인공 `dawn`은 25%). 우리는 주인공과 트레이너를
배틀용 등신 모델로 세우므로 나란히 서면 계통이 어긋난다(실제로 「엄마 얼굴이
이상하다」는 보고를 받았다). 배틀용이 아예 없는 사람이 굽은 116벌 중 스물일곱이고
대부분 이야기 인물이라, 판때기로 되돌리면 오프닝 바로 다음 장면의 엄마가
종잇장이 된다.

그래서 `engine/model/chibi`가 `fc`로 시작하는 번들만 이렇게 세운다. 굽힌 116벌 중
치비는 **스물일곱**이고 전부 `Head`·`Neck`·`LHand`·`RHand` 뼈를 갖고 있어 하나도
안 빠진다:

1. **`Head` 뼈를 0.24로 줄인다** (`CHIBI_HEAD`). 머리는 목 관절을 원점으로
   줄어들어 이음매가 안 벌어진다.
2. **머리를 줄인 뒤에 키를 재고, 거기에 2.09를 곱한 키로 세운다**
   (`CHIBI_GROW`). ⚠️ **줄이기 전 상자로 재면 머리카락이 키를 정한다** — 그래서
   쪽찐 할머니(`fc2016_00`)가 1.525로 제일 컸고 어른들은 1.36~1.44로 주인공
   (1.564)보다도 작았다.
3. **그 늘림을 굵기에서 도로 뺀다** (`CHIBI_SLIM`). 균등 배율이면 굵기도 같이
   커져 「몸이 굵다」는 보고가 온다.
4. **다리만 단면을 0.7로 더 조인다** (`CHIBI_LEG`). ⚠️ **③을 더 조여서 맞추면
   안 된다** — 굵은 것은 다리뿐인데 ③은 몸 전체에 걸린다. 1.4로 올려 봤더니
   허벅지는 맞았지만 목 둘레가 키의 5.2%(등신 5.8~7.0%)·어깨 너비가 3.5%
   (등신 5.0%)로 좁아져 **머리가 가느다란 목 위에 얹힌 꼴**이 됐다.
5. **머리는 눌린 만큼 도로 편다.** 머리뼈의 로컬 축은 X가 위, Y가 좌우, Z가
   앞이라(실측) 가로 보정이 `scale.y`·`scale.z`로 간다.
6. **`LHand`·`RHand` 뼈를 0.5로 줄인다** (`CHIBI_HAND`). 손가락은 자식이라
   따라온다. ⚠️ **손뼈의 자식이 손가락만은 아니다** — 웨이트리스(`fc1026_00`)는
   `Tray`와 그 위의 `BallA`·`BallB`를, 아이돌(`fc1085_00`)은 `Mike`를 손에 달고
   있어서 그대로 두면 **쟁반과 마이크도 반이 된다.** 손가락(`[LR]Finger*`)이
   아닌 자식은 `1/0.5`로 되돌린다. 나머지 열다섯은 빈 부착점(`LItem1`·`RItem1`,
   `fc2040_00`만 `LItem`·`RItem`)이라 되돌려도 보이는 변화가 없다.

⚠️ **치비는 필드에만 선다.** 배틀·오프닝·전당은 `tr`·`pc` 번들만 세운다 —
배틀 번들이 끊긴 셋(`tr1026`·`tr1078`·`tr1085`)은 구운 것이 아예 없어서
`trainerModelBundle`이 그 자리에 아무것도 안 준다(절차형 몸이 대신 선다).
...
배율이 통째로 달라진다. 바인드 포즈로 재면 가로로 뻗은 것이 전부 짧고 굵게
보여서, 실제로 그 값을 보고 팔을 2.1배 늘였다가 되돌린 적이 있다.

| | 키 | 머리 높이 | 목 둘레 | 어깨 너비 | 어깨 요크 | 허벅지 폭 | 발 길이 | 팔 길이 |
|---|---:|---:|---:|---:|---:|---:|---:|---:|
| 광휘 (등신) | 1.564 | 21.4% | 7.0% | 12.8% | 5.7% | 11.3% | 18.1% | 24.7% |
| 리오 (등신) | 1.749 | 16.0% | 5.8% | 17.0% | 7.4% | 8.2% | 14.0% | 26.1% |
| 신사 (등신) | 1.788 | 20.7% | 9.9% | 16.6% | 7.2% | 9.3% | 16.2% | — |
| 핸섬 (치비) | 1.745 | 15.2% | 6.4% | 14.7% | 6.5% | 9.6% | 15.9% | 23.0% |
| 엄마 (치비) | 1.785 | 22.5% | 5.6% | 13.9% | 6.3% | 7.5% | 15.0% | 22.4% |
| 마박사 (치비) | 1.781 | 21.3% | 6.6% | 14.1% | 6.4% | 9.0% | 14.0% | 22.5% |

여덟 자리 모두 등신 셋이 만드는 폭 안에 들거나 그 언저리다. **어깨 너비는 두
어깨 관절(`LArm`·`RArm`) 사이**로 잰다 — 가슴뼈에 매달린 정점 상자로는 못
잰다(등신 쪽은 `Spine2`에 0.5 넘게 매달린 정점이 거의 없다). **어깨 요크**는
어깨뼈에서 위팔 관절까지고, 팔 길이를 어깨뼈에서 재면 이것이 섞여 든다. **팔 길이는 위팔
관절에서 손까지**로 잰다 — 어깨뼈에서 재면 어깨 너비가 섞여 든다.

⚠️ **팔은 안 건드린다.** 선 자세에서 팔은 다리와 같은 대접을 받는다 — 길이가
키 늘림, 단면이 굵기다. 그것이 맞는 대접이라 손댈 것이 없다.

⚠️ **BDSP 필드 치비는 몸을 한 벌만 만들어 돌려 쓴다.** 목 높이가
0.6383·0.6387·0.6409로 소수점 셋째 자리까지 같고 다리도 0.3765로 같다. 그래서
머리를 줄이고 난 키가 0.835~0.874로 한 무리고, 거기에 상수 하나(2.09)를 곱하면
스물일곱 벌이 다 어른 키로 선다. **아기(`fc2025_00`)만 몸이 따로다**(목 0.441 ·
다리 0.236) — 상수를 곱하는 것이라 어른의 0.75배로 남는다.

⚠️ **아이는 어른과 같은 키로 선다.** 아이 치비(`fc2017_00`)의 목 높이·다리
길이가 어른과 **같은 값**이고 머리만 크다 — BDSP가 아이 몸을 따로 안 만들었다.
원본에 없는 정보라 여기서는 못 만든다. 낮추려면 그림마다 키를 적은 표가 따로
있어야 한다.

⚠️ **날렵하게 만들면 손이 커진다.** 세로 늘림(×1.745)이 **아래로 뻗은 것을 다
늘이는데**, 팔을 내리고 서면 손은 길이축이 아래를 향한다. 게다가 치비 손은
원래 벙어리장갑이라 손목보다 넓다 — 둘이 겹쳐 「손이 진짜 크다」는 보고를 받았다.
스킨을 먹인 정점 1,580개를 훑어 화면에 선 왼손 상자를 재고, 같은 방법으로 잰
주인공(`pc0001_00`)의 왼손 `0.116 × 0.144 × 0.113`과 견줬다(팔을 내린 자세라
**세로가 손 길이**다):

| 손뼈 | 가로 × 세로 × 앞뒤 | 세로/주인공 | 가로/주인공 | 앞뒤/주인공 |
|---|---|---|---|---|
...
세 축을 곱해 세제곱근을 내면 ×0.557이 주인공 손과 같은 크기지만, 눈에 걸리는
것은 길이라 ×0.5로 둔다. **한 손씩 재야 한다** — 두 손을 한 상자에 담으면 가로가
손 폭이 아니라 두 손 사이 거리가 되어 주인공 손이 2.76타일로 나온다.

**팔다리 길이는 그대로다** — 가로·세로를 눌러서 얻은 날렵함이라 가로로 뻗은
부위는 그만큼 짧아지고, 걸을 때 앞뒤 보폭이 좁아 보인다. 뼈를 늘려 제대로
등신으로 만드는 것은 리타깃이라 안 한다. 같은 까닭으로 **발도 세로로 1.7배
늘어나 있다**(엄마 `0.201 · 0.195 · 0.309`, 주인공 `0.142 · 0.113 · 0.272`) —
손처럼 줄이지는 않았다.

갈래 이름표로 찾는 사람(간호사·메이드·게임 디렉터)만 옷 번들 여럿을 만난다.
그때 이름순 첫 번째를 쓰는데, `pc0001_12`가 `["hero","heroine"]`이라
`heroine`의 첫 번째로 뽑힌 적이 있다 — 남주 옷 번들에 여주 텍스처가 섞여 든
것이라 **구우면 터진다.** 갈래를 하나만 든 번들이 그 사람 본체다.

⚠️ **굽는 쪽이 바뀌면 다시 굽는다.** 번들 날짜만 비교하면 `bdspGlb.py`를 고쳐도
구운 것이 전부 "그대로 둔 것"이 되어 낡은 glb가 남는다. `npcModels.mjs`가 굽는
스크립트의 날짜도 함께 본다.
...
⚠️ **그냥 구우면 못 싣는다.** 주인공 등신(`pc0002_00`)을 세 가지로 구워 재면:

| | 크기 | 클립 |
|---|---:|---:|
| 그대로 | 9.77MiB | 23 |
| 텍스처 긴 변 256 | 6.53MiB | 23 |
| + 애니메이션 클립 없이 | **1.48MiB** | 0 |

**클립이 5.05MiB로 77%다.** 그래서 **다 싣지 않고 쓸 것만 고른다**
(`clipFilterFor`): 등신은 배틀에서 이어 붙는 넷, 주인공은 거기에 걷기·뛰기,
치비는 제 걷기·뛰기·서기 셋이다. 트레이너 걷기는 굽지 않고 **화면에서 주인공
것을 그 몸으로 옮긴다** (PLAN §16.5) — 몸마다 구우면 33MB가 는다.

클립이 없는 몸은 절차형(`actor/locomotion`)이 맡고, 그때도 **서 있는 사람을
한 번은 돌려야** 한다. 안 돌리면 바인드 포즈, 즉 팔을 벌린 T 자세로 선다.
...
#### 변장한 트레이너가 쓰는 더미 넷 — `fldeff.narc`

닌자보이 열과 검은띠 하나는 이동 유형이 51~54(`DISGUISE_SNOW`·`SAND`·`ROCK`·
`GRASS`)다. 원작은 그 사람을 **32단위 땅 아래로 내리고** 그 자리에 더미 모델을
그린다 (`sub_0206A0BC`가 `SetSpriteJumpOffset {0, −32, 0}` + `HIDE_SHADOW`,
`ov5_021F3D90`이 모델). 모델 번호는 `Unk_ov5_02200678`에 넷이 그 차례로 적혀
있다 — `fldeff.narc` 103~106.

**모델을 안 굽는다.** 넷을 열어 재 보니 전부 같은 모양이었다:

| 멤버 | 텍스처 | 꼭짓점 | 삼각형 | 높이 | 면 |
|---|---|---|---|---|---|
| 103 눈 | `nin_hide_s` 16×16 | 4 | 2 | 0.1875 | 양면 |
| 104 모래 | `nin_hide_b` 16×16 | 4 | 2 | 0.1875 | 양면 |
| 105 바위 | `nin_hide_g` 16×16 | 4 | 2 | 0.1875 | 양면 |
| 106 풀 | `nin_hide_k` 16×16 | 4 | 2 | 0.1875 | 양면 |

한 칸 크기의 **바닥에 눕는 평면 하나**다. 껍데기가 고정이라 남는 것은 16×16
그림 넷뿐이고, 가로로 이어 붙인 한 줄(`data/npc/disguise.png`, 64×16)로 낸다.
그리는 쪽은 `scene/DisguisePlates`다.

#### 3D 오브젝트 열 종은 아직 아무것도 안 선다

배치된 NPC가 쓰는 그림 172종 중 **열 종**이 화면에 아무것도 안 세운다 (배치
247건): 간판 여섯(91~96 · 212건) · 눈덩이(118 · 19건) · 책(183 · 7건) ·
사천왕 방문(209 · 8건) · 로토무 방 벽(262 · 1건).

**2D 그림표에 없는 것이 맞다.** 원작의 렌더러 표(`ov5_021FAF40.c`)가 이들을
사람 렌더러(`Unk_ov5_021FAFD8`)가 아니라 `Unk_ov5_021FB000`(간판·책·문·벽) ·
`Unk_ov5_021FB050`(눈덩이)으로 보낸다 — 원작에서 3D 오브젝트다.

⚠️ **BDSP 모델로 짝을 못 이었다.** `Characters/objects`에 번들 128개가 있는데
**이름이 번호뿐이다**(`ob0201_00`). 번들 안을 열어 봐도 자산 이름이 그 번호를
되풀이하거나(`ob2002_00_box` · `ob1003_00_bicycle`) 재질 이름이 나올 뿐이라
(`openball02`), 어느 것이 눈덩이이고 어느 것이 간판인지 **이름으로는 안 갈린다.**
AssetAssistant 트리 전체에서 `ob0201`을 찾아도 그 번들과 `Characters.bin` 말고는
나오는 데가 없다 — 이름표가 아예 없다.

128개를 다 굽고 눈으로 고르는 길이 남지만 그것은 「그럴듯한 짝」이라 안 한다
(`npcModels.json` 머리말의 규칙). 붙일 자리는 그 자료 파일과
`tools/extract/npcModels.mjs`다.

⚠️ **그림이 없어도 길은 막는다** — 통행은 `actor/obstacles`의 `solidNpcAt`이
따로 맡는다 (PARITY §1.28).

#### 변수로 정해지는 그림은 이미 풀린다

`OBJ_EVENT_GFX_VAR_0`~`VAR_F`(101~116, 배치 62건)는 자리표시자고 실제로 무엇이
설지는 맵 초기화 스크립트가 변수에 넣는다. 사람을 세울 때 `resolveGfx`가 그
변수를 읽어 실제 번호로 바꾸므로(`actor/npcs`) 그리는 쪽은 이 번호를 볼 일이
없다 — 201번도로에서 「내가 아닌 쪽 주인공」이 서는 자리가 그것이다.
...
0 타일 · 1 팔레트 · 2 셀 · 3 애니 · 4 통짜 그림. 105갈래 × 5 = 525칸이 맞는다.

⚠️ **원작이 쓰는 것은 0번인데 우리가 쓰는 것은 4번이다.** 0번은 OBJ 타일이라
셀(2번)을 읽어야 사람 모양으로 서고, 4번은 같은 그림이 한 장으로 이미 펴져
있다(160×80). NCER을 해석하지 않아도 되는 쪽을 골랐다.

⚠️ **둘의 규칙이 정반대다.** 0번은 암호가 없고(배경 82.5%) 타일 배치인데,
4번은 암호가 걸려 있고(풀기 전 6.2% → 풀면 91.3%) **선형**이다. 4번을 타일로
읽으면 줄무늬가 되고 0번을 선형으로 읽으면 흩어진다. 암호는 `pl_pokegra`와
같은 16비트 LCG라 그쪽 함수를 그대로 쓴다 (§2.17).

한 줄이 160픽셀이고 80×80 두 컷이 나란히 있다 — 쓰는 것은 첫 컷이다.

⚠️ **성 안내인(102번)만 팔레트 셋째 묶음을 쓴다**
(`if (trainerClass == TRAINER_CLASS_CASTLE_VALET) paletteIdx = 2`). 팔레트가
256색 한 판인데 그림이 4비트라 16색씩 열여섯 묶음으로 나눠 쓰고, 그 한 사람만
0번이 아니다.

#### 2.17.3 pl_otherpoke — 폼 그림과 **알 그림**

`pl_pokegra`는 494종뿐이라 폼도 알도 칸이 없다. 그 둘이 옆 아카이브에 있다
(PARITY §3.4 · §8.10). 픽셀 암호도 팔레트 모양도 같다 — 같은 도구가 구웠다.

파일 253개가 어떻게 떨어지는지가 배치의 증명이다. 원작은 종마다 산술로 자리를
내는데(`BuildPokemonSpriteTemplate`의 `72 + (face / 2) + form * 2` 같은 식 열셋),
그 식의 **밑수를 짐작하지 않고 센다** — 등록 차례
(`res/pokemon/form_registry.json`)대로 폼 수만큼 쌓으면 된다:

| | 칸 | 무엇 |
|---|---|---|
| 0~153 | 154 | 그림. 테오키스 8 · 안농 56 · 캐스퐁 8 · 도롱충이 6 · 도롱마담 6 · 조개무지 4 · 트리토돈 4 · 체리버 4 · 아르세우스 36 · **알 2** · 쉐이미 4 · 로토무 12 · 기라티나 4 |
| 154~247 | 94 | 팔레트. 폼마다 보통·색이 다른 두 벌씩, 테오키스·안농은 폼끼리 나눠 쓴다 |
| 248~252 | 5 | 공용 — 대타출동 앞·뒤·팔레트, 그림자·팔레트 |

⚠️ **기라티나만 그림이 맨 뒤다** (`__last_sprite`). 등록 차례대로 쌓으면 쉐이미와
로토무가 네 칸씩 밀려서 로토무 팬 자리에 기라티나가 뜬다.

⚠️ 배치 변주 셋도 등록부가 적어 둔다 — `__all_back_then_front`(뒷모습을 다 놓고
앞모습을 다 놓는다: 캐스퐁·조개무지·트리토돈·체리버), `__all_normal_then_shiny`,
`__front_only`(알). 표는 `pnpm gen:otherpokeTable`이 굽고, 마지막 합이 248이
안 되면 굽기가 선다.

→ `data/pokemon/{종}-{폼}_{front,back}.png` 130장. 기본형은 안 싣는다 —
`pl_pokegra`에 같은 그림이 이미 있다. 알만 예외다 (`494_front.png`).
...
#### ⚠️ 색 그림이 없는 재질 — 흰 덩어리가 붙던 자리

BDSP의 불·연기·오라 재질에는 **색 그림이 없다.** 한동안 그런 재질을 통째로
건너뛰었는데, 그러면 그 조각에 재질이 안 붙고 **three가 기본 흰색으로 그린다.**
실측: 54종 233조각이 그랬다 — 리자몽 꼬리불, 또가스·마자용 연기, 파이어·매그마
불길, 로토무 오라, 초염몽 불꽃이 전부 흰 덩어리였다.

색은 재질이 적어 두었다. 그림이 없을 뿐이다:
...
빼고, 조각이 하나도 안 남은 껍데기는 껍데기째 뺀다 (glTF가 빈 메시를 안 받는다).

##### ⚠️ 깎개가 **깊이로** 깎으면 그 알맹이는 못 굽는다

또도가스(`pm0110`)가 **몸을 통째로 가리는 크림색 구**로 섰다. 굽는 쪽 둘이 갈린
것이 아니라 둘 다 그렇게 그렸다 — 그 연기의 모양을 내는 것은 색 그림이 아니라
...
`MASK_CALC_MODE_*`가 붙은 깎개는 **색을 섞는** 도구고, 그것 없이 깊이만 켠 깎개가
**깎아 내는** 도구다. 후자를 짝으로 둔 알맹이(`*Mask` → `*Core`)는 우리 손에서
안 깎인다.
...
실제로 걸리는 것은 두 종의 껍데기 여섯 장이다 (`carvedShells` ·
`carved_shells`).

#### ⚠️ 색 그림이 다른 판의 번들에 있는 종

캥카(`pm0115_00_00`)의 몸 재질은 `_Col0Tex`로 `pm0115_51_00_BodyA_col`을
가리키는데, 그 그림은 `common/pm0115_51_00`에 있다. 번들 셋만 열면 그 재질이
안 구워지고 **몸이 통째로 하얘진다.** 안농도 같다 — 판 12~38이 판 11의 그림
(`common/pm0201_11_00`)을 나눠 써서, 스물여섯 판이 **그림 0장**으로 구워져
있었다. 글자가 안 보이는 안농은 안농이 아니다.

그래서 그 종의 **세 토막짜리 텍스처 번들을 전부** 같이 연다. 세 토막
(`pm0115_51_00`)은 실측으로 `Texture2D`만 들어서 다른 판의 몸이 섞일 일이 없다.
두 토막(`pm0115_00`)은 메시가 있어서 안 연다.

⚠️ **인물 번들에 포켓몬이 들어 있는 자리가 있다.** 대호평 아가씨(`tr1021_00`)가
우리(`pm0438`)를 안고 서 있는데, 인물은 `_MainTex`에 포켓몬은 `_Col0Tex`에 색을
싣는다 — 앞의 것만 보면 그 열두 조각이 하얀 덩어리가 된다. 둘 다 본다.

⚠️ **`_CullMode`는 Back이 2,797이고 Off가 26, 없는 것이 46이다.** Off인 26개가
전부 `FireMask*`라 지금도 아무 데도 안 걸린다 — 그것들은 위에서 빠진다.
...
### 2.28 graphic/poketch.narc — 포켓치 지도와 액정 팔레트

`graphic/poketch.narc` **126칸** 중 넷 → `data/poketchMap.png` 16.2KB +
`data/poketchMap.json` (`tools/extract/poketchMap.js` · PARITY §7.3):

| 칸 | 무엇 | 크기 |
|---:|---|---|
| 115 | `marking_map.NSCR` | 32×24칸 |
| 116 | `berry_searcher.NSCR` | 같은 지도 + 아래에 열매 표시 3×2칸 |
| 117 | `map_bg_tiles.NCGR` | 27×4타일 = 108장, 4bpp |
| 0 | `generic_bg_tiles.NCLR` | 팔레트 열여섯 벌 |

⚠️ **액정은 네 단계다.** 팔레트 열여섯 칸 중 실제로 쓰는 것은 **4·15·8·1**
넷뿐이고 나머지는 흰색으로 채워져 있다. 밝은 쪽부터 4 → 15 → 8 → 1이며,
**바탕이 제일 밝은 4번**이다 — 실측으로 지도 두 장 98,304픽셀 중 4번이
84,476개(86%)다. 색 0(초록)의 넷은 `#73b573 #528452 #395231 #102918`.

⚠️ **우리 색이 손으로 고른 두 단계였고 명암이 뒤집혀 있었다.** 어두운 바탕에
밝은 글씨로 그리고 있었는데 원작 액정은 그 반대다. 롬 팔레트를 재고 나서야
보였다 — 지금은 설치본의 값이 이기고, 자료가 없을 때만 우리 색으로 떨어진다
(`poketchShades` · `fallbackShades`).

⚠️ **한 색이 팔레트 두 벌을 쓴다** (`SLOTS_PER_POKETCH_THEME`이 u16 서른둘) —
앞이 보통, 뒤가 백라이트다. 우리는 백라이트를 안 켜므로 앞만 쓴다.

⚠️ **두 지도는 여섯 칸만 다르다.** 나무열매탐색기 쪽에 (120,160)부터 3×2타일짜리
열매 표시가 더 붙어 있고 나머지는 같다. 아틀라스는 가로로 지도 둘, 세로로 색
여덟이다 (512×1536).

**지도 위 자리는 코드 안의 표라 따로 굽는다** (`pnpm gen:poketchmap` →
`engine/world/poketchMapTable.ts`): 격자 30×33이 한 칸 6픽셀이고, 밭 118곳의
격자 자리(자리 서른여섯으로 뭉친다)와 숨은 자리 넷의 픽셀 좌표가 들어 있다.
⚠️ **격자 한 칸이 맵 한 장이다** — 원작이 주인공 타일 좌표를 32로 나눈다.
⚠️ **세로 앞 다섯 칸이 0이다** — 지도 위쪽 밖이라 안 쓴다. 0을 좌표로 믿으면
신오 북쪽 맵이 전부 화면 맨 위에 겹친다.

### 2.30 기술가르침 — 값이 돈이 아니라 조각 넷이다

교사 셋(선단시티 동쪽 집 · 212번도로 집 · 서바이벌에리어 북쪽 집)이 쓰는 표는
**NARC이 아니라 빌드 때 헤더로 굽히는 자료**다:

| 무엇 | 어디 |
|---|---|
| 기술 · 빨강/파랑/노랑/초록 조각 값 · 장소 | `res/pokemon/move_tutors.json` (38줄) |
| 종마다 배울 수 있는 기술 | `res/pokemon/<종>/data.json`의 `learnset.by_tutor` |
| 폼이 따로 갖는 것 | `res/pokemon/<종>/forms/<폼>/data.json` (열둘) |
| 값 판정 · 지불 규칙 | `src/overlay005/scrcmd_move_tutor.c` |
| 장소 상수 | `generated/tutor_locations.txt` (셋) |

롬 안에서는 그 둘이 `sTeachableMoves`(값 표)와 `sSpeciesLearnsetsByTutor`
(**종마다 5바이트 비트마스크**)로 굳는다. 비트 번호가 곧 값 표의 줄 번호라
**줄을 옮기면 온 신오가 다른 기술을 배운다.**

⚠️ **폼을 종으로 뭉치면 안 된다.** `Pokemon_ReadMovesetMaskByte`가 종과 폼을
같이 보고 표의 다른 줄로 간다 — 데오키시스 셋 · 도롱마담 둘 · 기라티나 오리진 ·
쉐이미 스카이 · 로토무 다섯. 실측으로 **데오키시스 넷과 도롱마담 셋, 쉐이미
둘이 서로 다르고** 기라티나·로토무는 폼과 종이 같다.

장소별 기술 수는 **212번도로 13 · 서바이벌에리어 17 · 선단시티 8**이고 서로
안 겹친다.

우리 쪽은 `pnpm gen:tutorMoves`가 `src/engine/world/tutorMoves.ts`로 굽는다
(`tools/extract/tutorMoveModule.cjs`). 담기는 것은 번호와 수뿐이다 — 기술 이름은
한 바이트도 안 담고, 화면에는 기술 이름표에서 읽어 넣는다.

### 2.29 낱말 고르기 — 낱말 1,495개

**글자는 뱅크 열하나**(`pnpm extract:dialogue`)**, 어느 낱말이 어느 무리에
드는가는 코드 안의 표**다 (`pnpm gen:easychat` →
`engine/world/easyChatTable.ts` · PARITY §4.8).

낱말 번호는 **뱅크 열하나를 이어 붙인 통번호**다
(`EasyChatWord_FromBankAndEntry`):

    종족명 496 → 기술명 468 → 타입 18 → 특성 124 → 트레이너 38 → 사람 38
    → 인사 107 → 생활 104 → 기분 47 → 어려운말 32 → 유니언 23   = 1,495

그래서 **종족 낱말은 종족 번호와 같고 기술 낱말은 496 + 기술 번호**다.

| 무리 | 낱말 | 여는 조건 |
|---|---:|---|
| 포켓몬 · 포켓몬2 | 235 + 258 | 도감에서 **본 것만** |
| 기술 · 기술2 | 223 + 241 | **명예의 전당에 오른 뒤** 통째로 |
| 스테이터스 | 138 | 늘 |
| 트레이너 · 사람 | 38 + 33 | 늘 |
| 인사 | 102 | 다른 나라 말만 풀어야 뜬다 |
| 생활 · 기분 | 104 + 47 | 늘 |
| 어려운 단어 | 32 | 하나씩 (32비트) |
| 유니언 | 23 | 늘 |

⚠️ **포켓몬 무리 둘의 합이 정확히 493이다** — 이 파싱이 맞다는 증거다. 무리
표의 끝이 `WORD_NONE`이고 원작이 길이를 `NELEMS(…) - 1`로 넘기는데, 그 칸을
못 알아보고 뒤에서 하나 잘라 내면 **진짜 낱말이 하나 사라진다** (유니언 낱말이
스물셋인데 스물둘이 나왔던 자리다).

⚠️ **원작은 낱말 넷을 대문자 뱅크에서 읽는다** (`move_names_uppercase` 따위) —
**한국어·일본어 롬에는 그 뱅크가 아예 없다.** 대문자가 없는 글자라 롬이 표를
안 들고 있다. 그래서 세 로케일 다 보통 이름표를 쓴다(종족·기술은
`names/*.json`, 타입·특성은 `labels.*.json`) — 로케일마다 다른 낱말 목록이
생기는 편이 훨씬 나쁘다.

⚠️ **가나다 차례표는 안 굽는다.** 원작의 것은 알파벳 스물여섯 칸이고
(`sWordsByAlphabet`) 한국어 롬은 그 자리가 「가나다 모드」다 — 영어 첫 글자로
묶은 표를 한국어 화면에 올릴 수가 없다. 화면이 **그 로케일 이름으로** 정렬한다.
...
| 사람·포켓몬 도트 | `pokegra`, `npcSprites`, `trainerSprites` | PNG·index |
...
#### 번들 하나만 열면 사람이 캡슐로 선다

유니티 `PPtr`은 `m_FileID`가 0이면 제 파일이고 **1부터는 그 파일의 바깥
참조표**(`FileIdentifier`: guid 16B · type · path)를 가리킨다. 번들 하나만 열면
그 참조가 안 풀리고, 재질을 못 찾은 껍데기는 버려진다
(`import/bdsp/model.ts`) — 등신은 껍데기가 `baseSkin` 하나뿐이라 그대로
「세울 것이 없다」가 되고 배틀에 캡슐 사람이 선다.

**어느 번들이 무엇을 가리키는지는 매니페스트가 안다.** `Characters/Characters`
(15KB)가 `AssetBundleManifest`이고 번들 441벌의 `m_Dependencies`를 든다. 그것을
읽어 TS 모듈 하나로 굽고(`pnpm gen:bundleDeps` → `import/bdsp/bundleDeps.ts`)
**노드 추출기와 브라우저 변환기가 같은 표를 본다.**

실측 — 번들 441 중 **58이 딴 번들을 가리킨다**:

| 어디 | 몇 벌 | 무엇을 가리키나 |
|---|---:|---|
| `persons/field` | 29 | `pc_parts` · 같은 번호의 다른 옷 번들 |
| `persons/battle` | 9 | 주인공 옷 넷 · `objects/ob0204_00` 다섯 |
| `objects` | 9 | |
| 그 밖 (`scenes`·`debug`…) | 11 | |

⚠️ **셰이더 번들은 안 담는다** — 재질은 우리가 합성하므로 열 이유가 없다.

⚠️ **사슬이 있다.** `pc0002_12` → `pc0001_12` → `pc0001_00`이라 한 단계만
따라가면 가운데가 빈다 (`bundleDeps()`가 너비 우선으로 다 따라간다).

⚠️ **빈 참조가 있는 번들도 있다.** `tr1078_00`은 매니페스트에 의존이 없는데
`m_Bones`에 `m_PathID == 0`인 칸을 들고 있어 읽다 죽는다. 그 칸은 **뿌리로
...
**소품 쪽**에 있다 — `Characters/objects/ob1004_00`이고, 번들 안 텍스처 이름이
`ob1004_00_bicycle_col`이라 이름이 스스로 무엇인지 말한다.

⚠️ **한동안 다른 자전거를 굽고 있었다.** 소품 쪽에 자전거가 셋인데
(`ob1003`·`ob1004`·`ob1012`) 오래 `ob1003_00`을 세웠다. 원작이 주인공과 **한
리그**로 묶어 든 것은 `ob1004_00`이다 — 주인공 옷 번들 `fc0001_11`이
`ob1004_00_bicycleSkin`을 제 뼈대에 담고, 클립 `bike_wait/walk/run_f`가 사람과
자전거를 함께 돌린다. 실측으로 둘은 다른 물건이다:
...
| 앞바퀴 z | 0.4458 | 0.5738 |
| 크랭크 반지름 | 0.17 | 0.084 |
| 뼈 | 54 (안장·손잡이 셋·경적) | 27 (**안장 뼈가 없다**) |

⚠️ **자리를 눈으로 맞출 것이 없다.** 붙이는 자리를 가리키는 `loc_attach`가
(0, 0, 0)이고 메시 바닥도 0.0013이라, **주인공 발밑에 그대로 놓으면 바퀴가
땅에 닿는다.** 앞도 +Z라(앞바퀴 z +0.574 · 뒷바퀴 −0.293) 돌릴 일이 없다.

**앉는 자세는 지어내지 않는다.** 자리를 번들과 원작 클립에서 뽑는다 — 골반은
(0, 0.6498, 0.0212), 발은 크랭크(0, 0.210, 0.070)를 반지름 0.084로 도는
페달에, 손은 손잡이 끝(±0.3481, 0.6588, 0.2653)에. 각도는 **역기구학**이
...
⚠️ **손잡이가 뒤로 휜다.** 끝 뼈가 없어서 `Handle` 뼈에 매달린 정점 중 제일
바깥을 잰다 — x 0.3481 언저리 18점이 y 0.6345~0.6863 · z 0.2542~0.2797에
모여 있다. 뼈 `Handle` 자체는 z 0.393이라 그것을 쓰면 14cm 앞이다.

**상체가 숙는 각도 단마다 원작 값이다** (골반→목 벡터, 여덟 점 평균):
...
단은 걷기의 몇 배인가로 갈린다(`BIKE_GEARS` 2 · 8/3 · 4). 오래 0.30 하나로
돌고 있었는데, 그 값으로는 `ob1004`의 낮고 먼 손잡이에 어깨가 안 나간다.

⚠️ **손이 손잡이 끝에 5.2cm 모자란다 — 그것이 지금 맞는 값이다.** 우리 등신은
골반에서 어깨까지가 0.34인데 원작 치비는 0.076이라, 같은 각으로 숙여도 어깨가
훨씬 높은 데서 출발한다 — 팔(0.396)로는 그 거리(0.448)가 안 닿는다.
**원작도 안 닿는다**: 치비의 손이 손잡이 끝보다 14cm 안쪽 허공에 있다. 우리가
9cm 더 가까이 잡는 셈이다. 발은 페달에서 한 바퀴 내내 **5cm 안**이다(시험).

⚠️ **팔은 척추를 숙인 뒤에 풀어야 한다.** 어깨가 척추의 자식이라 상체가 숙는
만큼 함께 돌아간다 — 바인드 자리로 풀면 손이 딱 그만큼 어긋난다.
...
바퀴와 크랭크는 **구른 만큼** 돈다(위상 = 거리 ÷ 바퀴 반지름 0.2661). 기어비는
원작에 없어서 1:1이다. ⚠️ **`ob1004`는 페달이 `Gear`의 자식이라** 크랭크를
돌리면 발판도 같이 돈다 — 반대로 같은 각을 돌려 수평을 지킨다.

⚠️ **클립은 자전거 없는 몸(`fc0001_00`)의 계층에 적혀 있다** — 바인딩 198개가
그쪽에 100% 맞고 옷 번들에는 0% 맞는다. 그 안에 `Bicycle/Origin/…`이라는
**자전거 전용 리그가 한 벌 더** 들어 있고, 사람과 자전거를 같이 미는 것은
그쪽이다.
...
메울 것은 **둘레 지형의 그 타일**이다. 바닥 삼각형(법선의 y 성분이 0.7 넘는 것)의
**서브메시와 UV 평면을 그대로 늘려 쓴다** — 무게중심 좌표는 삼각형 밖에서도
살아서, 옆 타일의 그림이 이어진다. 색도 그림도 우리가 지어내지 않는다.

⚠️ **칸마다 제일 가까운 것을 집으면 숲 바닥이 누더기가 된다.** 한동안 그랬다.
그 자리에 뭐가 가깝든 상관없이 나무 밑에는 나무가 설 만한 땅이 깔려야 하는데,
「제일 가까운 것」은 절벽·모래·눈을 그대로 집어 온다. 씬에 광선을 쏘아 줄기가
밟고 선 그림을 세어 보면:

| 자리 | 줄기 | 밟고 선 것 |
|---|---:|---|
| 떡잎마을 | 111 | **63그루가 눈**(`s_sonwp`·`s_snow04`) — 초록 마을인데 |
| 영원의숲 | 107 | **33그루가 절벽**(`criffp`) |
| 예지호수 | 84 | **68그루가 풀이 아니다**(`nectgr` 39 · `criffp` 29) |
| 209번도로 | 113 | 16그루가 절벽·모래·바다 |

그래서 **그림 하나를 골라 그것만 쓴다** (`plates.oneGround`). 고르는 순서는
① `groundRank`의 등급 ② 같은 등급이면 실제로 제일 넓게 깔린 것. 고를 것이 하나도
없으면 거르지 않는다 — 발밑이 뚫리는 것이 누더기보다 나쁘다.

⚠️ **누가 고르느냐가 그다음 고장이었다.** 청크가 혼자 고르게 두면 **청크 선이
그대로 드러난다.** 자기 청크만 보므로 옆 청크와 답이 갈리기 때문이다. 실측
(창 하나 안에서, 칸을 하나하나 광선으로 찍어 `.audit/floorSeam.mjs`):

| 자리 | 갈린 꼴 |
|---|---|
| 예지호수 (0,0)·(1,0) | 제 바닥이 절벽뿐이라 **분홍 바위 `criff`로 792칸** — 바로 옆 (0,1)은 풀 `ngrass` 616칸 |
| 영원의숲 (0,1) | `criffp`(어두운 절벽) 983칸 — 나머지 여덟 청크는 풀 |
| 선단시티 (9,5)·(9,6)·(10,6) | `criff` — 나머지는 `ngrass`. 둘 다 흰 눈인데 경계 130자리가 드러난다 |

숲 한복판에 **칼로 그은 듯한 직선**이 그어지는 것이 이것이다. 그래서 고르는 것을
청크 밖에서 한다 — **자기 + 이웃 한 겹(3×3)**의 바닥 넓이를 합쳐서 고르고
(`plates.groundArea`+`pickGround`), 그 그림이 제 청크에 없으면 이웃에서 그것만
빌려 온다. 맞닿은 두 청크는 아홉 칸 중 여섯을 같이 보므로 답이 거의 늘 같아진다.
창 전체로 넓히지 않는 것은 설원과 초원이 한 창에 같이 실릴 때 한쪽이 통째로 남의
땅이 되기 때문이다. 고친 뒤 여섯 자리 전부 청크마다 같은 그림을 깐다
(`criff` 1,651칸 → 0 · `criffp` 983칸 → 0).

⚠️ **등급을 이름으로 매기면 안 된다.** 롬 이름에는 오타가 있고(눈 그림 하나가
`s_sonwp`다) 뜻을 못 읽는 이름이 태반이다(`nectgr`·`criffp`·`fenter`). 그림
조각의 **불투명 텍셀 평균색**으로 가른다 — 실측(그림 1,133가지):

    풀   nectgr 초록 130 · ngrass 98 · nhana 95 · fenter 78
    흙   nsandp 26 · ckado 7 · criffp −27 · criff −36
    눈   s_snow04 밝기 253.6 색기 0.006 · s_sonwp 246.6 0.008 · c09_ice2 248.6 0.037
    모래 beach 밝기 240인데 **색기 0.108** — 눈과 갈리는 자리가 밝기가 아니라 색기다

문턱은 그 사이의 빈 구간에 둔다(초록 40 · 밝기 240 · 색기 0.05). 색으로 재야 하는
까닭이 하나 더 있다: **설원에서는 롬이 같은 이름에 눈 팔레트를 쓴다.** 선단시티의
`ngrass`와 `criff`가 둘 다 rgb(250,250,255)다 — 이름으로 갈랐으면 217번도로를
「풀」로 읽었을 것이다.

⚠️ **평균색만 보면 「덧그림」이 눈으로 뽑힌다 — 그것이 「투명 타일」의 정체다.**
원작은 잔디·바위 **위에 겹쳐 까는 한 겹**을 따로 갖고 있다. 그 그림은 대부분이
투명이고 남은 텍셀만 새하얀데, 위의 잣대는 **불투명한 텍셀만** 세므로 그대로
「눈」 2등급을 받는다. 그걸 숲 바닥에 깔면 알파 컷(0.5)에 잘려 **발밑이
반쯤 뚫린다.** 실측(`.audit/groundOpaque.mjs`, 그림 1,140가지의 불투명 비율):

| 그림 | 불투명 | 무엇인가 |
|---|---:|---|
| `s_sonwp` · `s_snow02` · `s_snow` | 23.6% · 29.7% · 49.2% | 눈 덧그림 |
| `c09_ice2` · `c09_ice` | 48.2% · 55.2% | 얼음 덧그림 |
| `bridge` · `dhole_05` | 84.9% · 85.8% | 다리·구멍 |
| `ngrass`·`nectgr`·`criff`·`criffp`·`s_snow04`·`beach`·`beachp`·`nsandp`·`wcliff` | **전부 100.0%** | 진짜 바닥 |

진짜 바닥 그림은 하나도 빠짐없이 100.0%이고 그 아래는 98.8%(`temp_05`·`mroom_09`)
부터다 — 그 빈 구간에 문턱을 놓는다(`GROUND_OPAQUE` 0.995). 미달이면 색을 보기
전에 **0등급**이라 바닥으로도 옆면으로도 안 쓴다. 고치기 전에는 선단시티과
217번도로의 `메운 바닥`에 `c09_ice`·`c09_ice2`가 각각 104삼각형 깔려 있었고
(`.audit/patchTex.mjs`), 고친 뒤 두 자리 다 남은 그림이 전부 100.0%다.

⚠️ **풀숲 그림은 아예 후보에서 뺀다.** 색으로만 고르면 원작이 풀숲을 그리는 데
쓰는 그림이 제일 초록이라 자주 이기는데(예지호수에서 `nectgr`이 `ngrass`를
11.6% 대 11.1%로 이겼다), 깔고 나면 숲 바닥이 통째로 풀숲으로 보인다 — 정작
인카운터가 나는 칸은 거기가 아닌데도. 어디가 풀숲인지는 그림이 아니라 **거동값
0x02·0x03**이 말하므로(`Grass.tsx`), 그림마다 덮은 칸 중 풀숲 칸의 비율을 센다
(`plates.tuftTextures`). 실측이 깨끗하게 갈린다:

| 그림 | 영원의숲 | 209번도로 |
|---|---:|---:|
| `nectgr` | **100.0%** (244칸) | **100.0%** (60칸) |
| `l_grass_u` · `l_grass_m` | | **100.0%** (22 · 89칸) |
| `ngrass` | 0.5% (6,220칸) | **0.0%** (18,820칸) |

⚠️ **번호가 아니라 이름으로 거른다.** 이웃에서 빌려 온 바닥은 재질 번호가 이
청크의 것이 아니라(`borrowFloors`가 배열 뒤에 덧붙인다), 번호로 거르면 빌려 온
풀숲이 그대로 통과한다.
...
⚠️ **「바닥이 하나도 없을 때만 빌린다」가 위의 분홍 바위 판을 만들었다.**
예지호수 (0,0)에는 바닥이 있긴 있었다 — 절벽뿐이었을 뿐이다. 그래서 빌려 오는
길로 안 가고 제 절벽으로 숲을 792칸 깔았다. 지금은 **고른 땅(3×3의 답)이 제
청크에 없으면** 빌린다. 빌릴 때도 그 그림을 **가진** 이웃만 고르고 그 삼각형만
베낀다 — 아무 이웃이나 잡으면 절벽뿐인 옆 청크에서 절벽을 빌려 놓고 「빌려 왔다」로
끝내 버리고, 통째로 베끼면 쓰지도 않을 삼각형을 수만 개 옮긴다.
...
#### 원작 지형에는 옆면이 없다 — 턱을 세워 준다

⚠️ **4세대 땅은 「높이가 다른 가로 판」을 쌓은 것이고 세로면이 아예 없다.**
카메라가 위에서 비스듬히 한 방향만 보므로 옆을 만들 까닭이 없었다. 절벽조차
「절벽처럼 보이는 그림(`criffp`)을 깐 가로 판」이다. 실측(`.audit/wallPool.mjs`,
창 하나의 `지형` 메시에서 |n·y|/|n| < 0.5인 삼각형):

| 자리 | 세로 삼각형 |
|---|---|
| 217번도로 | 82개 — 부두 `seaside3` 20 · 난간 `c2_rale_b` 12 · `imped` 10 · 계단 `newstep` 40 |
| 선단시티 | 같은 82개(같은 소품) |
| 예지호수 | **0개** |

전부 소품이고 **땅에서 나온 것은 하나도 없다.** 1인칭으로 돌면 그 턱마다 밑이
훤히 보인다 — 이웃보다 0.3 넘게 높은 자리에서 옆으로 광선을 쏴 본 값
(`.audit/floorSeam.mjs`, 「뒷면만 있는 것」이 0이므로 **감는 방향 문제가 아니라
없는 것**이다):

| 자리 | 고치기 전 뚫림 | 고친 뒤 |
|---|---:|---:|
| 217번도로 | 243 / 243 (**100%**) | **0** |
| 열매숲 | 405 / 1,152 (35.2%) | 93 (8.1%) |
| 떡잎마을 | 75 / 273 (27.5%) | 35 (12.8%) |
| 선단시티 | 493 / 1,007 (49.0%) | 184 (18.3%) |
| 영원의숲 | 353 / 849 (41.6%) | 336 (39.6%) |
| 예지호수 | 148 / 519 (28.5%) | 148 (28.5%) |

바닥이 있는 칸마다(`FloorSource.levels`) 네 이웃의 **그린 높이**와 견주고, 낮은
쪽으로 세로 판을 세운다(`floorPatch`의 `skirt`).
...
| 자리 | 위 / 아래 | 통행 | 옆면 |
|---|---|---|---|
| 축복시티 건물 (177,752) | `c1_o02` y5.69 / y2.16 | 막힘 | 맨 위에서 — 그 판이 곧 건물 외벽이다 |
| 포켓몬리그 로비 (9,2) | `leag_kabe02` y9 / `leag_yuka01` y−0.13 | 막힘 | 맨 위에서 — 그 판이 곧 방 벽이다 |
| 연고시티 성문 아치 (458,684) | `area4_gate_a` y8.69 / `lgreen` y2 | 통행 | **안 세운다** — 걸어 지나는 문이다 |
| 연고시티 성문 옆 (458,678) | `area4_gate_b` y8.68 / `ngrass` y2 | 막힘 | 아래 참조 (등급 0이라 안 세운다) |

걸어 다니는 칸에서는 **걷는 높이에 제일 가까운 층**을 쓴다 — 이 함수가 「그 층이
덮였는가」를 볼 때 쓰는 잣대와 같다(`LEVEL_SLACK`). `near`로 맨 위를 넘겨 주므로
층이 진짜로 겹친 자리(영원의숲 위단 y=2.813 / 아랫길 y=1)에서는 여전히 윗단이
나온다 — 거기는 윗단에도 걸을 판이 있다.

⚠️ **옆면의 그림은 그 층을 그린 삼각형에서 가져온다.** 높이와 그림을 따로 고르면
엉뚱한 그림이 발린다 — 포켓몬리그 로비의 9타일짜리 벽에 포켓몬센터
바닥(`pc_room1`)과 프렌들리숍 바닥(`fs_room1`)이 발려 있었다.

⚠️ **원작이 세로면을 이미 그려 둔 칸에는 안 세운다** (`FloorSource.standing`).
연고시티 성문 옆기둥이 그 자리다 — 원작 벽이 서 있는데 그 그림이 불투명 58%라
등급 0이라서, 이웃에서 `ngrass`를 빌려 오면 진짜 벽 앞에 4.63타일짜리 **잔디
커튼**이 한 겹 더 선다. 청크 57의 `area4_gate_a`·`_b` 37삼각형이 그 청크의
**유일한** 세로면이고, 그것으로 충분하다.

세로면은 위에서 보면 넓이가 0이라 칸 훑기에 안 걸린다 — 상자로 찍어 적는다.

⚠️ **새까만 그림은 세워도 구멍과 구별이 안 된다** (`PITCH_BLACK`). 글로벌터미널
뒷벽이 그랬다: 원작이 그 칸 위 y4.38에 `black`(8×8, 평균 밝기 0)을 한 장 깔아
두었고, 「그 층을 그린 그림」을 그대로 접어 내리니 **세로 삼각형 344장이
새까맣게** 서서 화면 위쪽이 통째로 검은 구멍이 됐다. 문턱은 잴 것도 없이 비어
있다 — 그림 칸 **2,736가지**의 평균 밝기 분포에 **0.0인 여섯 가지**
(`h_kage`·`black`·`m_4ten_kage`·`m_lhouse01_03`·`ume`·`m_dun06_kage`, 셋은
이름이 그늘이다)와 16 위밖에 없고 사이가 없다. 등급 0으로 떨어뜨리면 그 칸은
...
| 자리 | 고치기 전 | 고친 뒤 |
|---|---|---|
| 연고시티 | `ngrass` 212 · `lgreen` 12 · `lgreenp` 12 | 80 (성문 바깥쪽 담) |
| 꽃향기마을 | `ngrass` 164 | **0** |
| 축복시티 | `c1_g1` 480 | **0** |
| 선단시티 | `criff` 390 · `wcliff` 358 · `criffp` 214 | **0** |
| 포켓몬리그 | `pc_room1` 128 · `leag_yuka01` 112 · `fs_room1` 96 | **0** |

남은 것은 그 칸을 실제로 그린 그림이다 — 연고시티는 성문(`area4_gate_*`),
포켓몬리그는 방 벽(`leag_kabe02` 320장), 선단시티는 영역 그림(`a8_sora_f` 674장).

⚠️ **아직 남은 자리.** 대습초원 전망대는 계단 쪽 모서리가 데크 그림으로 서는데
긴 옆면은 여전히 땅 그림이다 (높이가 1타일이라 위 표의 1.5타일 잣대에 안 걸린다).
무엇이 그 면을 그리는지 아직 안 쟀다.

⚠️ **이웃과 견줄 때는 걷는 높이로 견주면 안 된다.** 둘이 늘 같지 않다 —
예지호수 지형 1,295칸 중 986칸이 걷는 높이와 어긋나고 흔한 차이가 −2.5다.
메울 것은 **보이는 틈**이므로 그린 높이끼리 견준다. 걷는 높이가 쓰이는 자리는
위의 「그 칸의 **어느 층**에 서는가」 하나뿐이고, 거기서는 그 칸 안의 층들 중
하나를 고르기만 하므로 어긋난 값이 새 층을 만들지 않는다. 반대로 **메운 바닥은 걷는 높이와 한 칸도 안 어긋난다**
(217번도로 0/7,559 · 선단시티 0/8,600 · 예지호수 0/3,569) — 눈밭에 보이던 단차는
우리가 낸 것이 아니라 원작의 단이고, 없던 것은 그 **옆면**이었다.

⚠️ **`split.cells`를 돌면 안 된다.** 그것은 「잎이 덮은 칸」이라 청크의 일부일
뿐이다 — 그걸로 돌았더니 열매숲 턱 405자리 중 312자리가 그대로 남았다.

베낄 벽이 없으므로 **윗면 그림을 그대로 접어 내린다** — 눈밭 턱은 눈, 잔디 턱은
잔디가 된다. 원작에 없는 면이라 「원작대로」가 없는 자리다. 한 칸(1단위)씩 끊어
쌓고 UV는 타일 **안쪽으로** 되짚는다: 바깥으로 한 칸 더 가면 UV가 타일을 벗어나
가장자리 텍셀로 눌린 민무늬 띠가 된다(`lay`가 겪은 것과 같은 고장).

⚠️ **속이 빈 그림으로는 옆면을 못 세운다.** 그 칸을 그린 것이 원작이 잔디 위에
겹쳐 깐 얼음 덧그림(`c09_ice` 55% · `c09_ice2` 48%)일 수 있다 — 세워 놓으면 반이
비쳐 턱을 메우기는커녕 창문이 된다. 그래서 옆면 후보에서 **등급 0을 뺀다**
(물·풀숲·속 빈 그림). 걸어 다니는 칸이면 그때만 이웃 바닥에서 그림을 빌린다.

남은 것은 **원작이 바닥으로 안 치는 비탈**(|n·y|/|n| < 0.7)에 붙은 턱이다.
영원의숲 위단(y=2.813)과 예지호수 물가 비탈(y 1 → 0.5 → −0.5 → −1.5)이 그렇다.
그 면은 `levels`에 안 적히므로 어느 높이의 턱인지 정할 수 없다 — 못 세운다.

#### 실내에는 앞벽이 없다 — 세워 준다

⚠️ **원작 실내는 카메라가 한 각도로 고정이라 안 보이는 쪽 벽을 아예 안 만들었다.**
문이 있는 앞벽(남쪽)과 한쪽 옆벽이 그렇다. 위에서 비스듬히 내려다보는 두 화면에서는
그 자리가 화면 밖이지만, 카메라가 도는 우리 화면에서는 **그 자리가 통째로 검다.**

실측 (`pnpm holes --walls` — 바닥이 끝나는 자리마다 세로 삼각형이 있는지 센다.
칸이 아니라 **모서리**로 세는 것이 중요하다: 「이 칸에 벽이 있다」로 두면 북쪽 벽
하나가 그 칸의 서·동·남까지 막힌 것으로 세어 진짜 구멍을 놓친다):
...
굴은 원작이 사방을 다 만들어 두었다 — 그래서 거기서는 한 장도 안 는다.

⚠️ **위 「둘레」는 그때 세던 법으로 잰 값이다.** 그 뒤 검사가 **맵 밖**과 **바위 위**를
빼게 되어 (DEPLOY.md §5.2) 지금은 조금 작다 — 주인공 방 32 · 집 1층 33 · 프렌들리숍 38 ·
포켓몬센터 55 · 박물관 68 · 도서관 56. **벽이 없는 쪽은 그대로 0**이다.

⚠️ **「세로 면이 있다·없다」로는 모자란다 — 어디까지 덮는지를 봐야 한다.**
무릎 높이 굽도리 한 장이 있으면 그 모서리가 막힌 것으로 세어지고, 눈높이 광선은
그 위로 그냥 넘어간다. 실측으로 주인공 방 북쪽의 덮는 높이가 `-0.1~4.3`이라
넉넉해 보였지만 그것은 **낮은 굽도리와 높은 지붕의 최소·최대**였고 가운데가
비어 있었다. 그래서 모서리마다 **높이 구간**을 모아 합치고, 안 덮인 띠에만
판을 세운다 (`roomWalls.gapsOn`). 넷으로 갈린다:

| 그 모서리의 세로 면 | 뜻 | 하는 일 |
|---|---|---|
| 아예 없다 | 원작이 앞벽을 통째로 안 만들었다 | 바닥부터 세운다 |
| 눈높이 띠에 **걸친다** | 벽은 있는데 덜 덮는다 | **빈 구간만** 메운다 |
| **발치에만** 있고 옆이 **끝내 선다** | 벽에 난 구멍이다 | 옆 벽 높이로 세운다 |
| **발치에만** 있고 옆도 그렇다 | 바닥이 단으로 끝난다 | **안 세운다** |

넷째가 없으면 깨어진 세계의 뜬 발판마다 상자가 씌워진다 — 거기는 옆면이 아래로만
뻗어 있어서 「벽이 없다」로 보이기 때문이다 (맵 573, 274자리).

⚠️ **셋째에서 묻는 것은 「옆에 원작 벽이 있는가」가 아니라 「옆이 끝내 서는가」다.**
도서관 남쪽이 그 반례다. 발치만 덮인 셋(9·13·17,11) 양옆은 원작에 벽이 **아예
없어서 우리가 세우는** 자리라, 원작만 보면 옆도 비었으니 안 세우게 된다 — 실측으로
그 상태에서 눈높이 광선 **108발**이 그 셋으로 샜다. 그래서 `roomWalls`는 두 번
돈다: 먼저 모서리마다 갈래만 매기고(없다·걸친다·발치뿐), 그 다음에 발치뿐인
자리가 **매겨진 갈래**로 옆을 본다. 출입구는 비우는 자리라 갈래에서 뺀다 —
문간을 「선 벽」으로 세면 그 옆 단이 벽이 된다.

실제 카메라에서 쏜 광선 중 **새는 것** (`--eyes`. 실내 1인칭은 24시점 × 117발 =
2,808발, 3인칭은 자리마다 117발):

| 자리 | 처음 | 구간으로 본 뒤 | 옆을 본 뒤 |
|---|---:|---:|---:|
| 주인공 방 | 532 | 9 | 9 |
| 무쇠탄갱 | 202 | 2 | **0** |
| 천관산 | 161 | 94 | **0** |
...
무쇠탄갱은 원작 벽이 바닥+2.8에서 끝나 그 위 0.2타일로 새고 있었다. 천관산 94는
맵 밖으로 나간 광선이었다 (`맵 밖` 갈래를 검사가 몰랐다 — DEPLOY.md §5.2).

확인 지점 27자리를 통째로 훑으면 **광선 66,600발 중 49발**이 샌다 (벽 35 · 바닥 14).
스물일곱 중 **열아홉 자리가 0**이다. 남은 것은 아래와 같고, 주인공 방·집 1층·
상점의 1인칭이 그 절반이다 — 셋 다 새는 높이가 **바닥 + 2.8~2.9**로 같다.

| 남은 자리 | 3인칭 | 1인칭 |
|---|---:|---:|
| 집 1층 · 프렌들리숍 · 주인공 방 | 0 · 0 · 0 | 15 · 10 · 9 |
| 무쇠시티(실외) | 8 | 0 |
| 진실호수 · 깨어진 세계 · 떡잎마을 · 영원의 숲 | 0 · 2 · 1 · 0 | 3 · 0 · 0 · 1 |

⚠️ **1인칭 수는 판마다 1~2씩 흔들린다.** 같은 소스로 두 번 재면 집 1층이 16·17,
프렌들리숍이 10·9다(2026-09-23 · `--visual=legacy`로 새 레시피를 끄고 한 번 더 쟀다).
새는 자리는 늘 같은 띠 — 북쪽 벽 **바닥 + 2.8**이다. 그래서 여기 수와 ±2 안이면
늘어난 것이 아니다. 새로 생긴 자리인지는 `.audit/holes.json`의 `atFirst`(칸과 높이)로 본다.

⚠️ **출입구에는 문 모양 구멍을 남긴다 — 비우지도, 메우지도 않는다.** 워프 칸에는
바닥에서 `DOOR_OPEN`(2.25타일)까지를 비우고 그 위로만 세운다. 옆 칸의 온전한 벽이
문설주가 되고 위가 인방이다. 위 표의 `0`은 전부 「출입구를 뺀 수」다.

양 극단을 다 해 봤고 둘 다 틀렸다: **통째로 비우면** 문간 칸에 선 사람의 정면이
통째로 검고(원작이 그쪽 벽을 아예 안 만들었다), **통째로 메우면** 문이 사라진다
(파일럿 보고 2026-09-22 · REPAIR §61). 2.25타일은 원작 문짝 19종의 윗변
1.938~2.75타일과 사람 눈높이 1.5타일 사이다.

⚠️ **폭은 워프 칸 그대로다.** 두 칸짜리 문은 **워프 칸이 둘**이라 자료에 이미 들어
있다 — 양옆까지 넓히면 문이 아닌 벽에 구멍이 난다.

⚠️ **실외에는 안 건다.** 거기서 바닥이 끝나는 자리는 맵 가장자리라, 세우면 세계가
상자 안에 갇힌다.

그림은 **그 방의 벽에서 베낀다** — 제일 가까운 세로 삼각형의 서브메시와 UV 평면을
그대로 쓰고, UV는 그 벽의 자리로 되짚어 가장자리 텍셀에 안 눌리게 한다

⚠️ **「제일 가까운」만으로는 못 고른다 — 원작 벽은 층이 겹쳐 있다.** 주인공 방
(맵 415)의 `z=3.6` 벽을 재면 네 층이 같은 평면에 있다:
...
⚠️ **「막힌 쪽으로 민다」만으로는 모자랐다 — 오히려 집 안으로 밀어 넣었다.**
그 규칙은 「길 위에 서지 마라」라서, 집도 울타리도 막힌 칸이니 나무를 그쪽으로
민다. 원작 나무는 위에서 본 그림 한 장이라 집에 걸쳐 있어도 화면에서는 집이
덮었는데, 그걸 입체로 세우니 **줄기가 벽을 뚫고 잎이 지붕에 박혔다.** 실측
(`.audit/treeProp.mjs`, 마을마다 창 하나에서 그루마다 소품의 월드 상자와 견준다):

| 자리 | 고치기 전 | 제일 깊은 것 | 고친 뒤 |
|---|---:|---|---:|
| 연고시티 | 29그루 | 2.17타일 | **0** |
| 선단시티 | 22 | 1.92 | 1 (0.02타일) |
| 잔모래마을 | 19 | 2.14 | **0** |
| 무쇠시티 | 10 | 1.49 | **0** |
| 장막시티 | 9 | 1.82 | **0** |
| 운하시티 | 7 | 1.53 | **0** |
...
같은 열 마을을 다시 재면 **98 → 1**이다. 남은 하나는 선단시티 (361,201)에서 잎이
소품에 **0.02타일** 걸친 것 — `CROWN_REACH`로 깎고 남은 자리다. 열 곳 다 발밑은
깨끗하다(**바닥 뚫린 칸 0/6,561** · 지평선 밑 하늘 **0/28,320픽셀**). 무쇠시티만
(306,718)·(306,719) 두 칸이 뚫려 있는데 그건 나무가 아니라 원작 지형 쪽이다.

막힌 칸을 두 갈래로 모은다 — 배치 기록이 세우는 **소품**(집·간판·다리)과, 원작
청크에 세워져 있는 **판**(울타리·표지판. 소품이 아니라 청크 지오메트리라 배치
기록에 없다). 소품은 모델 상자를 회전·크기까지 먹여 칸으로 찍고, 판은 판 한 장
(정점 6개)씩 잘라 찍는다 — 청크 통짜 상자로 보면 청크 전체가 막힌 것이 된다.

**지우지 않고 줄인다.** 원작 판이 거기 있었다는 것은 사실이라, 나무를 통째로
빼면 집 둘레에 민둥한 고리가 생긴다. 잎이 옆으로 제일 멀리 뻗는 것이 `0.97 r`
(`CROWN_REACH`, 덩이 여섯의 `max(|x| + r)`)이므로 남은 거리를 그 값으로 나눠
반지름을 깎고, **최소 크기(`RADIUS_MIN` 0.95)로도 안 들어가는 자리만** 비운다.
...
✅ **다리 밑을 지나간다.** 원작이 다리 위인지 아래인지를 가르는 상태
(`MAP_OBJ_STATUS_ON_ELEVATED_BRIDGE`)를 `engine/actor/bridge.ts`가 든다 —
**들어온 길이 층을 정한다.** 다리 어귀(`BRIDGE_START` 0x70)를 밟아야 「위」가 되고,
다리가 아닌 칸을 밟으면 풀린다. 그래서 같은 칸이 위를 건널 때는 땅이고 밑을 지날
때는 물이다(`MapObject_IsOnWater`). 한동안 다리 셋(0x73·0x78·0x7c)을 파도타기
목록에서 **빼는 것**으로 「위를 건넌다」를 흉내 냈는데, 그러면 밑을 지나는 순간
파도타기가 풀렸다 — 원작 거동표에는 그 셋에 `SURFABLE`이 서 있다 (PARITY §1.16).
...
⚠️ **둘 다 원작 스크립트로 들어간다.** 조건만 우리 것이고 그 뒤는 롬이 한다 —
`scripts.json`의 구간표에 `{from: 10000, file: 409, msg: 381}`이 있어서 **10000이
`SCRIPT_ID(FIELD_MOVES, 0)`**이고, 진입점 차례는 `scripts_field_moves.s`의
`ScriptEntry` 순서 그대로다. 앞 칸이 부르는 셋은 `Field_TileBehaviorToScript`가
돌려주는 번호(폭포 6 · 록클라임 3 · 물 4)이고, 기술 창은 `field_move_tasks.c`의
각 태스크가 넘기는 번호(풀베기 8 · 바위깨기 9 · 괴력 10 · 록클라임 11 ·
파도타기 12 · 폭포오르기 13 · 안개제거 14 · 플래시 15)다. 그 스크립트가
「물은 깊고 푸른 색이다」와 「○○가 파도타기를 썼다!」와 컷인과 여행기록을
전부 들고 있으므로, 조건만 보고 바로 움직이면 그 넷이 통째로 빠진다.

⚠️ **나무·바위·큰 바위는 앞 칸 표에 없다.** 그 셋은 지형이 아니라 배치 객체라
말을 거는 길로 제 스크립트가 이미 걸린다 — 실측으로 `events.json`에 10000번
49개 · 10001번 590개 · 10002번 49개다.

기술 설명은 `TEXT_BANK_MOVE_DESCRIPTIONS` 468줄이다. 줄 바꿈까지 원작 것이라
`
`을 지우면 안 된다.

**안개제거와 플래시는 날씨가 연다.** `FieldSpot`의 `fog`·`dark`가 지금 걸린
오버월드 날씨에서 온다 (`engine/world/overworldWeather`의 `OVERWORLD_WEATHER` —
안개 14 · 어둠 16). 자료는 맵 헤더에 있다: 593개 맵 중 **안개가 51개**,
**어둠은 하나**(맵 284 `D21R0101` — 미혹의 동굴)다.

⚠️ **그 둘은 기술 창에서만 나간다.** `Field_TileBehaviorToScript`에 안개도 어둠도
갈래가 없고, `FIELD_MOVES`의 안개 진입점 5번은 이름부터 `FieldMoves_Fog_Unused`다.
그래서 `tryFieldMove`는 `fog`·`dark`를 꺼 놓고 자리를 묻는다 — 안 그러면 안개 낀
맵에서 아무 벽에 대고 A를 눌러도 안개제거가 걸린다.
...
540칸에 0번 264 · 1번 121 · 2번 155가 쓰이고(합이 540이다), 짐작으로 0번을 쓰면 풀색
포켓몬이 보라색이 된다. → `data/pokeIcons.png` 768×736, 104KB.

⚠️ **앞 495칸만 종 번호와 같다.** 그 뒤 45칸이 **폼 아이콘**이다 (PARITY §3.4) —
차례는 `res/pokemon/meson.build`의 `poke_icon_files`가 정하고, 495 마나피 알 ·
496~498 테오키스 · 499~526 안농 스물여덟 · 527~528 도롱충이 · 529~530 도롱마담 ·
531~532 조개무지·트리토돈 동쪽 바다 · 533 기라티나 오리진 · 534 쉐이미 스카이 ·
535~539 로토무 다섯이다. 어느 칸인지는 `engine/pokemon/form.iconSlot`이 답한다
(`PokeIconSpriteIndex`).

⚠️ **495번은 나쁜알이 아니라 마나피 알이다.** 나쁜알은 `icon.png`가 없고
(`ls res/pokemon/*/icon.png` = 495장), 그 폴더의 팔레트 번호가 마나피 알 것으로
쓰인다 — `speciesproc.c`가 `[ICON_MANAPHY_EGG] = bad_egg.icon_palette`로 내보낸다.

⚠️ **안농은 `a`가 한 번 더 들어간다** (`form_registry.json`의 `__dupe_base_icon`).
기본 아이콘과 같은 그림이지만 아카이브에는 칸이 따로 있어서, 등록부만 세면
칸이 하나 밀린다.
...
무리가 처음부터 다 보여서, 밟으면 나타나야 할 블록이 **이미 서 있다.** 그리고
층을 갈아탈 때마다 이어받는 것이 아니라 그 층 기본값으로 **다시 세운다**
(`SetPersistedHiddenGhostPropGroups(0)` 뒤 `InitActiveGhostPropManager(TRUE)`).
밟아야 나타나는 블록은 네 층에 42개다 — 576:23 · 577:5 · 582:6 · 583:8.

⚠️ **기라티나 방의 여섯은 방아쇠가 아니라 사건이 세운다.**
`showGiratinaRoomPlatforms`가 무리 1·2·3을 48프레임 간격으로 올린다(첫 뜸 36).
거둘 때는 거꾸로다(첫 뜸 16). 이 명령 둘을 「연출」로 넘기면 그 방에서 못 나간다.

⚠️ **맵 581의 유일한 방아쇠는 소품이 없는 무리 0을 가리킨다.** 원작 자료가
그렇다 — 밟아도 아무 일도 안 일어난다. 고치지 않고 그대로 둔다.

⚠️ **이 세계의 맵에는 BDHC 판이 없다.** 격자에 지면 높이를 물으면 0이 오는데
실제 지면은 그 위라, 물어본 대로 끌어내리면 주인공이 판에 파묻힌 채로 걷는다.
판이 있는 층은 바닥 판의 y를, 없는 여섯 층은 **그 층에 선 사람들의 y 중앙값**을
쓴다(배치표의 y는 고정소수점 세계 높이다 — 난천이 18939904 = 4624단위 = 타일
289, `offsetY` 288이니 제자리는 1). `distortionBridge.groundY`가 그 다리다.

⚠️ **바닥 통행을 맵의 2D 격자가 안 잡는다.** 층마다의 진짜 바닥은 **떠 있는 판**
넷 갈래(바닥·서쪽 벽·동쪽 벽·천장)이고, 어느 칸을 밟을 수 있는지는 그 판이
가리키는 통행 격자가 정한다. 맵 격자만 보면 허공이 전부 걸을 수 있는 땅이다.

⚠️ **격자를 갈래마다 다르게 전치한다.** 세로축이 바닥은 `x - bx`, 동쪽 벽은
`y - by`인데, **서쪽 벽과 천장은 뒤집힌다**(`sy - (y - by)` · `sx - (x - bx)`).
칸 번호는 `세로 + 가로 × 줄수`다.

⚠️ **경계가 닫힌 구간이다.** `size`는 개수가 아니라 차이라 `start + size`가
안에 든다. 하나를 빼면 판마다 한 줄씩 못 밟는다.

⚠️ **맵 열 개가 하나의 수직 통로로 쌓여 있다** — 1F가 y=289, B7F가 65,
기라티나 방이 1이다. 우리 격자는 층마다 지역 좌표라 경계에서 오프셋을 더한다.

⚠️ **578번 맵이 비어 있다.** 573~583 사이에 쓰지 않는 번호가 하나 끼어 있어서
「B4F 다음이 B5F」로 세면 통째로 한 칸 밀린다.

#### 층을 잇는 것은 승강 발판뿐이다

계단도 사다리도 없다. 떠 있는 발판 **서른넷** 중 **열여덟**이 승강이고
(`sMovingPlatformsMapTemplates`), 그 위에 서면 경로 표 스물둘(`sElevatorPlatformPaths`)이
어디로 얼마나 가는지 정한다. 나머지 열여섯은 그냥 밟고 지나가는 판이다.

경로가 맞는지는 **다른 자료와 교차검증**했다 — 경로의 `finalTileYOffset` 합이
`tw_arc` 맵 표의 층 오프셋 차와 열여덟 자리 전부에서 같다. 두 층을 한 번에
지나는 자리 둘(B3F↔B5F, 경로 8→9과 15→16)도 그 안에 든다.

⚠️ **경로 20·21은 아무도 안 쓰고, 21번은 부호가 뒤집혀 있다.** 아래로 32칸을
가야 하는데 한 걸음이 위쪽이라 목표에 영영 못 닿는다 — 원작에서는 쓰는 발판이
없어서 안 터진다. 우리는 길이를 재서 0이면 안 태운다.

⚠️ **바위가 떨어지는 자리는 열넷뿐이다** (`sBoulderFallLocations`). 그중 여덟은
`flagIndex`가 **`-1`**인데, 「자리 없음」 값(17)과 **다른 값이어야 한다** —
원작이 `u32`에 −1을 넣어 두어 `switch`의 어느 `case`에도 안 걸리고 `default`로
떨어지는 것이 곧 「틀린 웅덩이」다. 둘을 합치면 B6F의 엉뚱한 구멍이 전부 정답이 된다.

### 2.23 anim.s — 기술마다 **전용 연출 대본**이 있다
...
⚠️ **「원작도 틀 몇 개로 때웠다」가 아니다.** 한동안 그렇게 적고 기술 번호
산술(`(id*7)%11`)로 크기·회전을 흔들었다. 「471개가 서로 다른 서명을 갖는다」는
시험이 초록이었지만 화면은 전부 같았다 — 서로 다른 것과 **맞는 것**은 다르다.
그래서 지금 시험은 「배경 색이 468가지가 아니라 **13가지로 뭉치는지**」를 본다.

⚠️ **사라지는 것은 `RemovePokemonSprite`가 아니다.** 그건 대본이 더 붙인 그림을
치우는 것이고, 쓴 쪽이 땅에 숨거나 하늘로 뜨는 것은 `Func_HideBattler`다.
지진·땅가르기처럼 전용 함수로 빠지는 기술이 있어서 `Func_ShakeBg`·
`Func_Earthquake`도 같이 본다 — 안 보면 땅을 흔드는 기술이 화면에서 조용해진다.
...
`fielddata/pokemon_trade/fld_trade.narc` 넷, 하나가 **u32 스무 칸(80바이트)**이다
(`NPCTradeMon`). → `data/npcTrades.json` 1KB
(`tools/extract/npcTrades.js` · `import/platinum/npcTrades.ts`).

```
종족 개체값6 (미사용) 트레이너번호 콘테스트5 성격값 지닌도구 성별 (미사용) 언어 요구종족
 63  15 15 15 20 25 25   28    25643    0 0 0 0 0   142    155     1     0     2     66
```

⚠️ **개체값 차례에 스피드가 넷째다.** 능력 화면 차례(HP·공격·방어·특공·특방·
스피드)가 아니라 저장 차례(HP·공격·방어·**스피드**·특공·특방)다. 뒤집으면 특공
25인 아브라가 스피드 25로 온다.

⚠️ **표에 레벨 칸이 없다.** 받는 마리의 레벨은 내가 건네는 마리의 것이다
(`NPCTrade_FillAnimationTemplate`이 고른 자리에서 읽어 넘긴다).

⚠️ **이름은 이 표에 없다.** 별명 넷과 트레이너 이름 넷이 뱅크 370
(`TEXT_BANK_NPC_TRADE_NAMES`) 한 벌에 0~3·4~7로 들어 있다 —
`NPCTrade_GetOTName`이 `MAX_NPC_TRADES + id`로 뒤쪽을 집는다. 장면의 네 줄은
뱅크 350(`TEXT_BANK_TRADE`)이고, 둘 다 스크립트가 안 가리켜서 대사 추출기의
`EXTRA_BANKS`에 손으로 적어야 실린다.

확정은 대조로 했다 — 디컴프가 같은 자료를 사람이 읽는 이름으로도 들고 있어서
(`res/npc_trades/*.json`) 스무 칸을 하나씩 맞대 본다. 브라우저 변환기는 디컴프가
없으므로 대신 **노드 산출물과 바이트로 같은지**를 시험이 본다.
...
분류2 세트수2 세트번호2×n                        ← 4 + 2n이 파일 크기와 315/315 맞는다
```

⚠️ **세트 수가 홀수면 뒤에 2바이트가 남는다.** NARC이 항목을 4바이트에 맞추기
때문이고, 315칸 중 93칸이 그렇다 — 전부 홀수 쪽이다. 이걸 모르고 크기로
배치를 검증하면 93건에서 깨진다.

⚠️ **개체값도 노력치도 이 표에 없다.** 개체값은 뽑는 쪽의 층이 정하고(0·4·8…31을
여섯에 똑같이), 노력치는 `evFlags`가 켠 능력에 510을 나눠 넣는다 — 그래서 이
표만으로는 능력치가 안 나온다 (`FrontierPokemon_Init`).

⚠️ **0번 형은 자리표시자다** — 기술이 하나도 없다. 뽑는 구간이 전부 1부터라
걸릴 일이 없지만, 951을 950으로 착각하면 자리가 하나 밀린다.

⚠️ **트레이너의 세트 번호는 배틀타워 것이다.** 배틀팩토리는 상대가 데리고 나올
것을 그 자리에서 형 표에서 뽑는다 (`ov104_0223AB0C`) — 시설장 소튼(309·310)이
세트를 0개 가진 것이 그 증거다.

이름과 대사는 여기 없다. 뱅크 21(이름 315)·614(대사 945 = 315 × 인사·이김·짐)에
있고 대사 추출기의 `EXTRA_BANKS`에 손으로 적어야 실린다 — 팩토리 화면의 뱅크
셋(363·364·365)도 같다.
...
팔레트 21+k  타일 12+k  배치 6+k     ← 아래 화면
```

⚠️ **위 화면 것만 굽는다.** 아래 화면에도 같은 하늘이 깔리지만 그쪽은 **3D
장면이 그 위에 서는 판**이다 (`whichScreenIs3D = DS_SCREEN_SUB`) — 두 장을
위아래로 이어 붙이면 하늘이 두 번 나오는 그림이 된다.

⚠️ **한 팔레트 파일에 16색 벌이 여럿이다.** 어느 벌을 쓰는지는 배치 칸의 **위
4비트**가 정한다 — 한 벌만 읽으면 그림 절반이 딴 색으로 나온다. 4bpp인 것은
`bpp` 칸이 3인 것으로 안다(4가 아니다).

⚠️ **장마다 크기가 다르고 그래서 파일도 따로다.** 첫 장이 512×256, 나머지 둘이
256×256인데 화면은 256×192다 — 큰 것은 **흐르라고** 그렇다. 화면이 이 그림을
감아 돌려야 해서(`background-repeat`) 한 장에 모으면 옆 장이 딸려 나온다.

⚠️ **아래쪽이 넓게 한 색인 것은 원작 그림 그대로다.** 하늘이 위 96픽셀이고
나머지는 단색인데, 원작은 그 위로 사람 스프라이트가 지나간다.

흐르는 차례(237줄의 자리와 정렬)는 이 아카이브에 없다 — 오버레이 99의 코드
표라 디컴프에서 굽는다 (`pnpm gen:credits`). 글도 여기 없고 대사 뱅크 548이다.

### 2.27 mmodel/fldeff.narc — 머리 위 느낌표 둘

`data/mmodel/fldeff.narc` **201칸** 중 **4번과 13번** → `data/npc/emote.png`
183B (`tools/extract/npcSprites.js` · PARITY §1.13 · §7.9). 변장 더미 넷
(103~106번)과 같은 아카이브에서 같은 방식으로 뽑는다.

⚠️ **오버레이가 적어 둔 `{ 1, 12 }`는 NARC 번호가 아니라 자원 슬롯이다**
(`ov5_021F5C18`). 슬롯에 무엇이 실리는지는 등록하는 쪽이 정하고
(`ov5_021F5AB4` · `ov5_021F5B00`), 텍스처는 `ov5_021DFA3C(…, 4, …)`와
`(…, 13, …)`이다. 슬롯 번호로 뽑으면 4칸짜리 32×32 그림이 나와 크기 검사에서
걸린다 — 실제로 거기서 한 번 걸렸다.

롬 안의 이름이 그 둘을 다시 확인해 준다: **`sisen_ef`**(視線 — 눈이 마주쳤다)와
**`saisen_ef`**(再戦 — 재대결). 둘 다 16×16 4bpp 한 장이다.

원작이 이걸 얹는 방법은 **이동 동작**이다 (`MOVEMENT_ACTION_EMOTE_*`). 사람에게
걸음 목록을 걸듯 느낌표를 걸고, 그 동작이 끝날 때까지 다음 걸음이 안 간다.
튀어 오르는 곡선은 속도 하나로 만든다 (`ov5_021F5CD4`) — 처음 6이고 오른 자리가
0이 아닌 동안 프레임마다 2씩 깎아, 정확히 일곱 프레임 만에 0으로 돌아온다:

```
6 · 10 · 12 · 12 · 10 · 6 · 0   (16분의 1칸. 꼭대기 12 = 4분의 3칸)
```

⚠️ **오래 서 있는 표시가 아니다.** 내려앉은 뒤 서른 프레임을 서 있다 사라진다 —
다 해서 **37프레임(0.6초)**이다. VS시커가 세운 트레이너를 나중에 알아보는 것은
이 표시가 아니라 **그 사람이 도는 것**이다 (`MOVEMENT_TYPE_VS_SEEKER_SPIN`).

⚠️ **모델은 안 굽는다** — 변장 더미와 같은 이유다. 다만 원작 판때기는 제 높이를
...
**BTP0(PAT0)**는 재질마다 키프레임 `(frame u16, tex u8, pal u8)` 목록이고 값이
다음 키까지 버틴다. 자리는 공개 문서 그대로고 롬으로 확인했다 —
**트랙 이름 스물셋이 스물셋 다 그 소품의 재질 이름과 맞는다**
(`import/platinum/matAnim.test`). 에스컬레이터가 이 갈래의 본보기다:
`esca_up1.1~4`를 다섯 프레임마다 갈아 끼운다.

⚠️ **PAT0가 부르는 그림은 어느 재질도 안 가리킨다.** `esca_up1.2~4`처럼 **첫
장만** 재질이 쓰고 나머지는 TEX0 안에만 있다 — 소품 시트를 「재질이 쓰는 것」만
구우면(`wantedItems`) 계단이 멈춘 그림으로 선다. 그래서 `convertProps`가 PAT0
...
| 채널 | 깃발 | 값 | 무엇인가 |
|---|---|---|---|
| 0 | 48 (62/62) | fx32 1.0 | U 배율 |
| 1 | 48 (62/62) | fx32 1.0 | V 배율 |
| 2 | 32 (62/62) | u16 둘 `[0, 0x1000]` | 회전 (sin, cos) — 전부 단위 |
| 3 | 16 또는 48 | 배열 또는 상수 | **U 이동** |
| 4 | 16 또는 48 | 배열 또는 상수 | **V 이동** |

⚠️ **깃발을 비트로 가르면 안 된다.** 상수 깃발이 48(0x30)이라 「배열」 비트
0x10이 **서 있다** — 마스크로 보면 상수를 배열로 읽어서 자전거 비탈의 U가
674까지 튄다. 문서대로 **16과 정확히 같을 때만** 배열이다.

⚠️ **이동값의 자는 「텍셀 × 32」다** (`1.10.5`). 정규화 UV(fx32)로 읽을 수도
있었는데, 그림 크기가 2의 거듭제곱이라 「한 바퀴로 딱 떨어지나」로는 안 갈렸다 —
두 읽기가 서로를 함의한다. 가른 것은 **같은 재질의 두 축**이다: 애니 45
`lambert4`가 U축(그림 16텍셀)에 2,052를, V축(32텍셀)에 3,080을 쓴다. 텍셀 자로
읽으면 각각 **4바퀴**(4×16×32 = 2,048)와 **3바퀴**(3×32×32 = 3,072)로 맞고,
정규화로 읽으면 0.501·0.752바퀴다 — 480프레임짜리가 반 바퀴에서 끊길 리가 없다.

#### 언제 도는가 — 자료가 말해 준다

`MapPropAnimationManager_LoadPropAnimations`가 소품 모델을 올릴 때 애니마다
`loopCount = -1` · `looping = TRUE`로 세우고 첫 프레임으로 보낸다. 곧 **기본이
무한 반복**이고, 거기서 빠지는 것은 목차에 적힌 둘뿐이다:

그리고 배치를 세울 때 `MapPropManager`가 `AddAllAnimationsToRenderObj`로 **그
소품의 애니를 다 붙인다**(`map_prop.c` 105줄). 하나만 붙이는 길
(`AddAnimationToRenderObj(…, 0, …)`)은 스크립트가 세우는 소품 쪽이다.

| 조건 | 실측 | 원작이 하는 일 |
|---|---:|---|
| `flags & 1` (미룬 적재) | **34개** | `LoadPropAnimations`가 그 자리에서 `return` |
| `flags & 2` (미뤄 붙이기) | **3개** | `AddAllAnimations…`가 아무것도 안 붙인다 |
| `isBicycleSlope` | **2개** | `paused = TRUE` · `loopCount = 1` — 올라탈 때 한 번 |
| 나머지 | 75개 | 무한 반복 |

미룬 적재 서른넷이 문 스무 종 · 운하시티 다리 둘 · 노트북 셋 · 포켓몬센터
PC·치료기·에스컬레이터 넷 · 승강기 표시등 · 대습원 열차다. 미뤄 붙이기 셋은
꿀나무(26)와 자전거 비탈 둘인데, 꿀나무는 포켓몬이 붙어 있을 때만
`honey_tree.c`가 흔들 클립 **하나를 골라** 붙인다. 그래서 폭포·용암·물결·야도
체육관 물바닥이 아무 신호 없이 돌고 문과 꿀나무는 가만히 있는다.

⚠️ **야도 체육관 단추 셋(239·240·241)은 목차에 미룬 적재가 안 걸려 있는데도
저절로 돌면 안 된다** — 체육관이 그것들을 한 번짜리로 따로 세운다
(`PastoriaGym_UpdateButtonAnimations`가 눌린 단추는 정방향, 나머지 둘은
역방향으로 한 번씩 돌린다). 반복을 걸면 세 단추가 초당 네 번 깜빡인다.

#### 굽는 쪽이 싣는 것

`placeByNode`가 노드 변환을 **정점에 발라 버리므로**(그래야 안 움직이는 소품
590개가 메시 하나로 선다) 노드를 다시 움직이려면 기본 변환을 되돌려야 한다.
애니가 있는 **112개에 한해** 모델 속살을 같이 싣는다 (`data/props/anims.json`의
`models`, 31.3KB):

```
submeshNodes  조각(=`packChunk`의 `submeshes`) 차례마다 붙은 노드 번호
nodes         노드마다 기본 변환 m·s·t — 화면이 이걸 뒤집어 곱한다
materials     재질 이름 — BTA0·BTP0 트랙이 이름으로 재질을 고른다
uv            재질마다 텍셀 → 정규화 UV 배수 [magW/origW, magH/origH]
```

그리고 애니 자체는 **원작 바이트 그대로** 옮긴다 (`data/props/anims.bin`,
멤버 98벌 **70.5KB**) — 입자 묶음(§2.28)과 같은 길이라 굽는 쪽 둘이 저절로
같아지고, 읽는 것은 화면 쪽 `nsbca`·`nsbta`·`nsbtp` 하나뿐이다.

실측 — 애니 있는 소품 112개 중 노드가 여럿인 것 **21개** · 기본 변환이 단위가
아닌 노드를 가진 것 **26개**다. 문 스무 종은 대부분 노드 하나에 정점 4~40개고,
기본이 단위가 아닌 것은 둘뿐이다(대저택 441 · 백화점 442의 문짝이 ±10·±12유닛
밀려 있다). BTA0·BTP0를 쓰는 84개는 **전부 노드가 하나**라 기하를 쪼갤 일이
아예 없다.
...
⚠️ **잔디 위의 허연 판은 우리가 베낀 것이 아니다.** 한동안 그렇게 적어
두었는데 광선으로 되물으니 아니었다 — 그 픽셀에 맞는 것이 `s_snow` ·
`s_snow04` · `s_sonwp`이고 전부 **원작 지형**이다. 원작이 `ngrass`(y = 1.0)
위에 그 그림을 한 겹(y = 1.06) 얹어 놓았다. 같은 자리에서 우리 것(`메운 바닥`)이
맞는 픽셀은 `ngrass`다. 영원의숲의 널따란 크림색 판도 같다 — `지형|beachp`이고
텍셀이 (255,255,231)이다. 팔레트를 잘못 고른 것도 아니다: 그림 1,140가지 중
팔레트가 여럿인 이름이 28가지뿐이고 `beachp`·`s_snow`는 하나씩이다.
**눈으로 보고 "베낀 것이 틀렸다"고 적었던 것이고, 재 보니 우리 몫이 아니었다.**

허옇다고 다 원작인 것은 아니다. **분홍 바위 판(`criff`)과 어두운 절벽 판
(`criffp`)은 우리 것이었다** — 청크가 혼자 땅을 고르던 것이 임자고, §2.2에
고친 내력이 있다.
