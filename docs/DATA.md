...
기본 샷(5.68m·화각 30°)에서 화면을 넘겼다. BDSP도 큰 포켓몬일 때 리그를 바꾼다 —
`DefaultCameraPlacementData`의 두 번째 자리가 (−4.6, 0.8, 7.3)으로 무대 한가운데
에서 8.63m, **기본의 1.52배**다. 우리는 화면에 서는 키(중앙값 1.17)로 그 사이를
잇고 1.52배에서 멈춘다. 좁은 방에서는 **벽이 이긴다** — 물러날 데가 없으면
잘리는 편이 벽을 뚫는 것보다 낫다.
