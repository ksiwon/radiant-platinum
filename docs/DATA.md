...
**클립이 5.05MiB로 77%다.** 그런데 **걷기는 `actor/locomotion`이 뼈를 직접 돌려서
만든다**(주인공도 그렇다). 그래서 안 싣는다 — 대신 **서 있는 사람도
`updateLocomotion`을 돌려야** 한다. 안 돌리면 바인드 포즈, 즉 팔을 벌린 T 자세로
선다. 지금 116벌에 159.8MiB고, 한 맵이 쓰는 서로 다른 그림이 중앙값 셋이라 맵 하나에
3MB쯤이다.
