...
⚠️ **글자를 찍는 시간은 이제 0이다.** 글자 속도 항목을 없애고 쪽을 통째로
올리기 때문이다 (§2.11) — 위 표에서 남은 것은 **줄마다 머무는 시간과 게이지**고,
"배틀 진행"이 거기에만 곱한다. `playback.ts`가 내는 프레임 수는 원작 그대로라
설정을 **원작대로**로 되돌리면 첫 줄이 그대로 나온다.
