...
2. **머리를 줄인 뒤에 키를 재고, 거기에 2.09를 곱한 키로 세운다**
   ...
   (1.564)보다도 작았다.
3. **그 늘림을 굵기에는 1.4배로 되돌려 뺀다** (`CHIBI_SLIM`). 균등 배율이면
   굵기도 같이 커져 「다리가 굵다」는 보고가 온다.
4. **머리는 눌린 만큼 도로 편다.** 머리뼈의 로컬 축은 X가 위, Y가 좌우, Z가
   앞이라(실측) 가로 보정이 `scale.y`·`scale.z`로 간다.
5. **팔 마디를 `세로배율 ÷ 굵기배율`만큼 늘이고, 팔뼈 로컬 Y를 그만큼 누른다**
   (`ARM_ROOT`). ⚠️ **③이 가로로 뻗은 것을 짧게, 그리고 세로로 굵게 만든다** —
   다리는 세로라 길이가 키 늘림을 받고 단면(가로·앞뒤)이 굵기를 받는데, 팔은
   누워 있어 **길이가 굵기를, 단면의 세로가 키 늘림을** 받는다. 실측으로
   위팔관절~손이 키의 20.0%(등신 25.0~26.3%)였고 소매 세로가 13.2%(등신
   5.6~7.2%)였다.
   - **길이**는 배율이 아니라 **마디 위치**를 옮겨 되돌린다 — 배율로 하면
     팔꿈치가 굽었을 때 아래팔이 밀린다(전단). 손에서 멈춰 손가락은 안 늘인다.
     ⚠️ **어깨 관절 자리(`LArm.position`)는 안 건드린다** — 그건 몸통 값이라
     늘이면 어깨가 위로·옆으로 밀린다.
   - **단면**은 팔뼈 로컬 Y만 누른다. 쉬는 자세에서 팔뼈의 로컬 축이 월드와
     그대로 맞는다(실측: `LArm`의 배율이 축별로 0.994·2.09·0.994). 맨 위 뼈에만
     걸어야 한다 — 마디마다 걸면 사슬을 따라 제곱으로 눌린다.
...
| | 키 | 머리 높이 | 허벅지 폭 | 발 길이 | 팔 길이 | 소매 세로 |
|---|---:|---:|---:|---:|---:|---:|
| 광휘 (등신) | 1.564 | 21.4% | 11.3% | 18.1% | 25.0% | 5.6% |
| 리오 (등신) | 1.749 | 16.0% | 8.2% | 14.0% | 26.3% | 6.1% |
| 신사 (등신) | 1.788 | 20.7% | 9.3% | 16.2% | — | 7.2% |
| 핸섬 (치비) | 1.745 | 15.2% | 10.4% | 17.3% | 24.5% | 7.7% |
| 엄마 (치비) | 1.785 | 22.5% | 7.8% | 15.8% | 24.0% | — |
| 마박사 (치비) | 1.781 | 21.2% | 9.4% | 14.9% | 24.0% | 7.8% |

다섯 자리 모두 등신 셋이 만드는 폭 안에 든다. **팔 길이는 위팔 관절에서
손까지**로 잰다 — 어깨뼈에서 재면 어깨 너비가 섞여 들고, 그 너비는 굵기
되돌림이 일부러 좁힌 값이다 (치비 3.5% ↔ 등신 5.0%).
