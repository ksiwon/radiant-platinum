...
- **그래서 가로를 아예 없앤다** (`wallStrip`). 옆벽이 그 높이에서 제일 많이
  ...
  줄무늬는 애초에 원작에 없던 것이다. 값으로 드로우콜도 준다 — 벽이 방향을 넘어
  재질 하나로 모인다.
...
##### ⚠️ 모부기 계열 셋은 몸 색이 조각보로 나온다 — 아직 못 고쳤다

**증상.** 387·388·389의 몸이 큰 색 조각으로 갈린다 — 등딱지가 연둣빛이고
머리가 초록·갈색·자줏빛 쐐기로 나뉘며, 잎 둘 중 하나만 초록이다. 뒤에서 보면
정확히 **한가운데를 세로로** 갈라 좌우가 서로 다르게 칠해져 있다.

**범위.** 49종을 중립 조명에서 홀로 세워 견줬고 **모부기 계열 셋만** 그렇다
(1·4·7·25·133·390·393·396·399·403·448·483·487과 스물여섯 종이 다 멀쩡하다).

**아닌 것으로 확인된 것들** — 여기까지가 지금까지 잰 값이다:

| 의심 | 재 본 방법 | 결과 |
|---|---|---|
| 뼈대·스킨 | 바인드 자세로 홀로 세워 실루엣 확인 | **멀쩡하다.** 모양이 정확히 모부기다 |
| 크기 | 바인드 높이 0.787 = 0.40×1.98 | 맞다 |
| 그림을 잘못 골랐나 | 재질 일곱의 `_Col0Tex`를 전부 덤프 | 몸 다섯이 다 `Body_col` 하나다 (정상) |
| 그림이 깨졌나 | 번들에서 512×512 원본을 직접 뽑아 대조 | 우리 256과 배치가 같다 |
| 그림 형식 | `Body_col` 형식 50 · 512×512 · 밉 10 | 멀쩡한 종들과 **완전히 같다** |
| 재질별 UV 배율/오프셋 | 일곱 재질의 `_Col0Tex_ST` 비교 | 전부 scale (2,1) offset (0,0) — 차이 없음 |
| UV가 깨졌나 | 체커 텍스처를 입혀 렌더 | **깨끗하다.** 섬이 매끈하고 텍셀 밀도도 고르다 |
| UV 전체 변환 | 그대로·u×2·u÷2·u+0.5·v뒤집기·flipY 등 여덟 벌 | **어느 것도 안 맞는다** |
| 면 방향 | 앞면·뒷면·양면 | 관계없다 |
| 서브메시 경계 | 서브메시별 단색 렌더 | 조각보 경계가 **서브메시 경계를 가로지른다** |

정점 위치와 UV는 같은 파서가 같은 색인으로 읽는데 위치는 정확하다 — 그러니
UV 배열 자체가 깨진 것은 아니다. 그런데 UV 섬이 그림의 그 부분에 안 앉는다.
남은 갈래는 "이 UV가 이 그림용이 아니다"인데, 번들에 `Body_col`이 하나뿐이고
재질 일곱이 다 그것을 가리킨다.

⚠️ **한 가지 눈에 띄는 차이**: 멀쩡한 종은 UV 채널이 둘(`TexCoord0`·`TexCoord1`)인데
**387은 `TexCoord0` 하나뿐이다.** 다만 388·389는 둘을 다 갖고도 깨지므로 이것만으로는
설명이 안 된다. 다음에 볼 자리는 여기다.
