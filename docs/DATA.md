...
| 예진호수 | 84 | **68그루가 풀이 아니다**(`nectgr` 39 · `criffp` 29) |
...
| 자리 | 갈린 꼴 |
|---|---|
| 예진호수 (0,0)·(1,0) | 제 바닥이 절벽뿐이라 **분홍 바위 `criff`로 792칸** — 바로 옆 (0,1)은 풀 `ngrass` 616칸 |
| 영원의숲 (0,1) | `criffp`(어두운 절벽) 983칸 — 나머지 여덟 청크는 풀 |
| 눈꽃마을 (9,5)·(9,6)·(10,6) | `criff` — 나머지는 `ngrass`. 둘 다 흰 눈인데 경계 130자리가 드러난다 |
...
쓰는 그림이 제일 초록이라 자주 이기는데(예진호수에서 `nectgr`이 `ngrass`를
...
예진호수 (0,0)에는 바닥이 있긴 있었다 — 절벽뿐이었을 뿐이다. 그래서 빌려 오는
...
영원의숲 위단(y=2.813)과 예진호수 물가 비탈(y 1 → 0.5 → −0.5 → −1.5)이 그렇다.
