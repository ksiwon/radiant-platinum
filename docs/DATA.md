...
| 소리 | `data/sound/pl_sound_data.sdat` | ✅ 곡 1013 · 악기 뱅크 521 · 파형 874 전부 풀림 (§2.18). 아직 안 울린다 |
...
| 소품 | +X | −X | +Y | −Y | +Z | −Z |
|---|---|---|---|---|---|---|
| 22 | 43면 8.7 | 41면 8.5 | 24면 26.1 | **0** | 50면 18.7 | **0** |
| 23 · 236 | 49면 29.9 | 40면 28.6 | 31면 27.5 | **0** | 99면 29.8 | **0** |

여섯 방향에서 렌더해 보면 **뒤가 완전히 비어 있다.** 구멍이 아니라 뒷면 자체가
없다 — 원작 카메라가 고정 각도라 만들 이유가 없었다. 뒤로 돌아가면 앞벽의
**안쪽**이 보인다. 경계 모서리도 고리 25개로 흩어져 있고 그중 평면인 것은
바닥 윤곽 하나뿐이라, "구멍을 덮는다"는 접근 자체가 성립하지 않는다.

⚠️ **면적 벡터로는 이걸 못 잰다.** 한 번 그렇게 접근했다가 접었다. 경계 고리를
부채꼴로 덮으면 면적 벡터는 130종 중 75종에서 0에 수렴하는데, **그 수치가
속는다** — 문틀·창틀 같은 안쪽 테두리를 덮고 정작 뚫린 뒤는 그대로 두고도 합이
맞는다. 렌더해 보고서야 알았다.

그래서 **고리를 고르지 않는다**(`scene/shell.ts`). 삼각형을 전부 뒤판(z=minZ)으로
눌러 붙인다 — 눌러 붙인 것들의 합집합이 곧 뒤에서 본 실루엣이라, 어느 고리가
진짜 구멍인지 고를 일이 없어진다.

- **두께 0.02타일 안에 담고 z 순서를 뒤집는다.** 한 평면에 완전히 눕히면 겹친
  삼각형끼리 깊이가 같아 깜빡인다. 뒤집어 담으면 뒤에서 볼 때 원작의 앞면이 제일
  앞에 온다 — 박공은 지붕이 아니라 벽으로 보여야 맞다. 거울로 뒤집거나 통째로
  밀어 붙이는 방법은 집이 두 배로 깊어져 뒷길을 막는다.
- **−Z를 보게 다시 감는다.** 원작 감는 방향을 물려받으면 면은 다 있는데 뒤에서
  하나도 안 보인다 — 실제로 처음에 그렇게 나왔다(덮은 칸 0/1631).
- **모로 선 면은 버린다.** 옆벽은 뒤에서 보면 선이라 실루엣에 아무것도 안 보탠다.
  주인공 집은 219개 중 109개가 그렇고, 뒤판은 110삼각형이다.
...
- **그늘로 따로 깎지 않는다.** −Z를 보는 면은 태양(24, 42, 18)도 필(−14, 12, 26)도
  못 받아 반구광만 닿는다. 이미 그늘이라 여기서 또 깎으면 이중이 된다.
- **오려 낸 그림은 뒤판을 안 만든다.** 판 한 장짜리 울타리·간판에 없던 널판이 생긴다.

재는 것은 넓이가 아니라 **뒤에서 본 실루엣이 다 막혔는가**다. 앞에서 보이는 칸을
뒤에서도 무언가가 막는지 격자로 래스터라이즈한다 — 문틀을 덮는 것으로는 못 속는
잣대다. **소품 590개 전수에서 뚫린 칸 0.** 뒤판이 안 나오는 98개는 전부 한 장짜리로,
깊이가 0(간판·그림자)이거나 뒤에서 봐서 넓이가 0(옆을 보고 선 울타리)이다.

바닥(−Y)은 안 만든다. 건물 밑은 볼 자리가 없다.
...
역공학은 끝났다. 남은 것은 **연주기를 만드는 일**이다 — 틱마다 악보를 밟는 VM,
표본을 재생 속도로 음정 맞추기, ADSR 포락선, PSG 두 종류. Web Audio의
`AudioBuffer` + `playbackRate` + `GainNode`로 갈 수 있다.
