...
아예 없다. 우리는 롬 두 개에서 뽑은 글을 같이 싣고 있으니 바꿀 수 있다 —
번역이 아니라 **그 나라 롬에 찍힌 글**이다.
...
일본어는 안 싣는다. 이름표(`names/*.ja.json`)는 세 언어가 다 있지만 대사 뱅크가
없어서 고를 수 있게 두면 스크립트 글이 통째로 빈다. 한국어 롬의 헤더 표는
찾았지만 일본어는 아직 안 찾았다. `LANGUAGES`가 실린 로케일 목록
(`dialogue/index.json`)을 벗어나면 시험이 걸린다.
