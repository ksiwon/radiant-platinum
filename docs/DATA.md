...
##### 나무 바닥 타일은 나무가 들고 있어야 한다

그 평평한 판이 곧 **그 자리의 바닥**이다. 원작 숲에는 땅이 없어서 그동안 옆
타일을 베껴 메웠는데, 베끼기가 물가에서 웅덩이를, 벼랑 밑에서 벼랑 그림을,
축복시티 가로수 밑에서 **횡단보도**를, 떡잎마을 숲에서 **하얀 모래**를 깔았다
(메운 칸 101,082개의 출처 상위: `ngrass` 31,887 · `puddlep` 8,121 ·
`nectgr` 7,848 · `criffp` 7,546 · `nhana` 7,294 · `sea` 6,820).

⚠️ **청크 지오메트리에 그려 넣으면 안 된다.** 세 번 다르게 해 보고 다 틀렸다:

1. 평평한 판을 그대로 되살린다 → 3인칭 카메라가 5.5타일 안의 나무를 지우는데
   (`HIDE_RADIUS`) 판은 지형이라 남는다. 발밑에 **줄기 없는 그루터기**가 격자로 깔린다.
2. 칸마다 한 장씩 접어 깐다 → 16타일짜리 숲 지붕(`conttree_b`)은 무게중심이 한
   칸에 있어서, 칸마다 그 한 칸을 복사하면 우듬지 윤곽선 조각이 깔린다. 영원의
   숲 바닥이 **흰 초승달과 검은 고리**로 뒤덮였다.
3. 걸어 다니는 칸만 뺀다 → 지우는 것은 걸을 수 있느냐가 아니라 카메라와의
   거리라 아무것도 안 달라진다.

**나무의 인스턴스로 만들면 나무가 사라질 때 같이 사라진다.** 규격이 종류마다
하나로 떨어지므로(위의 2.0×1.88) 한 벌만 만들면 되고, 그림 칸도 그 종류의 것을
그대로 쓴다. 한 그루짜리 판만 쓴다 — 숲 지붕은 그루마다 한 벌씩 붙일 수가 없다.
판은 **안 돌린다**: 위에서 내려다본 그림이라 세계 격자에 붙어 있어야 하고,
그루마다 아무렇게나 돌리면 그루터기가 줄기 밖으로 돌아 나간다.
...
- **그림은 옆벽이 쓰는 칸을 통째로 늘려 쓴다.** 눌러 붙인 삼각형이 제 UV를 들고
  가면 앞벽의 문과 창이 뒤에 찍힌다 — 뒷벽 자리에 있어야 할 그림은 옆벽이 갖고
  있다. 옆벽의 UV를 **아핀 함수로 읽어 그 자리 좌표를 넣는 방법은 두 번 틀렸다**:
  법선 방향 기울기를 0으로 두면 판의 가로가 통째로 한 값이 되고(원작 벽 그림은
  널판이 세로줄이라 변화가 죄다 가로에 있다 — 주인공 집 뒷벽이 텍셀 하나짜리
  통짜 갈색이 됐다), 가로를 옆벽의 길이 방향으로 바꿔 넣으면 값이 그림 칸을
  벗어난다(`t1_h01` 64×64 안에 지붕·창·널판·기단이 다 들어 있어서, 조금만
  넘쳐도 벽 자리에 지붕이 찍힌다). **칸 안에서만 셈하면** 벗어날 수가 없다.
...
- **물리는 그림이면 늘리지 않고 되풀이한다.** 칸을 판 가로에 한 번 늘려 붙이면
  널판 한 벌이 뒷벽 폭에 맞춰 늘어나서 옆벽의 널판보다 굵어지고, 칸에 같이 든
  기단 한 줄이 벽 폭만큼 넓은 띠가 되어 파랗게 번진다. 옆벽에서 잰 눈금
  (u/타일)으로 되풀이하면 널판이 네 벽에서 같은 굵기가 되고 그 한 줄은 **일정한
  간격의 모서리 기둥**으로 읽힌다. 눈금은 옆벽의 길이 방향에서 재고 네 벽이
  같다고 본다 — 그 하나가 여기 넣은 가정이다. 안 물리는 그림은 되풀이할 수가
  없어서(가장자리 텍셀로 눌린다) 그때만 늘려 붙인다.
...
| 명령 | 자리 | 막는 것 |
|---|---|---|
| `GivePokedex` | **1** | 잔모래마을에서 도감을 못 받는다 — **그 뒤 이야기 전부** |
| `OpenPokemonNamingScreen` | 5 | 별명 짓기. 도감 다음 대목이 여기다 |
...
⚠️ **물은 메울 바닥이 못 된다.** 물가에서는 제일 가까운 바닥 삼각형이 물이다 —
메운 칸 101,082개 중 **6,167개가 웅덩이**(`puddlep` 4,583 · `puddle_b` 1,584)에서,
1,208개가 바다에서 왔다. 바닥 넓이의 2~4%짜리 장식이 숲 안쪽까지 번져서, 떡잎마을
숲을 위에서 보면 나무 사이사이에 **파란 마름모**가 떠 있었다. 이름으로 거른다.
거르고 나서 바닥이 하나도 안 남는 청크(물뿐인 곳)에서는 그대로 쓴다 — 발밑이
뚫리는 것보다는 낫다.
...
⚠️ **자료가 아니라 화면이 남은 것도 하나 적어 둔다.** 숲 벽(`conttree`) 밑바닥은
아직 옆 타일을 베낀다 — 한 그루짜리 판은 제 바닥 그림을 들고 서지만(§2.2) 16타일을
한 장으로 덮은 지붕은 그루마다 한 벌씩 붙일 수가 없어서다. 떡잎마을 바닷가 숲
바닥이 하얀 모래인 것이 그것이다.
