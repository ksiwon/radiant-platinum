...
⚠️ **속이 빈 그림으로는 옆면을 못 세운다.** 옆면이 베낄 삼각형은 「제일 가까운
바닥」이라 깔 땅과 달리 한 그림으로 안 묶는데, 그러다 위의 얼음 덧그림
(`c09_ice` 55% · `c09_ice2` 48%)을 집었다 — 세워 놓으면 반이 비쳐 턱을 메우기는
커녕 창문이 된다. 그래서 옆면 후보도 **등급 0을 뺀다**(물·풀숲·속 빈 그림).
