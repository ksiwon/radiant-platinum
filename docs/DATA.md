...
⚠️ **치비로 서는 사람은 머리를 줄여서 세운다.** BDSP는 필드에 세울 사람을
**머리 큰 치비**로 따로 만들어 두었다 — 엄마(`fc2005_00`)는 머리가 키의
60%다. 우리는 주인공과 트레이너를 배틀용 등신 모델로 세우므로 나란히 서면
계통이 어긋난다(실제로 「엄마 얼굴이 이상하다」는 보고를 받았다). 배틀용이
아예 없는 사람이 굽은 106벌 중 열넷이고 대부분 이야기 인물이라, 판때기로
되돌리면 오프닝 바로 다음 장면의 엄마가 종잇장이 된다.

그래서 `scene/NpcModels`가 `fc`로 시작하는 번들만 **`Head` 뼈를 0.4로 줄이고
키는 그대로 둔다**(줄인 만큼 정규화가 몸을 키운다 — 머리는 목 관절을 원점으로
줄어들어 이음매가 안 벌어진다). 실측:

| 머리뼈 | 머리가 키에서 차지하는 몫 | 몸 배율 |
|---|---|---|
| ×1 | 60% | 1.019 |
| ×0.62 | 48% | 1.270 |
| **×0.4** | **37%** | 1.481 |

**등신이 되지는 않는다** — 몸통과 팔다리도 필드용으로 짧게 만든 것이라 사람
비율(13%)에는 못 간다. 더 줄이면 짧은 팔다리 위에 작은 머리가 얹혀 오히려
이상해진다.
