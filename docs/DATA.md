...
동안 게이지가 기다린다. 우리도 그 자리에 40프레임을 내주고(`vfx.ts`의
`MOVE_FRAMES`), 기술 이름을 **띄운 채로** 연출이 돈 뒤에 게이지가 움직인다.

⚠️ **박자와 무대가 같은 상수를 봐야 한다.** `BattleStage`의 `LUNGE`와 `MoveVfx`의
`DURATION`이 둘 다 `MOVE_FRAMES / 60`초다 — 어긋나면 연출이 끝나기도 전에 다음
글이 뜨거나, 다 끝나고도 화면이 멈춰 있다.
