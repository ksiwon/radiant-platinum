...
쓰는 눈금은 **필드 스크립트에 명령이 놓인 자리**다.

세는 방법이 중요하다. 바이트코드를 걷지 않고 **디컴프의 어셈블러 원본을** 센다
(`res/field/scripts/*.s`). 매크로를 `.short SCRCMD_*`가 나올 때까지 펼치고 그
상수를 `scrcmd.h`의 줄 순서로 우리 표 이름에 맞춘다. 바이트코드를 훑으면 자료
구간을 명령으로 잘못 읽는 일이 생기는데, 원본에는 라벨이 붙어 있어서 그럴 여지가
없다. 표가 한 칸도 안 어긋난다는 것은 840개 이름을 통째로 대조해서 확인한다.

| | 자리 |
|---|---|
| 스크립트 원본에 놓인 명령 자리 | 65,466 |
| 그중 도는 것 | **63,140 (96.4%)** |
| 안 도는 것 | 2,326 (서로 다른 명령 558종) |

남은 자리는 거의 전부 **우리에게 그 계통이 없는** 것들이다: 배틀타워
(`CallBattleTowerFunction` 121) · 모험노트(`CreateJournalEvent` 49) · 포켓치
(43) · 통신(`DoGroupConnectionAction` 등 129) · 지하통로 · 게임 기록.
...
⚠️ **그림은 원작과 다르다.** 원작 장면은 `NARC_INDEX_GRAPHIC__EV_POKESELECT`의
3D 모델과 NSBCA 애니메이션인데 우리는 아직 그 계통을 안 뽑았다. 대신 롬에서 이미
뽑아 둔 것만 쓴다 — 서류가방은 오버월드 스프라이트 174번(`BRIEFCASE`), 볼은 도구
아이콘 4번, 미리보기는 포켓몬 앞모습이다. **순서·소리·글·결과는 원작 그대로**고
없는 것은 움직이는 3D 장면 하나다.
...
떡잎마을 · 그 집 셋 · 201번도로 · 예진호수와 그 집 · 마사고마을. 이 여덟 맵이
읽는 스크립트 파일에서 아직 못 만든 명령이 남아 있다. 큰 것이 셋이다:

| 묶음 | 무엇이 없나 |
|---|---|
| 파트너 선택 | `StartChooseStarterScene` · `SaveChosenStarter` — 스크립트가 아니라 따로 도는 프로그램이라(`src/choose_starter/choose_starter_app.c`) 바이트코드가 없고 상태 기계를 옮겨야 한다 |
| 문 여닫는 그림 | `LoadDoorAnimation` · `PlayDoorOpen/CloseAnimation` · `UnloadAnimation` — 문 애니메이션 계통 자체가 없다 |
| 첫 배틀 | `StartFirstBattle` — 지는 것이 정해진 배틀이다 |

나머지는 낱개다: `GiveRunningShoes`(우리는 처음부터 뛴다) · `GiveJournal`(모험노트
계통이 없다) · `SetWarpEventPos` · `GetTimeOfDay` · 파트너 이름 채우기 둘.

**파트너 이름 채우기 셋은 만들었다.** 라이벌과 반대 성별 주인공의 파트너는
저장하는 값이 아니라 내 것에서 계산하는 값이라(`SystemVars_GetRivalStarter`)
고를 화면이 없어도 규칙은 옮길 수 있었다. 처음 고른 파트너 자체도 세이브에 새 칸을
안 만들었다 — 원작이 스크립트 변수 하나(`VAR_PLAYER_STARTER` = 0x4030)에 넣어 둔다.
