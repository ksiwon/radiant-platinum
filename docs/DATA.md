...
#### 얼마나 도는가 — 도달 가능한 자리의 95.4%

"몇 개를 만들었나"(144/840)는 눈금이 못 된다. 안 쓰이는 명령이 태반이라서다.
쓰는 눈금은 **진입점에서 제어 흐름을 따라가 닿는 명령 자리**다 — 분기 양쪽을 다
따라가므로 메뉴에 답을 안 해도 세어진다.

| | 자리 |
|---|---|
| 코드 파일 575개에서 도달하는 명령 자리 | 55,421 |
| 그중 도는 것 | **52,894 (95.4%)** |
| 안 도는 것 | 2,527 (서로 다른 명령 583종) |

⚠️ 예전 계산(선형 스캔)은 **틀렸다.** 진입점 표를 지나 파일을 처음부터 훑었더니
자료 구간을 명령으로 읽어서 337번째에서 멎었다. 지금은 진입점에서 시작해 분기
대상을 큐에 넣고 따라간다.

남은 2,527자리는 거의 전부 **우리에게 그 계통이 없는** 것들이다: 배틀타워
(`CallBattleTowerFunction` 121) · 모험노트(`CreateJournalEvent` 47) · 포켓치
(43) · 문 여닫는 그림(`LoadDoorAnimation` 등 89) · 통신(53) · 지하통로.
...
싣는 뱅크 447개 중 **`game_corner`(us#147) 하나가 일본 롬에 없다.** 열쇠 표와
...
사람 사이 크기 차이가 원작 그대로 남는다(`sceneRefs.playerScale`).
...
`g001` 하나에 정적 메시 **158개** · 재질 6벌(흙 · 풀 · 바위 · 나무 둘 · 산)이
들어 있다. `tools/extract/bdspArena.py`가 월드 행렬을 정점에 굽고 재질별로
합쳐 glb 하나로 낸다 — 메시 158개를 그대로 두면 드로우콜이 158개다.

| | 값 |
|---|---|
| 정점 · 삼각형 | 141,918 · 129,589 |
| 드로우콜 | **6** |
| 크기 | 112 × 13 × 104 m |
| 땅 | y = 0.001에 평평, 44 × 45 m |
| 파일 | 6.9 MB (배틀이 열릴 때 받는다) |

**우리 좌표와 1:1로 맞는다.** 두 포켓몬 자리(`SLOT` — p1 (−2.4, 1.6) ·
p2 (2.6, −3.2))에 땅이 정확히 한 겹씩 있고 높이가 0.001이다(`arena.test`가
glb를 직접 열어 잰다). 무대가 x −19~25 · z −18~27로 **비대칭**이라, X 뒤집기를
빠뜨리거나 두 번 하면 그 시험에서 걸린다.

하늘은 안 가져온다. 우리 하늘은 시간대를 타는데(§2.19) 무대 하늘은 한 벌이라,
해질녘에 걸어 들어왔는데 배틀만 대낮이 된다. 무대 색도 시간대의 지면색을 곱해
톤을 맞춘다.

⚠️ 아직 **무대가 하나뿐이다.** 어느 지역에서 싸우든 `g001`(풀밭)이 선다 —
지역별 배정은 BDSP 쪽 표를 더 읽어야 한다.
...
| `dialogue/ko/*.json` (뱅크 449개, 지연 로딩) | 999KB | 199KB |
...
|---|---|
| 잎이 덮은 칸 | 110,703 |
| 원작 지형이 이미 덮은 칸 | 12,463 |
| 메운 칸 | **98,240** |
| 그중 이웃에서 빌려 온 것 | 51,546 (52.5%) |
| 남은 빈 칸 | **0** |
...
늘었다. 그래서 **서 있는 판에만** 붙인다(법선의 y 성분 0.20 미만, `standCutouts`가
"이미 서 있다"고 보는 자리와 같은 문턱).

| | 삼각형 |
|---|---|
| 옆면 전체 (청크×묶음 조합 합) | 387,692 |
...
496칸에 0번 221 · 1번 117 · 2번 155가 쓰이고, 짐작으로 0번을 쓰면 풀색 포켓몬이
...
모듈레이션·포르타멘토·스윕이다. 무엇이 안 붙었는지는 PLAN.md §16에 실측으로
세어 뒀다.
