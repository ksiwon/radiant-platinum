...
| **오버월드 NPC 스프라이트** | `data/mmodel/mmodel.narc` | ✅ 470칸 순서 확정 — 텍스처 이름 396/396 일치 (§2.16). 게임에는 아직 안 붙였다 |
...
그림(§2.16), 배틀 그림(§2.17), 소리(§2.18), 그리고 아래의 스크립트 명령.
...
**맵 오브젝트 아홉은 §2.16이 먼저다.** `AddObject`가 캡슐을 하나 더 세우는 것으로
끝나면 누가 걸어 들어왔는지 화면에서 안 보인다.
