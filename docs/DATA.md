...
**AI 플래그는 아홉 가지 값뿐이다** (하위 6비트만 쓴다):

| 값 | 인원 | 뜻 |
|---|---|---|
| `0x01` | 580 | 나쁜 수 거르기 |
| `0x07` | 226 | + 쓰러뜨리기 노리기 + 유효성 판단 |
| `0x05` | 57 | + 유효성 판단 |
| `0x21` | 47 | + 강한 기술 선호 |
| `0x23` | 6 | + 쓰러뜨리기 노리기 |
| `0x0f` | 6 | `0x07` + 첫 턴 셋업 |
...
**여섯 가지 거동만 구현하면 게임 전체가 덮인다.** 같은 분류라도 트레이너마다 플래그가 다르므로(101종 중 25종이 여러 값을 쓴다) 분류가 아니라 트레이너 단위로 들고 다녀야 한다.
