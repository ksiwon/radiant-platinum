...
이렇게 이어서 배치 3,555개 중 **1,879개(52.9%)**가 모델로 선다. 남은 1,676개
중 사람은 **열여섯뿐**이다 — 물건이 1,118개(`ROCK_SMASH` 591 · `POKEBALL` 331 ·
환풍구 65 · 괴력바위 50 · 베어야 할 나무 49), 포켓몬이 115개(실물 3D로 따로
선다), 자리표시자가 427개(`OBJ_EVENT_GFX_VAR_*` — 대개 주인공 둘이라 이미
모델이다)다. **사람 그림만 보면 1,879 / 1,895 = 99.2%다.**

⚠️ **남은 열여섯은 핸섬(`icpo`) 12와 플루토(`gkanbu4`) 4고, 둘 다 BDSP에 몸이
아예 없다.** 플래티넘에만 나오는 사람이라 DP 리메이크에는 애초에 없다 — 필드
161벌·배틀 124벌을 다 세었고 이름표가 안 붙은 셋까지 열어 봤다
(`.audit/lookerCharonCands.mjs`).
...
이상하다」는 보고를 받았다). 배틀용이 아예 없는 사람이 굽은 114벌 중 스물다섯이고
대부분 이야기 인물이라, 판때기로 되돌리면 오프닝 바로 다음 장면의 엄마가
종잇장이 된다.

그래서 `engine/model/chibi`가 `fc`로 시작하는 번들만 이렇게 세운다. 굽힌 114벌 중
치비는 **스물다섯**이고 전부 `Head`·`Neck`·`LHand`·`RHand` 뼈를 갖고 있어 하나도
안 빠진다:
...
선다. 지금 114벌에 146.1MiB고, 한 맵이 쓰는 서로 다른 그림이 중앙값 셋이라 맵 하나에
3MB쯤이다.
