...
둘 이상이면 안 적는다: `dsboy`는 `child`·`child2` 중 어느 쪽인지 몰라 비어 있고,
`middlewoman2`는 `women2`·`women3`·`madam` 사이에서 비어 있다. 그리고 BDSP의
점원이 **여자 하나뿐**이라(화면으로 확인했다) `shopm1`에는 붙일 몸이 없다.

이렇게 이어서 배치 3,555개 중 **1,761개(49.5%)**가 모델로 선다. 남은 1,794개
중 사람은 **103개뿐**이다 — 물건이 1,121개(`ROCK_SMASH` 591 · `POKEBALL` 331 ·
환풍구 65 · 괴력바위 50 · 베어야 할 나무 49), 포켓몬이 115개(실물 3D로 따로
선다), 자리표시자가 427개(`OBJ_EVENT_GFX_VAR_*` — 대개 주인공 둘이라 이미
모델이다)다. **사람 그림만 보면 1,761 / 1,864 = 94.5%다.**

남은 103개는 BDSP에 그 사람이 아예 없거나(헨들러·배송원·체육관 안내원·샤론·
프론티어 안내원 셋) 어느 번들인지 못 짚은 것(마박사·오박사)이고, 열여덟은
**한 그림에 넷·셋이 함께 그려진 갤럭시단**이라 사람 하나가 아니다.
...
이상하다」는 보고를 받았다). 배틀용이 아예 없는 사람이 굽은 106벌 중 열넷이고
대부분 이야기 인물이라, 판때기로 되돌리면 오프닝 바로 다음 장면의 엄마가
종잇장이 된다.

그래서 `engine/model/chibi`가 `fc`로 시작하는 번들만 이렇게 세운다. 굽힌 106벌 중
치비는 **열일곱**이고 전부 `Head`·`Neck`·`LHand`·`RHand` 뼈를 갖고 있어 하나도
안 빠진다:

1. **`Head` 뼈를 0.2로 줄인다.** 키는 그대로 두고 줄인 만큼 정규화가 몸을
   키운다 — 머리는 목 관절을 원점으로 줄어들어 이음매가 안 벌어진다.
2. **그 늘림을 굵기에는 안 준다** (`CHIBI_SLIM`). 균등 배율이면 굵기도 1.7배가
   되어 「몸이 주인공보다 굵다」는 보고가 온다. 세로만 늘이고 가로·세로 두께는
   BDSP 원본 배율로 되돌린다.
3. **머리는 눌린 만큼 도로 편다.** 머리뼈의 로컬 축은 X가 위, Y가 좌우, Z가
   앞이라(실측) 가로 보정이 `scale.y`·`scale.z`로 간다.
4. **`LHand`·`RHand` 뼈를 0.5로 줄인다** (`CHIBI_HAND`). 손가락은 자식이라
...
실측(엄마, 키 1.44m · 화면에서 잰 값):

| 머리뼈 · 굵기 | 머리가 키에서 차지하는 몫 | 세로 배율 | 굵기 배율 | 화면 폭 |
|---|---|---|---|---|
| ×0.4 · 균등 | 35% | 1.481 | 1.481 | 0.775 |
| ×0.2 · 균등 | 24% | 1.745 | 1.745 | 0.913 |
| **×0.2 · 날렵** | **24%** | 1.745 | **1.019** | **0.533** |

주인공이 25%라 머리 비중은 맞는다. **상수 하나로 열일곱 벌을 다 덮는다** — 치비의
머리가 원래 키의 52.2~60.7%로 한 무리라(가장 작은 `fc2030_00` 52.2% · 엄마 55.6% ·
가장 큰 `fc2038_00` 60.7%), 열일곱을 다 세워 재니 **머리 22.0~27.9%**로 들어온다.
굵기 배율은 열일곱이 전부 정확히 **1.019**(=`BDSP_TO_WORLD`)고, 키는 1.10m
(`fc2025_00`)에서 1.62m(`fc2038_00`)까지 BDSP가 준 차이가 그대로 남는다. 손 상자도
열다섯이 `0.074 · 0.166~0.193 · 0.109~0.125`로 같은 무리고, 벗어나는 둘은 손에
물건을 든 사람이다(위 ④).

⚠️ **날렵하게 만들면 손이 커진다.** ②의 세로 ×1.745가 **아래로 뻗은 것을 다
...
선다. 106벌에 87.72MiB고, 한 맵이 쓰는 서로 다른 그림이 중앙값 셋이라 맵 하나에
3MB쯤이다.
