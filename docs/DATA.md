...
#### 평평한 잎 판은 잎이 아니라 숲 바닥이다

⚠️ **이걸 걷어내서 나무 밑이 통째로 뚫려 있었다.** 원작은 숲 바닥을 잎과 **같은
그림·같은 서브메시**로 깔았다. 가르는 잣대는 기울기다:

| 잎 삼각형 | 개수 | 지면(BDHC)과의 높이 차 |
|---|---|---|
| 완전히 평평 | 10,580 | **p05·중앙값·p95 전부 0.06타일(1도트)** |
| 기울어짐(35°·수직) | 34,592 | — |

표본 89,178개에서 흩어짐이 **0**이다 — 우연이 아니라 규칙이고, 지면 1도트 위에
깐 바닥 판이다. 잎이 덮은 칸의 69%를 이 판만 덮고 있어서, 빼고 나면 그 자리에
아무것도 안 남는다.

그래도 15.6%(10,095칸)는 안 덮인다 — 서 있는 잎 판이 제 바닥 판보다 옆으로 더
나가 있어서다. 덮인 칸을 실제로 세어 보고(원작 바닥 판이든 지형 삼각형이든)
**남는 칸에만** 판을 깐다. 높이는 BDHC가, 색은 그 칸을 덮던 원작 잎 그림의
제일 어두운 잎 색이 준다 — 숲 바닥은 잎에 덮여 그늘진 자리다.
