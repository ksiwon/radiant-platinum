...
#### 채운 면은 건물 모양을 따라가야 한다

빠진 면을 채울 때 삼각형을 **바운딩 박스의 끝면**에 눌러 붙이면, 처마가 벽보다
튀어나온 만큼 벽 자리의 판이 허공에 뜬다 — 건물 옆에 판때기가 따로 서 있는 것으로
보인다. 소품 590개의 실루엣 칸 132만 개를 재면 **51.4%가 그 자리의 진짜 바깥면에서
0.25타일(4도트) 넘게** 떨어져 있었고 p90이 1.93타일 · 최대 36타일이었다.

⚠️ **격자로 재도 안 된다.** 64칸 깊이 지도로 고쳐 봤더니 여전히 10.8%가 떴다.
칸 안에 튀어나온 면과 들어간 면이 같이 들면 칸 값이 튀어나온 쪽이 되어
**바깥면이 반 칸만큼 부푼다.** 꼭짓점 하나하나에 대고 그 자리의 바깥면을 정확히
찾아야 0이 된다 — 지금은 판 꼭짓점 89,991개 중 바깥으로 뜬 것이 **0개**다.
...
플레이어가 층을 가르는 규칙과 같은 것이다. 높이 자료가 없는 칸이 2,931개(6%)고
거기서만 잎 아래끝으로 물러선다.
