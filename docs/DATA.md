...
⚠️ **468 뒤의 번호 둘은 롬에서 온 것이 아니다.** 연결의 끈과 고운 비늘은
플래티넘에 없는 물건이라 이름·설명·아이콘이 롬에 없고, 우리가 붙인다
(PARITY §12가 왜 더하는지를 적는다). **롬 번호를 덮어쓰지 않고 표 뒤에 잇는다** —
`items.json`에서 그 둘은 `rom: false`로 갈라 두어, 「롬에서 뽑은 것」과 섞이지
않게 한다. 아직 안 만들었다.
