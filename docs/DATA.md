...
엉뚱한 그림이 발린다 — 대습초원 전망대는 위가 콘크리트인데 옆면이 습지 풀이었고,
포켓몬리그 로비의 9타일짜리 벽에는 포켓몬센터 바닥(`pc_room1`)과 프렌들리숍
바닥(`fs_room1`)이 발려 있었다.

⚠️ **막힌 칸에서는 남의 그림을 빌리지 않는다.** 그 판은 벽이고, 벽에 땅 그림을
발라 세우면 그것이 곧 잔디 커튼이다. 연고시티 성문 옆기둥(`area4_gate_b`는
불투명 58%라 등급 0)이 `ngrass`로 칠해져 아치 앞에 4.63타일짜리 잔디 벽이
서 있었다. 안 세워도 원작이 그 자리에 세로면을 갖고 있다 — 청크 57의
`area4_gate_a`·`_b` 37삼각형이 그 청크의 **유일한** 세로면이다.
...
| 자리 | 고치기 전 | 고친 뒤 |
|---|---|---|
| 연고시티 | `ngrass` 212 · `lgreen` 12 · `lgreenp` 12 | **0** |
| 꽃향기마을 | `ngrass` 164 | **0** |
| 축복시티 | `c1_g1` 480 | **0** |
