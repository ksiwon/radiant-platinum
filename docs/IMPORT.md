...
SDK와 원격 오류 수집기를 넣지 않는다.

⚠️ CSP `connect-src`에 바깥 오리진이 **하나** 있다 — 타이틀의 버그 제보가 쓰는
EmailJS다 (DEPLOY.md §3). **Import 라우트는 그것을 안 부른다.** 여기서 나가는
바이트는 여전히 없고, 사용자가 고른 파일의 내용·이름·해시는 어디로도 안 간다.
...
- 핸들을 얻는 길로 골랐으면 그 손잡이를 기억해 두었다가(§"최초 설치" 5) 다음에
  「지난번 그대로」로 낸다. 폴백(`<input>`)에는 손잡이가 없어 이 길이 없다.
...
1. 크기가 **범위 안**인지 — 아래는 제일 작은 지역판이 실제로 쓰는 크기,
   위는 자르지 않은 카트리지 이미지(128MB)
2. NDS 헤더·FNT·FAT 범위가 파일 안에 있는지, 그리고 **헤더 0x80이 적은
   “쓰인 크기”보다 파일이 짧지 않은지**
3. 필수 NARC·SDAT·오버레이 엔트리 존재
4. 지원 지역판과 리비전의 로컬 지문 — 게임 코드 · **헤더 0x80의 쓰인 크기** ·
   파일 수 · 대사 뱅크 수
5. 표본 NARC의 엔트리 수와 구조
6. 선택한 지역판에서 만들 수 있는 언어 선언

⚠️ **“정확히 128MB”를 요구하지 않는다.** NDS 덤프는 꼬리 패딩을 자르는 것이
흔한데, 실측하면 세 롬 다 헤더 0x80이 적는 크기가 **FAT의 최대 끝과 정확히
같고** 그 뒤로는 0x00/0xFF밖에 없다(잘라도 되는 꼬리가 27~30MB다). 곧 자른
덤프에도 우리가 읽는 바이트는 하나도 안 빠져 있다. 대신 **자른 것과 중간에
끊긴 것을 가른다** — 앞엣것은 그냥 쓰고, 뒤엣것은 “덤프가 끊겼을 수 있다”로
멈춘다. 판을 가르는 일은 파일 크기가 아니라 헤더 0x80이 맡는다(자르든 안
자르든 안 변하므로 파일 크기보다 좁게 가른다).
...
⚠️ **폴더 검증을 지났다고 끝이 아니다.** 그룹 검증은 색인·번들·표본 종까지만
보고, 판이 다른 덤프에서 무엇이 실제로 안 구워지는지는 굽기 전에는 모른다.
그래서 변환기 셋(`npcModels`·`monModels`·`arenas`)이 **굽겠다고 센 수와 구운
수를 맞대고, 하나라도 모자라면 그 자리에서 멈춘다.** 예전 바닥은 “하나도 못
구우면 실패”뿐이라 557종 중 300종이 빠져도 설치가 `ready`로 끝났다 — 무엇이
빠졌는지 세는 자가 아무 데도 없었다(`required.ts`는 그룹 **이름**만 세고
`integrity.ts`는 **쓴 것이 그대로인지**만 본다).
...
| 산출물 판과 재개 | ✅ 재개는 **바이트와 판을 둘 다** 본다. 판이 오른 그룹은 파일이 멀쩡해도 다시 만든다 | — |
...
4. **설치 기록에 원본을 남기지 않는다** — `.nds` 사본·romfs 사본·원본 파일명·
   경로·해시 어느 것도 `install.json`에 안 들어간다. 기록의 필드 목록을 시험이
   못 박는다 — 나중에 누가 `sourceFile`을 넣으면 거기서 걸린다
5. **고른 자리의 손잡이는 따로 기억한다** (`import/install/sourceHandles.ts`).
   기록이 아니라 별도 IndexedDB(`SOURCES_DB`)고, 담기는 것은 `FileSystemHandle`
   뿐이다 — 바이트도 경로 문자열도 아니다.

   왜 두느냐: 변환기를 고쳐 `GROUP_FORMAT`이 오르면 그 그룹을 다시 구워야
   하는데(§8), 손잡이가 없으면 그때마다 사용자가 **롬 파일과 수 GB짜리 BDSP
   폴더를 처음부터 다시 고른다.** 고르는 일은 소유를 확인하는 절차지 배포마다
   되풀이할 일이 아니다.

   지키는 것 셋: ⓐ 권한은 안 따라온다 — 다시 쓰려면 **사용자 제스처 안에서**
   `requestPermission()`을 한 번 더 받는다, ⓑ 「전부 지우기」가 이 창고를 같이
   지운다, ⓒ 화면이 기억한다고 **말하고** 「기억한 자리 잊기」를 같이 낸다
6. 모든 그룹의 임시 기록 → 길이·SHA-256 검증이 끝난 **뒤에만** `commit` 도장을
   찍는다. 도장은 매니페스트와 **같은 쓰기**로 나간다 (따로 두면 둘이 어긋난다)
7. `ready`가 되면 **reload 없이** OPFS provider로 갈아 끼우고 타이틀로 간다
...
| **그룹이 새로 생겼다** | ⚠️ **이미 깔린 사람에게는 안 들어온다.** 아래 |

⚠️ **새 그룹은 이미 깔린 설치본에 저절로 채워지지 않는다.** `installReady`가
보는 것은 「필수 목록이 다 있는가」지 「지금 앱이 아는 그룹이 다 있는가」가
아니고, 그럴 수밖에 없다 — 부팅이 `groups.ts`를 읽으면 변환기 열몇 벌이
첫 화면 청크로 딸려 온다(`install/installer.ts` 머리말). 그래서 이름만 든
`required.ts`를 본다.

그러면 갈래가 둘이다:

- **필수 목록에 넣으면** 이미 깔린 사람이 설치 화면으로 가서 **그 그룹만**
  다시 만든다 (`resumableGroups`가 나머지를 건너뛴다). 대신 원본을 다시
  집어야 한다.
- **안 넣으면** 그 그룹을 읽는 화면이 그것 없이 떠야 한다. ⚠️ **그 화면이
  `Promise.all` 한 뭉치로 읽고 있으면 그 하나가 나머지까지 데려간다** —
  `bagSprite`가 새로 생겼을 때 실제로 그랬다. 파일을 치우고 재 봤더니 가방
  그림뿐 아니라 **주머니 아이콘 여덟과 주머니 이름이 같이 사라지고** 도구
  이름·설명도 안 나왔다(`data`가 통째로 `undefined`가 된다). 낱개 실패는
  낱개로 받는다 (`BagScreen`).
