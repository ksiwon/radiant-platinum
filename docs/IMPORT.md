...
> (`src/import/install/required.ts`)을 **전부** 만든다. 실측으로 파일 7,096개
> 700.9MB를 4.9분에 옮기고 상태가 `ready`가 되며, 두 번째 실행은 아무것도 다시
> 만들지 않고 100ms 만에 갈래를 정해 152ms에 타이틀을 띄운다
> (`pnpm e2e`의 ⑮). Platinum 쪽 산출물은 개발 추출기가 만든 바이트와
...
| 새 설치가 **완료**된다 | ✅ 진짜 롬과 `AssetAssistant`만으로 **필수 28그룹 · 파일 7,096개 · 700.9MB**를 4.9분에 옮기고 `ready`가 된다 (⑮) |
