...
| **Platinum 변환 이식** | ✅ 필수 열아홉이 전부 브라우저에서 돈다. 산출물이 노드 추출기와 **바이트로 같다** — PNG만 펴서 픽셀로 잰다(deflate 구현이 달라 바이트는 같을 수가 없다) | — |
...
| 새 설치가 **완료**된다 | ⚠️ 필수 23개가 다 구현됐다. 진짜 입력으로 끝까지 도는 것은 아직 안 쟀다 (⑮) |
...
> ⚠️ **아직 확정으로 쓰지 않는다.** 진짜 입력으로 끝까지 설치한 뒤의 두 번째
> 실행을 브라우저에서 아직 안 쟀다 (DEPLOY.md §5의 ⑮).
