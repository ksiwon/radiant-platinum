...
> 700.3MB를 3.6분에 옮기고 상태가 `ready`가 되며, 두 번째 실행은 아무것도 다시
> 만들지 않고 67ms 만에 갈래를 정해 108ms에 타이틀을 띄운다
...
| 새 설치가 **완료**된다 | ✅ 진짜 롬과 `AssetAssistant`만으로 **필수 26그룹 · 파일 7,007개 · 700.3MB**를 3.6분에 옮기고 `ready`가 된다 (⑮) |
