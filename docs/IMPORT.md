...
> (`src/import/install/required.ts`)을 **전부** 만들고 상태가 `ready`가 되며,
> 두 번째 실행은 아무것도 다시 만들지 않고 곧바로 타이틀을 띄운다.
> **몇 개를 몇 분에 옮기는지는 [DEPLOY.md](DEPLOY.md) §5의 ⑮ 한 줄이 임자다** —
> 그 수는 실행마다 다시 재는 것이라 여기에 옮겨 적으면 반드시 낡는다.
> Platinum 쪽 산출물은 개발 추출기가 만든 바이트와 해시가 같고(`--only=09`),
> `src/import/bdsp/unityfs.ts`의 `SPIKE_BLOCKERS`는 비어 있다.
>
...
### 게임이 읽는 파일 ↔ 설치가 만드는 파일

⚠️ **이 짝을 보는 자가 없어서 열다섯 파일이 조용히 비어 있었다.** 노드 추출기는
굽고 브라우저 변환기에는 짝이 없던 자료가 아홉 그룹이었고, 읽는 쪽이 전부
`.catch`로 감싸 두어서 **화면은 뜨고 내용만 없었다.** 있던 문지기 둘은 이걸 못
본다 — `install/required.ts`는 **손으로 적는** 목록이고 `convert.test.ts`는
그 손 목록이 변환기를 갖는가만 본다.

`src/data/assetContract.test.ts`가 셋을 한다:

1. `src/data/gameData.ts`를 **소스로 읽어** `fetchJson` · `assets().bytes` ·
   `pinAtlas`의 인자를 모은다 (2026-08-19에 36자리). 로케일·뱅크 자리는 뭉갠다
2. `ALL_GROUPS`의 `outputs`와 맞댄다 (`{…}`·`*`·맨 `N`이 와일드카드)
3. 짝이 없으면 **그 파일 이름을 그대로 찍고 선다**

⚠️ **경로를 소스에서 읽는 이유가 있다.** 런타임에 모으면 그 화면을 열어 본
경로만 모인다 — 도감·타운맵·프런티어처럼 e2e가 안 들르는 자리가 정확히 그때
빠진 자리다.

⚠️ **눈 감은 채로 초록일 수 있다.** 짝이 다 맞는 것이 정상이라, 정규식이 어긋나
아무것도 못 잡아도 똑같이 통과한다. 그래서 그룹 하나를 뺀 목록으로 한 번 더
돌려 **그 이름이 찍히는지**를 같이 본다.
...
- “에셋 다시 설치”는 `assets/`와 설치 저널만 지우며 `saves/`와 IndexedDB
  리포트를 건드리지 않는다.
- “전부 지우고 처음부터”는 리포트까지 지운다. 리포트 백업 다운로드와 별도 확인
  없이는 실행하지 않는다 (`install/wipe.ts` · DEPLOY.md §5 ㉘).

### 막힌 사람에게 남은 길이 브라우저 설정뿐이었다

설치가 반쯤 하다 죽으면 남는 것은 **아무것도 못 하는 상태**다 — 에셋은 절반,
저널은 “하는 중”, 매니페스트는 `partial`. 다시 눌러 같은 자리에서 또 죽으면 그
사람이 할 수 있는 일이 없다. 브라우저 설정에 들어가 사이트 데이터를 지우는 법을
알아야 하는데, 그 화면은 브라우저마다 다르고 우리가 안내할 수도 없다.

그래서 그 길을 화면 안에 둔다. 지우는 것은 넷이다 — OPFS 나무 하나
(`radiant-platinum/`, 에셋·세이브·저널·매니페스트가 다 이 아래다), IndexedDB
둘(`radiant-platinum`과 옛 이름 `pt-3d`), Cache Storage와 서비스 워커, 그리고
설정 키 둘. **이름은 `state/storageNames.ts` 한 군데서 온다** — 쓰는 쪽과 지우는
쪽이 각자 문자열을 들고 있으면 이름을 바꾼 날 지우기가 엉뚱한 창고를 지우고
“다 지웠습니다”라고 말한다.

⚠️ **세 가지를 지킨다.**

- **처음 오는 사람에게는 안 보인다.** 지울 것이 없는 브라우저에 “전부 지웁니다”를
  띄우면 겁부터 준다. OPFS에 남은 것이 있거나 이번에 무언가 죽었을 때만 나온다.
  `failed`만 보면 안 된다 — 설치 도중에 탭을 닫은 사람은 다시 왔을 때 화면에
  아무 실패도 안 적혀 있고, 정작 이 단추가 제일 필요한 사람이 그 사람이다.
- **한 번에 안 지운다.** 누르면 확인 단추로 바뀐다. 되돌릴 자리가 없기 때문이다.
- **지우기 전에 파일로 받을 수 있다.** 여기 온 사람은 타이틀에 못 가므로 게임 안
  백업 화면을 못 연다. 이때가 마지막 기회다. 못 읽는 리포트도 원본 그대로 담는
  갈래(§11 끝)를 그대로 쓴다.

하나가 죽어도 나머지를 계속한다 — 지우려는 이유가 대개 “뭔가 깨졌다”라, OPFS가
안 열려서 IndexedDB를 못 지우면 안 된다. 못 지운 것은 세어서 화면에 적는다.
IndexedDB 삭제가 `blocked`로 막히면 거기서 기다리지 않는다(연결이 닫힐 때까지
`success`가 안 온다) — 어차피 바로 새로 여는 자리라 막힌 채로 넘어간다.
...
타이틀 화면에 **“세이브 파일 불러오기”**를 항상 둔다. 현재 로컬 리포트가 없어도
보여야 한다.
...
| 문제 | 현재 사실 | 해결 게이트 |
|---|---|---|
| Vite 공개 폴더 혼입 | ✅ `copyPublicDir: false` + 허용 목록. 642.0MB가 나가던 것을 액수 목록으로 줄였다 — 지금 크기는 [DEPLOY.md](DEPLOY.md) §3이 임자다 | 남은 것은 개발 산출물을 `public/` 밖으로 옮기는 일 |
...
| **Platinum 변환 이식** | ✅ 필수 스물둘이 전부 브라우저에서 돈다. 산출물이 노드 추출기와 **바이트로 같다** — PNG만 펴서 픽셀로 잰다(deflate 구현이 달라 바이트는 같을 수가 없다) | — |
...
| **BDSP 변환** | ✅ 네 그룹이 다 돈다. 무대 g001이 메시 158·정점 141,918·삼각형 129,589·재질 6·픽셀 1,196,032까지 개발 추출기와 **모두 같다**. 포켓몬 목차는 종·판 557개가 **바이트까지 같다**(29,438). 인물·포켓몬의 그림만 최대 2~3/255 다르다(32비트 `powf`) | — |
| 번들 안의 제3자 데이터 | ✅ 8,881kB → **0**. 안 쓰는 표 7,149kB는 없앴고, 수치는 사용자의 롬에서 오고, 효과 구현은 우리 소스로 옮겼다 | DEPLOY.md §4 |
| 번들 안의 `eval(` | ✅ **0건**. `>eval` 디버그 명령을 빌드에서 지운다 | — |
| CSP | ✅ `https://radiant.siwon.it.kr/`의 **응답 헤더**를 실제로 쟀다 — 지시자 14개 일치 · SPA fallback · 외부 오리진 요청 0 | `pnpm verify:deploy <url>` |
| 무전송 검증 | ✅ 브라우저 실측 22건 PASS. 미설치·설치 중·두 번째 실행 전부 요청 0건 | DEPLOY.md §5 |
| **한 번 설치하면 다시 안 묻기** | ⚠️ 구현은 끝났고 진짜 바이트로 "다시 안 만든다"까지 쟀다. **진짜 완주는 아직** | §15 · DEPLOY.md §5의 ⑮ |
...
| 조건 | 상태 |
|---|---|
| 프로덕션 빌드에 원본 유래 파일 0개 | ✅ 매 빌드 재검사. `@pkmn/sim` 제3자 데이터도 0바이트다 |
| 사용자 입력 전송 0 | ✅ 128MB 롬을 변환하는 내내 요청 0건 (⑨) |
| 새 설치가 **완료**된다 | ✅ 진짜 롬과 `AssetAssistant`만으로 필수 그룹을 전부 옮기고 `ready`가 된다. **잰 수는 [DEPLOY.md](DEPLOY.md) §5의 ⑮가 임자다** |
| 취소·중단 뒤 재개 | ✅ 취소 ⑪ · 손상 파일 재생성 ⑩ |
| 두 Provider가 같은 계약 시험 통과 | ✅ 노드 시험 |
| 대표 자료가 raw 개발판과 동등 | ✅ 옮긴 그룹 전부 **바이트로 같다** (⑨). PNG는 픽셀로 |
...
> 실측이 뒷받침한다 (DEPLOY.md §5의 ⑮): 진짜 입력으로 끝까지 설치한 뒤 브라우저를
> 새로 열면 **변환기 0회 · OPFS 쓰기 0회 · `/data` 0건 · 외부 0건**으로 갈래를
> 67ms에 정하고 108ms에 타이틀을 띄운다.
