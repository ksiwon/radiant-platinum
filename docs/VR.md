...
### 1.1 대상 기계 — 이것이 경로를 거의 혼자 정한다

| | |
|---|---|
| **윈도우 노트북 (인텔 내장 그래픽)** | ⚠️ **무조건 굴러가야 한다.** 협상 대상이 아니다 |
| macOS | **범위 밖.** VR을 쓰려면 윈도우를 쓰라고 안내한다 (§1.2) |

⚠️ **그래서 VR 작업의 제1 규칙은 「기본 게임을 건드리지 않는다」다.** VR은
얹는 것이고, 얹다가 윈도우 노트북의 기본 경로가 느려지면 그것은 실패다.
§3.5의 갈래 선택이 바로 그 함정이다.

### 1.2 macOS에서는 VR이 어떤 형태로도 안 된다 (범위 밖이지만 알고는 있는다)

🔵 SteamVR의 macOS 지원이 2020-04-30에 끝났고, Quest Link는 윈도우 전용이며,
Safari의 `immersive-vr`은 **visionOS 전용**이다(macOS·iOS 계획이 공개된 것이
없다). 데스크톱 크롬의 WebXR도 OpenXR 기반이라 윈도우에서만이다.

**고칠 수 있는 것이 아니므로 안내로 끝낸다** — "VR은 윈도우에서". 기본 게임은
맥에서도 돌아야 하지만 그것은 VR 이야기가 아니다.

### 1.3 그래서 배포 경로는 A-2다

- **A-2(스탠드얼론)** — 헤드셋이 곧 컴퓨터라 **사용자의 기계가 무엇이든
  무관하다.** 인텔 내장 노트북을 쓰든 맥을 쓰든 헤드셋에서 URL만 열면 된다
- **A-1(Link)** — 윈도우 + 헤드셋 + 케이블 + **버틸 만한 GPU**를 다 요구한다.
  ⚠️ 우리 대상이 인텔 내장이라 **그 마지막 조건에서 대부분 걸린다.**
  개발 편의로는 쓰되 배포 경로로 세지 않는다
...
1. **늘 WebGL로 돈다** — 재마운트가 없다. ⚠️ **대신 화면 게임이 WebGPU를
   잃는다. §1.1의 제1 규칙과 정면으로 부딪힌다** — 윈도우 노트북의 기본 경로가
   느려지는 쪽이면 그것은 실패다. **이 갈래를 고르려면 기본 게임 회귀를 먼저
   재야 한다**
2. **VR 진입 때 한 번만 재마운트한다** — 텍스처·모델을 다시 받는다.
   `onProviderSwap`이 이미 텍스처를 버리고 다시 받는 길을 갖고 있으니
   (`NpcSprites`) 그 수법을 쓸 수 있을 것으로 보인다.
   ✅ **기본 경로를 안 건드리므로 §1.1에 맞는다 — 이쪽이 기본값이어야 한다**
...
🟡 [DEPLOY.md §5](DEPLOY.md) 실측 (2026-09-06, 960×640, ANGLE Intel Arc 140V D3D11,
**`WebGPUBackend`**, 86장면 전수):

- **판정에 든 자리가 `celestic` 하나만 빼고 다 p95 16.7~16.8ms · 60fps**
- 무거운 자리: 갤럭시단아지트 집회장 **715.0k**삼각형/469콜 · 들판시티
  235.4k/154 · 209번도로 포켓치 207.2k/296 · 떡잎마을 204.6k/207
- ⚠️ **끊김(100ms 초과)이 다섯 자리에 7번 남는다** — 다 판정 문턱(3번) 아래다
- ⚠️ **맵을 나가는 멈춤은 이 수치에 안 들어 있다** — 훑기의 프레임 창이 화면이
  다시 돌기 시작한 뒤에 열리기 때문이다. 그 멈춤은 [REPAIR.md](REPAIR.md) §8.1이
  맡고 거기서 **5.5초**다. VR에서는 이 자리가 제일 위험하다
...
**성능은 이 검토에서 제일 불확실한 칸이다.** 실측 없이 판단하지 않는다 — §13.1.

### 4.1 아직 안 당긴 지렛대

재기 전에 포기할 필요가 없다. 손댈 곳이 남아 있다.

✅ **그리고 이 지렛대들은 버려지지 않는다** — 해상도 배율 · 그림자 · 드로우콜은
**저사양 노트북(인텔 내장 · 구형 맥북)에 필요한 것과 같은 목록**이다. VR을 안
하기로 해도 이 작업은 대상 기계를 넓힌다.
...
| **판때기 NPC** | 🟡 배치의 31.4%가 아직 2D 컷아웃 ([3D_GAP_AUDIT](3D_GAP_AUDIT.md)) — **사람은 하나도 없고** 바위·볼·환풍구 같은 물건뿐이다. 스테레오에서 1m 앞의 평면은 스티커로 보인다 |
...
#### A-1. PC VR (Link) — **개발 편의다. 배포 경로로 세지 않는다**

⚠️ 윈도우 전용인 것은 문제가 아니다(§1.2 — macOS는 범위 밖이다). **걸리는 것은
GPU다.** 우리 대상이 인텔 내장 노트북인데 이 경로는 그 노트북이 눈당 ~2000² 두
장을 90Hz로 밀어야 한다. **대부분의 대상 기계가 여기서 걸린다.**

개발자 기계에서 빠르게 돌려 보는 데는 좋다. 그것이 이 경로의 값어치다.
...
| ⚠️ 그 기계가 뭔가 | 🟡 **`Intel Arc 140V` — 내장 그래픽이다** (DEPLOY.md §5). 아래 참조 |
| 남는 성능 격차 | 🔴 픽셀 3~5배 + 90Hz |
...
⚠️ **"PC가 그린다"는 그 PC가 충분할 때만 위안이다.** 🟡 우리 측정 기계는
`Intel Arc 140V`고 이것은 **내장 그래픽**이다. 눈당 ~2000² 두 장을 90Hz로 미는
것은 내장 그래픽에 만만한 일이 아니다 — 게다가 내장은 메모리 대역을 CPU와
나눠 쓰는데 VR이 대역을 많이 먹는다.

→ **그래서 §13.1(실측)의 무게가 오히려 더 커진다.** "Link니까 성능은 괜찮다"로
넘기면 안 된다. 🔴 외장 GPU가 있는 기계가 따로 있는지, 없다면 해상도 배율과
포비에이션(§4.1)으로 어디까지 내려갈 수 있는지가 **기기를 사기 전에** 답이
나와야 하는 물음이다.
...
#### A-2. 스탠드얼론 — **VR이 배포될 수 있는 유일한 길**

헤드셋 브라우저가 직접 그린다. 사용자는 헤드셋에서 URL을 열 뿐이고,
**그 사람의 노트북이 무엇이든 — 맥북이든 낡은 내장 그래픽이든 — 상관이 없다.**
헤드셋이 곧 컴퓨터다. §1에서 본 대로 이것이 유일한 배포 경로다.

| | |
|---|---|
| 그리는 주체 | **헤드셋 칩** (Quest 3/3S면 XR2 Gen 2) |
| 성능 표적 | 🔴 모바일급이다. §4의 문제가 **고스란히 돌아온다** |
| 남는 것 | §8.3-4(발열)가 그대로 온다 |
| ⚠️ 뜻하는 것 | **기기를 산다면 스탠드얼론으로 재야 한다.** Link로만 재고 "된다"고 하면 배포판이 실제로 도는지는 한 번도 안 본 것이 된다 |

⚠️ **그래서 §4(성능)가 다시 제일 중요한 칸이 된다.** A-1이 잠깐 그것을 덜어
주는 것처럼 보였지만, 배포를 생각하면 표적은 결국 헤드셋 칩이다.
...
🔵 **A-1은 메타 전용이 아니다** (2026-08 확인). 데스크톱 크롬은 OpenXR을 WebXR
백엔드로 쓰므로, **SteamVR에 붙는 기기면 무엇이든 WebXR이 돈다** — Valve Index ·
HTC Vive · Pimax · Bigscreen Beyond · PSVR2(어댑터) · Pico(스트리밍)가 다
포함이다. 퀘스트만 되는 것이 아니다.
...
| 조건 | 왜 |
|---|---|
| **A-2: 스탠드얼론 브라우저가 WebXR을 하는가** | ⚠️ **이것이 필수 조건이다.** §1에서 본 대로 VR이 배포될 수 있는 길이 그것뿐이다. **PC 전용 기기는 여기서 전부 탈락한다** — 사도 개발자 혼자 볼 뿐 아무에게도 못 준다 |
| A-1: OpenXR로 윈도우에 붙는가 | 개발 편의. 있으면 좋지만 없어도 A-2로 재면 된다. ⚠️ 맥에서는 어차피 안 된다 |
| 값 · 광학 | 장시간 착용 |

⚠️ **이 한 줄이 표의 절반을 지운다.** PSVR2 · Index · Vive · Pimax ·
Bigscreen Beyond는 전부 PC 전용이라 **배포판을 확인할 수가 없다.** 화질이
아무리 좋아도 우리 조건에는 안 맞는다.
...
🔵 2026-08 확인, 한국 중고 기준. **실시세는 흔들리므로 살 때 다시 본다.**

| 기기 | 값 | A-1 | A-2 | 연결 | 비고 |
|---|---|---|---|---|---|
| **Quest 3S 중고** | **28~41만원** | ✅ Link | ✅ | **USB-C** | 프레넬 1832×1920 · 96° · XR2 Gen 2 |
| Quest 2 중고 | ~22만원 | ✅ | ✅ | USB-C | ⚠️ **기능 갱신 2026-12 종료** · 보안 2027년 말 |
| Quest 3 중고 | 44만원~ | ✅ | ✅ | USB-C | **팬케이크** 2064×2208 · 110°. 장시간 착용이 낫다 |
| **PSVR2 + PC어댑터** | 14~45만 + 7.7만 | ✅ SteamVR | ❌ **없음** | ⚠️ **DisplayPort** | **OLED · 무압축 네이티브.** 화질은 이 표에서 제일 낫다 |
| Pico 4 / 4 Ultra | 국내 유통 얇음 | ✅ 스트리밍 | ⚠️ 내장 브라우저 WebXR이 제한적 — Wolvic이 필요하다 | | 미국 미출시 |
| Valve Index | 비싸다 | ✅ | ❌ | DisplayPort + **베이스스테이션** | 설치가 번거롭다 |
| HTC Vive (구형) | 싸다 | ✅ | ❌ | 베이스스테이션 | 해상도가 낮다 |
| Quest 3S 신품 | 55만(128GB) · 71.5만(256GB) | | | | ⚠️ 2026-04-19 인상 (43.9→55만) |
| ~~Rift S~~ | 싸다 | ⚠️ | ❌ | | 🔵 **사지 마라** — 메타 지원 2021년 종료, 최종 펌웨어 2022년. 런타임이 불안정하다 |
| ~~WMR (Reverb G2 등)~~ | 매우 싸다 | ❌ | ❌ | | 🔵 **죽었다** — 아래 |
| Valve Steam Frame | 🔴 미정 ($899~1,199 추정) | | | | 2026-08 현재 값도 날짜도 없다 |

⚠️ **오래된 안내문에 속지 않는다.** 아직도 "Reverb G2로 WebXR이 된다"고 적힌
글이 돌아다니는데 그것은 24H2 이전 이야기다. 실제로는 안 돈다.

#### 권고: **Quest 3S 중고 28~35만원대**

메타여서가 아니다. **A-2를 요구하는 순간 스탠드얼론만 남고, 그중 값과 지원이
맞는 것이 이것이다.**

1. ⚠️ **A-2가 되는 기기가 사실상 퀘스트와 피코뿐이고**, 피코는 국내 유통이 얇고
   🔵 내장 브라우저의 WebXR이 제한적이라 Wolvic을 따로 깔아야 한다. **우리
   사용자에게 그것을 시킬 수 없다** — 배포 확인용으로 부적합하다
2. 🔵 **퀘스트 브라우저의 WebXR 지원이 스탠드얼론 중 제일 넓다.** 그리고
   **우리 사용자가 실제로 들고 있을 가능성이 제일 높은 기기**이기도 하다 —
   배포 확인은 그 기기로 해야 뜻이 있다
3. Quest 2와 **값 차이가 6만원**인데(22 vs 28만) 그 6만원에 **칩 한 세대**가
   붙는다. ⚠️ **A-2에서는 그 칩이 곧 성능이다** — Link 때와 달리 무관하지 않다.
   게다가 Quest 2는 기능 갱신이 넉 달 뒤 끝난다
4. 🟢 3S는 Quest 2의 화면과 광학을 그대로 쓴다(1832×1920 · 프레넬) — **보이는
   것은 Quest 2와 거의 같고 속과 수명이 다르다.** 화질을 기대하면 안 된다

**갈래는 하나뿐이다:** 장시간 착용이 중요하면 → **Quest 3**(팬케이크 · 110° ·
2064×2208). 🔴 16만원을 더 낼 값어치인지는 갈리지만, **A-2 성능은 3S와 같은
칩이라 성능 때문에 3을 살 이유는 없다.**

⚠️ **PSVR2는 우리 조건에서 탈락한다.** 화질(OLED·무압축)은 이 표에서 제일
좋지만 **PC 전용이라 배포판을 확인할 수 없다.** DisplayPort 요구도 노트북에
부담이다. 개발자 혼자 감상하려는 것이 아니라면 답이 아니다.
...
1. 🟢 [`tools/distribution/csp.mjs`](../tools/distribution/csp.mjs)의 `connect-src` —
   `http://192.168.x.x`가 거기 없다. 바깥 오리진은 **버그 제보의 EmailJS 하나**뿐이고,
   그 하나를 열 때 「더 늘리지 않는다」를 같이 적어 두었다
2. 혼합 콘텐츠 — HTTPS 배포 페이지는 `http://192.168.x.x`를 못 부른다. CSP와 무관
3. **그리고 이건 뚫으면 안 되는 벽이다.** `connect-src`는 성능 설정이 아니라
   [COPYRIGHT.md](COPYRIGHT.md)의 근거다. VR 테스트 편하자고 낼 비용이 아니다.

   ⚠️ **2026-09-04에 이 벽이 한 번 열렸다** — 버그 제보 하나 때문이다. 그 선례가
   「필요하면 열면 된다」로 읽히면 안 된다. 그때 나간 것은 **사람이 친 글**이고,
   여기서 나갈 것은 **사용자의 롬에서 나온 바이트**다. 같은 종류가 아니다
...
§13의 관문 3~5를 기기 없이도 회귀로 잡을 수 있다. **사기 전에 이것부터 재 보는
것도 방법이다.**
...
| Rift S · 구형 Rift | 🔵 메타 지원이 2021년에 끝났고 최종 펌웨어가 2022년이다. 런타임이 불안정하다는 보고가 있다 (§7.4) |
...
## 13. 다음에 VR 작업을 시작할 때 — 순서와 판정

**이 절만 읽고도 시작할 수 있게 적는다.** 관문마다 **판정 기준**이 있고,
통과 못 하면 **다음으로 안 넘어간다** — 짐작으로 진행하지 않는 것이 이
저장소의 규칙이다.
...
| | |
|---|---|
| ① | ⚠️ **윈도우 노트북(인텔 내장)에서 기본 게임이 무조건 굴러간다.** VR을 얹다가 이것이 느려지면 **되돌린다.** 협상 대상이 아니다 (§1.1) |
| ② | macOS는 범위 밖. "VR은 윈도우에서"로 안내한다 (§1.2) |
| ③ | 배포 경로는 **A-2(스탠드얼론)**다. A-1(Link)은 개발 편의다 (§1.3) |

### 13.1 관문 0 — 저사양 실측 · **기기 불필요** · 🔴 반나절

VR을 하든 안 하든 **어차피 필요한 수**다. 여기부터 시작한다.

```
1. vite.config.ts에 server: { host: true } 추가
2. pnpm dev --host
3. 같은 와이파이의 폰 + 대상급 윈도우 노트북에서 접속
4. 무거운 자리에서 fps를 잰다 (들판시티 234k삼각형/156콜 · 209번도로 199.6k/333)
```
...
| 결과 | 다음 |
|---|---|
| 대상 노트북에서 60fps · 폰에서 30fps 이상 | 관문 1로 |
| 그 아래 | ⚠️ **VR 전에 §4.1을 먼저 당긴다.** 기본 게임이 대상 기계에서 안 도는 것이 VR보다 급하다 (§13.0-①) |

⚠️ 헤드리스가 아니라 **실기 브라우저**에서 잰다. `pnpm story`의 수는 WebGL2
폴백 값이라 대리값이다 (DEPLOY.md §5).

### 13.2 관문 1 — WebXR이 우리 스택에서 뜨는가 · **기기 불필요** · 🔴 2~3일

**여기서 막히면 기기를 사도 소용없다.** 30만원을 쓰기 전에 하는 확인이다.

```
1. Immersive Web Emulator (크로미움 확장) 설치 — §11
2. 렌더러를 forceWebGL: true로 만든다 (§3.5)
3. gl.xr.setAnimationLoop 심을 끼운다 (§3.1)
4. 빈 씬으로 immersive-vr 세션을 연다
```
...
| 결과 | 다음 |
|---|---|
| 세션이 뜨고 두 눈이 그려진다 | 관문 2로 |
| `setAnimationLoop` 심으로 안 되면 | 루프를 직접 몬다. 그래도 안 되면 **A안을 접고 B안(SBS)으로** (§7) |

⚠️ **에뮬레이터로 되는 것과 실기로 되는 것은 다르다.** 여기 통과는 "코드가
말이 된다"까지고, 실기 판정은 관문 5다.

### 13.3 관문 2 — 기기 구입 · 🔵 시세는 살 때 다시 본다

**관문 0·1을 통과한 뒤에만 산다.**

- **Quest 3S 중고 28~35만원** (§7.4). 조건은 **A-2가 되는 것** 하나다
- ⚠️ WMR(Reverb G2 등) · Rift S는 **사면 안 된다** (§12)
- ⚠️ PSVR2 · Index · Vive는 PC 전용이라 배포 확인을 못 한다
- Link 케이블(USB 3)이 따로 든다

### 13.4 관문 3 — 카메라 리그 · 🔴 1주

```
고칠 곳: scene/EngineDriver.tsx:138-141  ← 카메라에 쓰는 자리는 여기 세 줄뿐이다
```

- Group 리그를 만들고 카메라를 그 안에 넣는다
- **리그가 위치와 yaw만 갖고, pitch/roll/머리 오프셋은 헤드셋이 준다**
- `local-floor` 기준을 쓰면 `EYE_HEIGHT 1.38`은 버린다 (§2)
- ⚠️ **`FIRST_DAMPING 12`를 머리 회전에 걸면 안 된다** — 즉시 멀미다.
  리그 이동에만 남긴다
- 같이 볼 것: `InteractionPrompt:72` · `PropFade:139-143` (§6)

**판정:** `pnpm story` 68장면 PASS · 기본 게임(3인칭/1인칭) 회귀 없음

### 13.5 관문 4 — 컨트롤러 · 🔴 3~5일

- `XRInputSource.gamepad`로 받는다 (§9.0). **조이콘 매핑은 필요 없다**
- `worldState.input.{move, run, interact, cancel}`을 채운다
- 스냅 턴을 넣는다. ⚠️ `RUN_SPEED 8`(m/s)은 VR에 너무 빠르다 — 배율을 둔다
- ⚠️ **새 입력 모듈은 `keys.ts` 편에 놓는다** — `initialChunk.test.ts` 경계 (§10)

**판정:** `pnpm check` 통과 · `initialChunk.test.ts` 통과

### 13.6 관문 5 — 「걸어다니는 신오」 + **스탠드얼론 실측** · 🔴 누적 1~3주

⚠️ **여기가 진짜 판정 자리다.** UI 없이 필드만 도는 판을 만들고, **Link가
아니라 헤드셋 스탠드얼론으로** 잰다.
...
| 결과 | 다음 |
|---|---|
| 두 눈 90Hz (최소 72Hz) | 관문 6으로 |
| 그 아래 | §4.1의 지렛대를 당긴다 — 포비에이션 · 해상도 배율 · 그림자 · 렌더 창 |
| 지렛대를 다 당겨도 안 되면 | ⚠️ **A-2를 접는다.** 그러면 VR은 배포 기능이 될 수 없고 개발자 장난감으로 남는다 — 그 판단을 여기서 한다 |

⚠️ **Link로만 재고 "된다"고 하면 배포판이 실제로 도는지는 한 번도 안 본 것이다.**

✅ **여기서 한 번 끊는다.** 신오를 1인칭으로 걷는 것 자체가 완성품이고,
§5.1(UI 17,134줄)의 결정은 **써 보고 나서** 한다.

### 13.7 관문 6 — UI를 어떻게 할 것인가 · 🔴 6~10주

관문 5를 통과했고, 써 봤고, 계속하기로 했을 때만 연다.

- 6DoF 레이저 포인터가 표준 패턴이라 키 커서보다 오히려 쉽다 (§9.0)
- 순서: 대사창 → 시작메뉴 → 가방/파티 → 배틀
- ⚠️ **VR 진입 버튼을 타이틀 화면에 두지 않는다** — `initialChunk.test.ts`가
  깨진다. `/play` 쪽에 둔다 (§10)

### 13.8 갈래 — A를 접었을 때의 B안(SBS)

관문 1이나 관문 5에서 접었다면 여기로 온다. **헤드셋 없이, WebXR 없이 좌우
분할로 입체만 만드는 길**이다 (§7 B안). 🔴 2~3주.

```
1. SBS + 배럴 왜곡 렌더 모드 — 🔴 2~3주
   scene/EngineDriver.tsx의 렌더 구간에 ArrayCamera 갈래를 내고
   옵션(view: 2)으로 켠다. 기존 3인칭/1인칭 경로는 안 건드린다.
   화각 55° → 90~100° 확대 (scene/Stage.tsx:67)
2. DOM UI 두 벌 복제 + 리스너 억제 — 🔴 3~5일 (§5.2)
3. 자이로 → camera.yaw/pitch — 🔴 3~5일 (§8.3)
4. 게임패드 — 🔴 3~5일 (§9.1). 여기서는 조이콘 매핑이 실제로 필요하다
```

✅ **B안의 장점 둘:** §3.2의 포스트 체인이 살아 있고, WebXR을 안 타므로
**`pnpm shot`으로 찍을 수 있다** — 68장면 회귀가 그대로 남는다 (§11).
⚠️ **단점:** 3DoF뿐이고 발열이 있으며 §6은 그대로 남는다.

### 13.9 관문마다 돌리는 것

```
pnpm check          타입 · 린트 · 시험 (자료 있는 판과 없는 판)
pnpm story          68장면 — 카메라를 건드렸으면 반드시
대상 노트북 실기     ⚠️ §13.0-①. 기본 게임이 안 느려졌는가
```
...
| | 왜 |
|---|---|
| 타이틀 화면의 정적 import 그래프 | `initialChunk.test.ts` (§10) |
| 기본 경로의 렌더 백엔드 | §1.1 제1 규칙 · §3.5 갈래 1 |
| `connect-src` (바깥 오리진 하나) | 전송 경계 (§8.4) |
| 원작 충실도 (공간 음향 등) | §6.1 — 기술이 아니라 결정 문제다 |
