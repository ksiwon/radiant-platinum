...
⚠️ **그런데 대상 기계가 경로를 거의 혼자 정한다.** 우리 대상은 "대부분의
노트북"이고 그것은 **인텔 내장 그래픽 아니면 맥북**이다. 그러면:

- 🔵 **맥북에서는 VR이 어떤 형태로도 안 된다** — SteamVR의 macOS 지원이
  2020-04-30에 끝났고, Quest Link는 윈도우 전용이며, Safari의 `immersive-vr`은
  **visionOS 전용**이다(macOS·iOS 계획 없음). 데스크톱 크롬의 WebXR도 OpenXR
  기반이라 윈도우에서만이다
- 인텔 내장 노트북은 A-1이 이론상 되지만 **GPU가 못 버틴다** (§7 A-1)

→ **그래서 VR이 배포 기능이 될 수 있는 길은 A-2(스탠드얼론) 하나뿐이다.**
헤드셋이 곧 컴퓨터라 **사용자의 노트북이 무엇이든 무관하다** — 맥북 쓰는 사람도
퀘스트만 있으면 URL을 연다. A-1은 **개발 편의일 뿐 배포 경로가 아니다.**
...
#### A-1. PC VR (Link) — **개발 전용이다. 배포 경로가 아니다**

⚠️ 🔵 **윈도우에서만 된다.** SteamVR macOS 지원은 2020-04-30에 끝났고 Quest
Link도 윈도우 전용이다. Safari의 `immersive-vr`은 visionOS 전용이라 맥에서는
브라우저로도 길이 없다. **대상이 "대부분의 노트북"이면 이 경로로는 아무에게도
못 준다** (§1).
...
→ **그래서 §10-0(실측)의 무게가 오히려 더 커진다.** "Link니까 성능은 괜찮다"로
...
⚠️ **이 확인의 무게가 오히려 제일 크다.** §1에서 본 대로 VR의 배포 경로가
A-2(스탠드얼론)뿐이고, 그러면 표적이 **헤드셋 칩 = 모바일급**이다. 폰에서 재는
수가 그것의 **가장 값싼 대리 지표**다.

🔴 **30만원을 쓰기 전에 답이 나와야 하는 물음:** 폰에서 지금 몇 fps인가?
거기서 어림잡은 값이 XR2 Gen 2에서 두 눈 90Hz에 닿을 만한가? 안 닿으면
§4.1의 지렛대로 어디까지 내려가야 하는가?

✅ **이 실측은 VR을 안 하기로 해도 쓸모가 있다** — 대상이 "대부분의 노트북"인
이상 저사양에서 몇 fps인지는 어차피 알아야 하는 수다.

### 그 다음 — **헤드셋을 사기로 했으면**

⚠️ **1~3은 Link(A-1)로 개발하되, 4에서 반드시 스탠드얼론(A-2)으로 재 본다.**
Link로만 재고 "된다"고 하면 배포판이 실제로 도는지는 한 번도 안 본 것이 된다.
...
4. **"걸어다니는 신오"를 여기서 한 번 끊고, 스탠드얼론에서 잰다** — 🔴 누적
   1~3주. UI 없이 필드만. ⚠️ **여기가 진짜 판정 자리다** — 헤드셋 칩에서 두 눈
   90Hz가 나오는가. 안 나오면 §4.1로 내려간다. **써 보고 나서 §5.1을 결정한다**
