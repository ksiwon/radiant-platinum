...
| 조건 | 왜 |
|---|---|
| **A-1: OpenXR로 윈도우에 붙는가** | 이것만 되면 개발은 된다. **헤드셋 칩 성능은 무관하다**(PC가 그린다) |
| **A-2: 스탠드얼론 브라우저가 WebXR을 하는가** | ⚠️ **여기서 갈린다.** 배포판이 실제 사용자에게 도는지 볼 수 있는 것은 **스탠드얼론 기기뿐**이다. PC 전용 기기는 이걸 못 한다 |
| 값 · 광학 · 연결 방식 | 장시간 착용과 노트북 사정 |
...
메타여서가 아니라 **이 프로젝트의 조건 셋을 다 만족하는 것이 그것뿐이라서**다:

1. **A-2를 할 수 있다.** 우리 배포 모델은 "사용자가 브라우저로 URL을 연다"이고,
   그것이 실제 헤드셋에서 도는지 볼 수 있는 것은 스탠드얼론뿐이다. 🔵 퀘스트
   브라우저의 WebXR 지원이 스탠드얼론 중 제일 넓다
2. **USB-C로 붙는다.** ⚠️ 우리 측정 기계는 노트북이고, **PSVR2는 DisplayPort를
   요구한다.** 노트북에 DP가 없으면 USB-C DP alt mode에 기대야 하는데 될지 안 될지가
   기계마다 다르다. Link는 그냥 USB 데이터라 그 위험이 없다
3. **베이스스테이션이 없다.** Index·Vive는 벽에 센서를 달아야 한다
4. Quest 2와 **값 차이가 6만원**인데(22 vs 28만) 그 6만원에 칩 한 세대와 지원
   수명 3년이 붙는다. ⚠️ Quest 2는 기능 갱신이 넉 달 뒤 끝난다
5. 🟢 **3S는 Quest 2의 화면과 광학을 그대로 쓴다**(1832×1920 · 프레넬) — Quest 2
   대비 **보이는 것은 거의 같고 속과 수명이 다르다.** 화질을 기대하면 안 된다

**갈래가 갈리는 자리 둘:**

- **A-2를 포기한다면 → PSVR2가 화질로는 낫다.** OLED에 무압축 네이티브고 값도
  비슷하다. ⚠️ 다만 DisplayPort · SteamVR 의존 · 어댑터 품귀(중고가 신품보다
  비싸다)를 안고 간다. **배포판을 헤드셋에서 확인할 생각이 없을 때만** 고른다
- **장시간 착용이 중요하면 → Quest 3**(팬케이크 · 110°). 🔴 16만원을 더 낼
  값어치인지는 갈린다
...
⚠️ **A-1(Link)로 가더라도 이 확인을 건너뛰면 안 된다.** 그리는 것이 PC라도
🟡 그 PC가 `Intel Arc 140V` **내장 그래픽**이다 (§7 A-1). 눈당 ~2000² 두 장을
90Hz로 미는 것이 만만치 않다.

🔴 **기기를 사기 전에 답이 나와야 하는 물음:** 외장 GPU가 있는 기계가 따로
있는가? 없다면 해상도 배율·포비에이션·그림자(§4.1)로 어디까지 내려갈 수
있는가? 30만원을 쓰기 전에 재는 것이 순서다.
