...
| §8.5 | 문 여닫는 애니 | 길이와 갈래가 원작 클립에서 온다 (`scene/DoorAnimations`의 `doorClip` · `data/props/anims.json`). 한동안 스무 종이 다 200ms였는데 롬을 재면 나무 여닫이가 8프레임(133ms) · 포켓몬센터 미닫이 15프레임(25 |
| §8.6 | 관절 애니 (JNT0) | 길이는 헤더에서 읽어 박자를 맞추고, 도중 움직임이 없다. 디코더는 풀렸다 (DATA §2.31) — 공개 문서가 `PivotMatrix`에서 두 자리 틀렸고 apicula가 정본이다. 직교성으로 재면 회전행렬 54,452벌이 어긋남 0(최악 오차가 고정소수 한 칸)이 |
