...
| 아무도 안 부르는 `export` | 89 (값 81 · 타입 8) | 세는 자는 `.audit/deadExports.mjs`(타입스크립트 파서로 훑는다). ⚠️ 여기 남은 것은 손으로 봐야 하는 것만이다 — 기계가 할 수 있는 자리(제 파일 안에서만 쓰는 `export` 481개)는 걷어냈고 `pnpm exports:check`가 다시 자라는 것을 막는다. 이름이 어디에도 안 나오는 89개는 쓸어 담으면 안 된다: 표본 여섯 중 셋이 지우면 안 되는 것이었다 — `FormatsData`는 일부러 둔 빈 껍데기고(DEPLOY.md §4가 「껍데기가 걸린 모듈과 안 걸린 모듈이 섞이면 없는 것보다 나쁘다」고 적어 둔 그 자리다), `DECIBEL`·`POKETCH_MAP_STEP`은 만들어 낸 파일에 있어 만드는 쪽을 고쳐야 하며, `scene/journal.ts`의 넷과 `scene/safari.ts`의 셋은 안 이어 붙인 기능이라 지우면 하려던 것이 같이 지워진다. 값이 확실한 자리는 `.css.ts`의 죽은 스타일 11개(규칙이 실제로 구워져 나간다) · `scene/GreyBox.tsx`의 컴포넌트 하나 · `import/bdsp/astc.ts`의 ISE 해독기 셋. ⚠️ 시험만 쓰는 `export` 552개는 흙이 아니다 — 순수 함수를 시험이 직접 부르라고 연 자리고, 검사가 시험을 소비자로 안 세면 그것이 다 걸려서 못 쓴다 |
