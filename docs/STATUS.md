...
| §48 | ~~아직 쓰는 그림을 버렸다~~ 고쳤다 — `journey` ⑯이 이것 하나로 떨어졌다. 텍스처 열셋에 이름을 달아 임자를 `chunk-slice 32x32`로 짚었고, 버리려는 순간 씬을 훑으니 한 판에 47건이 아직 붙은 채였다. 씬이 손을 뗀 뒤에만 버린다. 프레임을 세는 것으로는 안 닫혔다(오류가 1·0·5 → 12·12·3·7로 늘었다) | 끝 |
