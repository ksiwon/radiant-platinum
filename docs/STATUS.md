...
| 우리가 만든 자리가 어긋난 것 | 21 | [REPAIR.md](REPAIR.md) |
...
| §48 | 공급자를 갈아 끼울 때 버린 텍스처를 제출한다 — `journey` ⑯이 이것 하나로 떨어진다. 간헐적이다(같은 빌드 네 판에서 1 · 5 · 0 · 4건). 임자를 짚었다: `npcTexture`의 `onProviderSwap`이 표를 통째로 `dispose()`하는데, 사람 판때기 356장 중 196장이 32×32고 옆의 같은 꼴 캐시 둘은 `clear()`만 한다. 고치는 길이 둘이라 갈림길을 정해야 한다 | 작음 |
