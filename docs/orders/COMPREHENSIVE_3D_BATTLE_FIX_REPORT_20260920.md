...
**원인은 아직 모른다.** 남은 자리는 스크립트가 아니라 붙이는 쪽이다 — 진짜 서비스에서
`ctx.pause`가 기다리는 조건(문·카메라·이동처럼 맵에 딸린 것)이 워프 뒤에 안 풀리는 경우가
남는다. 다섯 판에 한 번이고 Lv.80 전설전을 일부러 져야 닿는 자리라 지금 도구로는 못 재며,
**미해결로 남긴다.** 두 시험은 남긴다 — 원인을 못 찾았어도 진 갈래가 미검증 지대였던 것은
사실이고, 그 자리는 이제 닫혔다.
