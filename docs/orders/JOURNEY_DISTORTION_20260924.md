...
| ⑥ | 창기둥 **그런트 둘 더블**과 **라이벌 태그 배틀**이 제품에서 싱글로 준다 — 이야기는 이어지지만 **형식이 원작과 다르다. 반드시 고친다**(사용자 2026-09-24 「더블도 무조건 되어야 해」 — REPAIR §82) | 제품 |
| ⑦ | 파도타기 중인 리포트가 이어하기에서 파도타기를 잃는다 — B4F 천장 물에서 쓰지 않는다 | 제품 (작은 결함) |
...
- 창기둥 두 배틀이 싱글이다 — `StartTrainerBattle`이 둘째 상대를 버리고(`commands.ts` 2724~2728),
  태그 배틀은 마스와의 1:1이다(`fieldServices.ts` 1736~1753). **배틀 형식이 원작과 다르다** — PARITY §9의 멀티 배틀 보류와 같은 자리. 두 트레이너 2대1 · 파트너 태그(멀티)를 원작대로 만든다(REPAIR §82).
- 마스터볼은 늘 잡는다(`capture.ts` 138) · 기라티나전은 야생 배틀(`fieldServices.ts` 1661) · 잡으면 결과 4로 스크립트가 「잡았다」 갈래.
- 파도타기 상태가 세이브에 없다(`schema.ts` `position`) — 물 위 리포트를 이어하면 뭍 걸음이 된다.
...
| 208 2F | w0 (27,48) → w2 (7,23) 벽화 구멍 | **괴력** | 괴력 바위 (14,45) (`FLAG_MAP_LOCAL_HIDE_OBSTACLE_2`) — 남으로 민다. 구멍 (7,23)은 벽화 오브젝트(565로 숨음)가 서 있던 칸, 행동 0x6E WARP_NORTH → (7,24)에서 북으로 |
