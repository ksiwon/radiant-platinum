...
⚠️ **각도는 바꾸되 배율은 안 바꾼다.** 샷마다 피사체까지의 거리가 제각각이면
기술 한 번에 화면이 확 당겨졌다 풀린다. 재어 보니 폭이 **3.1배**였고
(1.95~6.03m), 게다가 `oncoming`은 **어느 쪽이 때리느냐에 따라 2.4배** 갈렸다 —
`behind`를 두 자리의 `max`로 잡아서 p1이 때리면 6.03m, p2가 때리면 2.51m다.
플레이해 보면 연출보다 그 줌이 먼저 보인다. 원작 DS는 카메라가 아예 안
움직이고 BDSP도 배율까지 흔들지는 않는다. 그래서 **수평 반지름을 `SHOT_REACH`
하나로 못 박는다** (`atReach`):

| 샷 | 예전 거리 | 지금 |
|---|---:|---:|
| `establish` | 5.79m | 5.79m |
| `oncoming` (p1 때림 / p2 때림) | 6.03 / 2.51 | 5.76 |
| `impact` | 1.95 | 5.70 |
| `reaction` | 2.72 | 5.75 |
| `faint` | 2.06 | 5.68 |
| `switchIn` | 2.83 | 5.79 |

**높이는 샷마다 그대로 둔다.** 셋을 다 맞추면 낮은 샷이 땅에 파묻힌다 — 기절
샷은 눈높이 아래에서 올려다보는 각이라 3차원 거리로 늘리면 카메라가 y 0.11m가
된다. 수평만 맞추면 실제 거리가 위 표처럼 2% 안에 들어와 화면에 뜨는 크기가
안 변한다.

그리고 **샷 안의 움직임도 밀고 들어가기가 아니라 선회**다(`ORBIT_DEG` 2.6°).
카메라가 아주 멈추면 무대가 배경 그림이 되므로 움직이기는 해야 하는데, 그
움직임이 앞뒤면 그것도 줌이다. `shots.test.ts`가 여섯 샷 × 양쪽의 `from`·`to`
반지름을 전부 잰다.
