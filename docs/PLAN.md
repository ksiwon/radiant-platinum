...
| **텍스트/대사** | ✅ JSON 추출 — **영어·한국어·일본어. 스파이크 검증 완료** | 자체 구현 디코더(`tools/spike/gen4text.js`, pret msgenc 알고리즘 이식)로 3개 로케일 전 뱅크 **2,147개 복호화 성공, 실패 0**. 4세대는 통합 문자 테이블이라 **pret charmap 하나로 한글까지 전부 커버**(KO 105만 문자 미매핑 0) — 한글 커스텀 테이블 불필요 확정. **⚠️ 남은 과제: 뱅크 순서가 지역판마다 다르다**(US↔KO 같은 인덱스 일치 ~10%, KO↔JA ~63%, 예: 종족명 US #412 / KO·JA #408) — 구간별 오프셋 매핑 테이블 필요. US는 종족명을 대문자(TURTWIG)로 저장 — 표기 정책 결정 필요 |
...
- ✅ **텍스트 — 완료.** `tools/spike/`의 자체 NDS/NARC/텍스트 디코더로 3개 로케일 전 뱅크 복호화 성공(실패 0), 한글 완전 커버 확인. 남은 과제는 뱅크 매핑 테이블뿐 (§4.2). 이 스파이크 코드가 Phase 1 `tools/extract`의 기반이 된다
