...
**연출이 제일 큰 구멍이었고, 이번에 메웠다** (DATA.md §2.10). 진입점에서 제어
흐름을 따라가 닿는 명령 자리 55,421개 중 도는 것이 **77.3% → 95.4%**다.
이번에 붙인 것:
...
⚠️ `LockAll`은 **깃발만 세운다.** 배치표의 이동 유형(`MOVEMENT_TYPE_WANDER_*`)을
아직 안 돌려서 평소에 돌아다니는 사람이 없다 — 멈출 것이 스크립트가 걸어 둔
걸음뿐이다. 배회를 붙이면 그 자리가 이 깃발로 온다.

남은 2,527자리는 거의 전부 **우리에게 그 계통이 없는** 것들이다: 배틀타워 ·
모험노트 · 포켓치 · 문 여닫는 그림 · 통신 · 지하통로.
