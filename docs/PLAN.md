...
| | 실측 |
|---|---|
| 코드 | 48,536줄 · 파일 276개 · 시험 파일 93개 / 시험 1,018개 |
...
**연출이 제일 큰 구멍이었고, 메웠다** (DATA.md §2.10). 필드 스크립트에 명령이
놓인 자리 65,466개 중 도는 것이 **96.4%**다. 붙인 것:
...
⚠️ **파트너 고르는 장면의 그림은 원작과 다르다.** 원작은 `EV_POKESELECT`의 3D
모델과 NSBCA인데 그 계통을 아직 안 뽑았다. 롬에서 이미 뽑아 둔 것만 쓴다 —
서류가방 스프라이트 174번, 도구 아이콘의 몬스터볼, 포켓몬 앞모습. 순서·소리·글·
결과는 원작 그대로다.
