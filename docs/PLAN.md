...
| 손잡이 | 여는 법 | 하는 일 |
|---|---|---|
| **확인 지점** | **` (백틱)** — 타이틀·게임 중 어디서나 | 이야기 순서로 늘어놓은 80개 지점. 고른 자리로 순간이동 + 그 자리의 파티·가방·소지금·배지·도감까지 채움 |
...
| 체육관 장치를 **걸어서** 확인하는 자리 | 8자리 중 0 | 확인 지점의 체육관 여덟이 전부 문간(일곱은 `warp` 0, 연고만 3번)에 서서 `battle`로 관장전을 곧바로 연다 — 꽃시계·문 고르기·샌드백·물바닥·뜨는 판·톱니를 **실제로 밟는 확인 지점이 하나도 없다.** 연고는 그 위에 `gym3`의 맵이 88(입구 방)이라 문 방 89·90에 아예 안 들어간다. ⚠️ **화면 밖은 다 덮었다** — 장치의 알맹이는 `scene/*Gym.test.ts` 여섯 벌 110개가, 맵에 들어서면 켜지는가는 `engine/script/mapFeatures.test.ts` 10개가, 손잡이가 제 장치로 가는가는 `scene/fieldServices.test.ts` 14개가 본다. 남은 것은 **눈으로 보는 것** 하나다 — `battle` 없는 지점을 장치 앞에 세워야 한다 |
