...
| 배포물 | `dist` 파일 65개 · **4.5MB**. 첫 화면이 받는 것 347.9KB · **gzip 123.9KB** (목표 135 · 예산 150). 한 번 열 때 실제로 나가는 바이트는 따로 잰다 — 첫 방문 **636kB** · 다시 열기 **0.8kB** (DEPLOY.md §3) |
| 번들 출처 | `battle-sim` 청크 6.55MB → **1.51MB**. 게임 데이터 8,881kB → **0** (`pnpm provenance`: 제3자 정적 게임 데이터 없음, DEPLOY.md §4) |
| 브라우저 실측 | `pnpm e2e` — **PASS 27 · FAIL 0 · BLOCKED 0 · 못 잰 것 0** (DEPLOY.md §5) · `pnpm story` **PASS 70 · FAIL 0** |
| 공개 배포 | `pnpm release:check` **exit 0 · 열린 blocker 0** · `https://radiant.siwon.it.kr/` (DEPLOY.md §1) |

⚠️ **첫 화면 123.9KB는 부팅 게이트를 포함한 값이다.** 공개판은 부팅할 때
