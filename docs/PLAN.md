...
| | 실측 |
|---|---|
| 코드 | 52,256줄 · 파일 292개 · 시험 파일 102개 / 시험 1,104개 (`src/**/*.ts{,x}`) |
...
남은 구멍: 배틀 카메라 샷 · **배틀 무대 한 벌** · 자전거 · 비전기술 필드 사용 ·
...
⚠️ **무대가 하나라 어디서 싸워도 같은 풀밭이다.** 찍어서 확인했다 — 무쇠탄갱
(실내) · 무쇠 체육관 · 챔피언방 셋 다 `g001`(Battle_001)이 뜬다. BDSP는
`arenas/ground/g0xx`를 여럿 갖고 있으므로 굽는 일이지 만드는 일이 아니다.
땅 종류와 무대를 잇는 표를 원작에서 확인해야 한다.

⚠️ **숲 바닥을 메운 판이 민무늬였다 — 고쳤다.** 안 물리는 그림에 UV 평면을
늘려서 아틀라스 밖으로 나간 값이 가장자리 텍셀로 눌렸다(DATA.md §2.2). 숫자로는
"빈 칸 0"이라 다 된 줄 알았고, 화면을 열고서야 보였다.

⚠️ **영원의 숲은 나무 사이로 하늘이 비친다.** 나무가 빽빽한 자리 너머에 바닥 면이
없다. **걸어 다니는 데는 지장이 없다** — 재 보니 통행 가능한 칸 1,607개 전부에
바닥 높이가 있고 구멍이 0이다. 없는 것은 **통행 불가 칸의 바닥**이라, 원작 DS
카메라로는 나무 그늘에 가려 영영 안 보이던 자리다. 소품 뒷면(DATA.md §2.2)과
같은 갈래고, 3인칭 카메라가 낮아서 드러났다. 고칠 자리는 청크 쪽이다 — 통행 불가
칸에도 바닥을 깔거나, 나무 아래를 어둡게 덮는다.
