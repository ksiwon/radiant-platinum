...
>
> **Phase 1 데이터 추출은 대부분 끝났다.** 맵 헤더 표 593개, 행렬 270개 충돌 격자, 워프 1213개, NPC 3555명, 야생 인카운터 183표, 종족 508종, 기술 471개가 전부 실측 확정 + 원작 대조를 통과했다. 압축 후 신오 전체가 210KB 남짓이다. `scr_seq`(1124개 이벤트 스크립트)도 포맷을 확정했다 — 디컴프 원본을 조립해 롬과 바이트 단위로 맞췄고(1124/1124), 바이트코드를 그대로 싣는다(DATA.md §2.10). 남은 것은 그것을 돌릴 VM이다.
