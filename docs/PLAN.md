...
`SAVE_VERSION`은 20이고 슬롯은 **리포트 한 벌**이다. `report()`를 부른 시점만
...
| 턱이 평지에 그은 선이다 | 720칸 | 원작 높이 자료가 평평하다(위 표). 나무·바위처럼 우리가 단차를 세워야 한다 |
...
| 턱에 단차가 있는가 | 720 / 720이 **평평하다** | 고장이 아니라 원작이 그렇다 — BDHC는 턱 양쪽이 같은 높이고 턱은 그림(`allpeak`)이다. 3인칭에서는 잔디에 그은 갈색 선으로 읽히므로, 입체로 세우려면 나무·바위처럼 우리가 만들어야 한다 (§16.6과 같은 갈래) |
