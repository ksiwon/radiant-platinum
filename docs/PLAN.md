...
>
> **Phase 1 데이터 추출은 대부분 끝났다.** 맵 헤더 표 593개, 행렬 270개 충돌 격자, 워프 1213개, NPC 3555명, 야생 인카운터 183표, 종족 508종, 기술 471개가 전부 실측 확정 + 원작 대조를 통과했다. 압축 후 신오 전체가 210KB 남짓이다. `scr_seq`(1124개 이벤트 스크립트)도 포맷을 확정했다 — 디컴프 원본을 조립해 롬과 바이트 단위로 맞췄고(1124/1124), 바이트코드를 그대로 싣는다(DATA.md §2.10). VM과 명령 40여 개가 올라갔고 **오버월드에서 대화·간판·좌표 이벤트·트레이너전이 전부 돈다** — 앞 타일의 NPC를 찾고, 플래그로 분기하고, 이름과 조사가 들어간 글이 한 자씩 나오고, 트레이너에게 말을 걸면 그 사람의 대사가 나오고 배틀이 열리고, 이기면 플래그가 세이브에 남는다. **목록 메뉴도 돈다** — 선단시티 백화점 엘리베이터에 층 일곱 개가 원작 글 그대로 뜨고, 고른 값이 변수로 들어가 스크립트가 갈린다. 아이템 468종·아이콘 468칸이 들어왔고 가방·포켓몬·도감·트레이너 카드·상점·리포트·설정 화면이 X 키로 열린다. **리포트가 진짜 세이브다** — 걸어다니는 동안에는 아무것도 안 남고, 리포트를 쓴 그 자리(맵·격자·좌표·방향)에서 다시 시작한다. 설정에서 처음부터 지울 수도 있다. **인트로가 붙었다** — 타이틀에서 "새로운 모험을 시작한다"를 고르면 마박사가 나오고, 성별과 이름을 짓고 라이벌 이름을 정한 뒤 떡잎마을 주인공 방에서 눈을 뜬다. 글은 전부 `rowan_intro` 뱅크고 새 게임 플래그는 원작의 `scripts_init_new_game`을 **돌려서** 세운다(112개, DATA.md §2.14). 그래서 임시로 넣어 뒀던 몬스터볼 5개와 가짜 이름이 사라졌다. 남은 것은 통신과, 방을 나선 뒤의 이야기다 — 엄마·라이벌·201번도로·예진호수가 파트너로 이어지는 대목은 아직 안 돈다.
>
> **필드가 블록아웃을 벗었다.** land_data 청크의 NSBMD를 풀어 원작 지오메트리와 텍스처로 그린다 — 정점 106만·삼각형 60만, MDL0 헤더가 적어 둔 수치와 666/666 일치한다. 길·계단·물가·나무·건물 윤곽이 전부 원작 모양이다. 법선만은 롬 것을 안 쓴다 — 나무 600삼각형이 쓰는 법선이 둘뿐이고 둘 다 위를 봐서, 빛을 걸면 네 면이 같은 밝기가 되어 납작한 마름모가 된다. 지오메트리에서 다시 계산하고 그림자를 켠다. 
>
> NPC는 아직 캡슐이다. 그림(`mmodel.narc` 421벌)은 읽히는데 **스프라이트 번호 → 파일 번호 표**를 못 찾았다 (DATA.md §5).
...
- **야생·트레이너 AI (§7.7)** — 야생은 무작위, 트레이너는 롬의 AI 비트를 옮긴 점수 매기기
...
- 체육관 + 배지 — **관장의 EXPERT 비트가 여기서 처음 의미를 갖는다 (§7.7)**
- 기술머신, 기술 배우기/잊기
- 비전기술 필드 사용 (거합베기, 괴력)
- 낮/밤 사이클 + 시간대별 인카운터
...
| 손잡이 | 여는 법 | 하는 일 |
|---|---|---|
| 인트로 건너뛰기 | 타이틀 버튼 | 마박사 대목(45줄)을 건너뛰고 주인공 방에서 시작 |
| **확인 지점** | **` (백틱)** — 타이틀·게임 중 어디서나 | 목록에서 고른 자리로 순간이동 + 파티·가방·소지금까지 채움 |
| 개발 콘솔 | `window.pt` | 트레이너 찾기·배틀 열기·파티 넣기·회복 |

셋 다 **배포 번들에 안 들어간다.** 닿는 길이 전부 `import.meta.env.DEV`로 감싼 동적
...
야생을 만나 배틀을 끝내고, 저장하고 껐다 켜면 그 자리다. 시험 607개가 그것을 묶고
...
| 자산 | 청크 30MB · 소품 4.8MB · 모델 4.9MB · 텍스처 824KB (전부 PNG, KTX2 0개) |

### 16.2 ① 미구현 — 큰 것부터

**포켓몬 모델이 없다.** 배틀에 서는 것은 `capsuleGeometry`다(`BattleStage.tsx`).
무대·카메라·조명·그림자는 다 있고 그 자리에 끼우기만 하면 된다. **지금 이 게임에서
제일 크게 비어 있는 칸이다.**

**기술 연출이 없다.** 원작은 글을 찍은 뒤 `PlayMoveAnimation`이 돈다. 지금은 글만
찍는다(`playback.ts`). §7.3의 DSL과 아키타입 5종이 여기 붙는다.

**소리가 없다.** `engine/audio/unlock.ts` 21줄이 전부다 — 오디오 언락 게이트만 있고
SDAT 추출도 재생도 미착수다. 곡이 없으면 완성도가 아무리 올라가도 죽은 화면으로
느껴진다. §4.5의 판단(OptimePlayer에 SDAT를 올려 소리가 나는지)이 아직 안 났다.

그 밖에: 자전거 없음 · 비전기술 필드 사용 없음 · PWA 0파일 · 긴 풀숲 인카운터율
70%가 미적용(타일 행동값 30종이 아직 안 갈렸다, `encounter.ts`).
...
지금 씬의 빛은 반구광 하나 + 태양 하나 + 필 하나다(§10.2의 "활성 라이트 3개 이하"에
딱 맞다). 그림자를 던지는 것은 태양 하나뿐이고 절두체 반경 30타일이다.

- **물이 아직 판이다.** `fx/`에 하늘(`sky`)·팔레트(`palette`)·후처리(`post`)는 있는데
  물이 없다. 원작 물가는 도트 애니메이션이라 1인칭으로 서면 장판이 된다 — 나무·풀과
  똑같은 문제이고 똑같이 풀 수 있다(거동값 `0x0015`·`0x0010`이 자리를 준다).
- **시간대.** `worldState.time.gameHour`는 있는데 하늘·조명이 그것을 안 받는다.
  4세대는 시간대별 인카운터가 있어서 데이터 쪽도 같이 열린다.
- **후처리.** `post.ts`가 있으나 프리셋(§10.1)과 연결돼 있지 않다.

### 16.4 ③ 최적화 — 재는 것이 먼저다

⚠️ **두 수가 3배 어긋난다.** 화면 오버레이는 175.4k 삼각형이라는데, 창 5×5의
나무만 세어도 3,583그루 × 156 = **55만**이다. 지형 3만을 더하면 더 벌어진다.
둘 중 하나는 틀렸고, **어느 쪽이 틀렸는지 모르는 채로는 최적화를 시작할 수 없다.**
숲 한가운데에 서서 오버레이를 읽는 것이 첫 번째 할 일이다.

그 뒤에 손댈 자리는 이미 보인다. 근거와 함께 적어 둔다:

- **인스턴스 프러스텀 컬링.** `Foliage`·`Grass`가 `frustumCulled = false`다. 인스턴스가
  청크를 가로질러 흩어져 있어서 메시 단위 절두체가 뜻이 없기 때문인데, 그 대신
  **뒤에 있는 나무까지 전부 그린다.** 인스턴스를 청크별로 쪼개면 절두체가 다시
  의미를 갖는다 — 드로우콜은 늘고 삼각형은 준다. 어느 쪽이 이득인지는 재고 정한다.
- **나무 LOD.** 그루당 156삼각형이 거리와 무관하게 고정이다. 멀리 있는 나무는
  큰 덩이 하나(20면)면 충분하다 — 잎 덩이 셋을 거리로 끄면 156 → 56이다.
  `nearScale`이 이미 그루마다 카메라 거리를 재고 있으므로 붙일 자리가 있다.
- **KTX2.** §10.2가 "텍스처는 KTX2 고정"인데 지금 전부 PNG다(KTX2 0개). PNG는
  VRAM에서 압축이 풀린다 — 4세대 텍스처는 작아서(824KB) 당장 안 아프지만,
  포켓몬·NPC 모델이 들어오면 여기가 먼저 찬다.
- **잔디 밀도 프리셋.** `TUFTS`가 3으로 박혀 있다. §10.1은 0.3/0.7/1.0이다.
...
⚠️ **BDSP 클립 경로가 완전히 막혀 있다.** 지금 로컬 상태를 다시 쟀다:
...
**NPC는 지금 2D 스프라이트다**(`NpcSprites.tsx` + `data/npc/*.png` 567KB). 원작
판때기를 그대로 세운 것이라 1인칭에서 옆으로 돌면 종잇장이다 — 나무와 같은 문제다.
오버월드에 NPC가 3,555명, `graphicsID`가 1~262종이다.

**포켓몬은 캡슐이다.** 493종이 필요하다.

둘 다 "모델을 어디서 가져오느냐"가 전부이고, 답은 하나가 아니다. 고르기 전에
비용을 적어 둔다:

| 길 | 얻는 것 | 드는 것 |
|---|---|---|
| BDSP 덤프 | 원작 그대로의 트레이너·포켓몬 | NSP 4.3GB 재확보 + nstool + UnityPy. **지금 전부 막혀 있다** |
| 4세대 롬의 `pl_poke_data` | 원작 4세대 모델(저폴리) | NSBMD 애니메이션 파서를 우리가 써야 한다 |
| 커뮤니티 모델 | 빠르다 (Dawn이 그 길이었다) | 종별로 스타일이 갈린다. 493종을 못 채운다 |
| 스프라이트 임퍼스터 | 지금 스프라이트를 그대로 쓰되 카메라를 향해 세운다 | 3D가 아니다. 다만 **원작 화풍에는 맞다** |

⚠️ **NPC에 임퍼스터를 그냥 쓸 수는 없다.** 나무에서 이미 겪었다 — 원작 그림이
*내려다본* 모양이라 카메라 쪽으로 세우면 누운 그림이 선다. NPC 스프라이트는
정면·측면·후면이 따로 있으므로(방향 4종) 그쪽은 사정이 다르지만, **세우기 전에
그림이 어느 각도에서 그려진 것인지부터 재야 한다.**

순서는 **포켓몬이 먼저다.** 배틀은 카메라가 고정이고 두 마리만 서므로 파이프라인을
한 번만 통과시키면 되고, 여기서 검증된 규격이 그대로 NPC 262종으로 간다. 반대로
하면 262종을 먼저 깔았다가 규격이 틀린 것을 나중에 안다.

### 16.7 ⑥ 소품 면 — 뒤만 막았을 뿐이다

집 뒷면은 막았다(§DATA 2.2). 그런데 **뒤만 막았다.** 소품 590종의 면 방향을 전수로
세어 보면 여섯 방향이 다 있는 것이 78종뿐이다:

| 그쪽을 보는 면이 0개 | 590종 중 | 오버월드 배치 501개 중 |
|---|---|---|
| **−Z (뒤)** | 322 (55%) | 323 (**64%**) ← 뒤판이 막았다 |
| **−X (왼쪽)** | 194 (33%) | 202 (**40%**) |
| **+Y (위)** | 88 (15%) | 154 (**31%**) |
| **+X (오른쪽)** | 183 (31%) | 111 (**22%**) |
| +Z (앞) | 93 (16%) | 28 (6%) |
...
3인칭 카메라가 26.6°로 **내려다보므로 +Y가 뚫린 31%가 눈에 들어온다.** 왼쪽이
뚫린 40%도 걸어 지나가면 보인다. 바닥(−Y)만 안 채운다 — 건물 밑은 볼 자리가 없다.

고칠 방법은 이미 있다. `shell.ts`의 뒤판을 **축 일반화**하면 된다. 지금은 z를
눌러 −Z 판을 만드는데, 같은 코드가 축만 바꿔 x·y로도 돈다. 실루엣 검사도
축 인자만 받으면 그대로 돈다 — 잣대는 바꾸지 않는다.

⚠️ 넓이가 아니라 **실루엣이 다 막혔는가**로 잰다. 넓이로 재면 문틀만 덮고도 합이
맞는다(그래서 한 번 틀렸다). 축을 늘려도 이 원칙은 그대로다.

한 가지 더: `h_kage`(그림자 데칼)가 순검정 판으로 그려진다. 반투명 블렌딩을
안 지켜서 집 밑에 검은 얼룩이 생긴다 — 뒤판과 무관한 별건이고, 소품 재질이
블렌드 모드를 읽게 하면 같이 풀린다.

### 16.8 순서

의존을 따라가면 순서가 저절로 나온다.

1. **소품 면 축 일반화** (⑥). 기계가 이미 있고 시험도 있다. 배치의 40%·31%·22%가
   즉시 막힌다 — 들이는 것 대비 화면에 남는 것이 제일 크다.
2. **프로파일 다시 재기** (③). 175k와 55만 중 어느 쪽이 맞는지. **포켓몬·NPC 모델을
   올리기 전에 끝나야 한다** — 예산을 모르는 채로 자산을 얹으면 나중에 무엇을
   되돌려야 하는지 알 수 없다.
3. **포켓몬 모델** (⑤). 지금 제일 크게 비어 있는 칸이고, 여기서 정해지는 규격이
   NPC로 그대로 간다.
4. **기술 연출** (①). 모델이 서야 연출이 뜻을 갖는다.
5. **NPC 3D** (⑤). 262종. 포켓몬에서 검증된 길로 간다.
6. **소리** (①). 위와 의존이 없으므로 언제든 병행할 수 있다. §4.5 판단(SDAT를
   OptimePlayer에 올려 소리가 나는가)부터.

**병행:** BDSP 경로 복구(NSP 재확보 → nstool → UnityPy)는 ④·⑤ 둘 다의 상류라
막힌 채로 두면 계속 대체물로 간다. 다만 그것이 4.3GB 다운로드 하나에 걸려 있어서
작업 순서에 넣지 않고 따로 뚫는다.

**안 하는 것:** 바닥면(−Y) 채우기 · 자전거 · PWA. 앞의 것들이 끝나기 전에는
화면에 남는 것이 없다.
