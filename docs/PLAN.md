...
없고, 손으로 적은 짝 13개를 더해 **156 중 45**가 붙는다:

| | 수 | |
|---|---|---|
| 등신 (`battle/tr####`) | 42 | 트레이너 갈래 전부가 여기 있다 |
| 치비 (`field/fc####`) | 3 | 게임 감독 · 메이드 · 조이 간호사 — **트레이너가 아니라 배틀 몸이 없다** |
| 못 붙임 | 111 | 아래 |

⚠️ **나머지 111은 일부러 비워 뒀다.** 느슨하게 맞춰 보면 몇 쌍이 더 붙는데 그중
...
- ⛔ **걷는 클립이 없다.** 등신 몸(`battle/tr####`)에 붙은 여덟은 전부 **제자리**
  배틀 동작이다 — 서기·말하기·등판·패배. 원작 BDSP에서 이 몸은 걷지 않는다.
  치비 쪽(`field/fc####`)에는 걷기·낚시·잠자기까지 **56개**가 있는데,
  ⚠️ **뼈 이름이 121 중 51개만 겹친다.** 리그가 다르므로 그대로 옮겨 붙일 수
  없다. 그래서 셋 중 하나를 골라야 한다: 걷는 NPC는 판때기로 두거나, 그 사람만
  치비를 쓰거나, 걷기를 우리가 리타깃하거나.
- ⚠️ **치비 번들은 자급자족이 아니다.** `fc0001_00`을 열면 메시가 다른 번들에
  있다고 나온다 — 폴더(644MB)를 통째로 열어야 풀린다. 등신 번들은 하나에 다
  들어 있다.
- ⚠️ **용량.** 애니메이션까지 넣으면 한 명 4.5MB다(그림 2.7 + 동작 1.8).
  §4.4의 KTX2 + Meshopt를 안 태우면 42명은 못 싣는다.
- ⛔ **화면에 세우는 일.** 위 셋을 정하고 나서다.
