...
106벌 중 열일곱이 필드 번들뿐이고 엄마·간호사처럼 대부분 이야기 인물이라,
판때기로 되돌리면 오프닝 바로 다음 장면이 종잇장이 된다. 그 열일곱은 **머리뼈를
...
| 자산 | 44그룹 · 파일 11,220개 · 946.2MB — **포켓몬 3D 672.9**(파일 3,933) · **배틀 무대 128.2**(30벌) · NPC 모델 87.7(106벌) · 청크 28.4 · 소리 7.5 · 주인공 4.8 · 대사 3.7 · 행렬 3.4 · 소품 2.8MB (전부 PNG, KTX2 0개). 설치가 **요구하는** 것은 그중 26그룹이다 (DEPLOY.md §5.3 ⑮) |
...
| **모델이 서는 배치** | **1,761개 (49.5%)** — 사람 그림만 보면 94.5% · 106벌 143.18MiB |
| 판때기로 남는 배치 | 1,794개 — 그중 **사람은 103개뿐**이고 나머지는 물건·포켓몬·자리표시자다 (3D_GAP §3.1) |
...
남은 것은 **비율**이다. 사람 그림 기준 94.5%라 한 화면에서 눈에 띄는 자리는 마박사뿐이다.
...
| ⑤ NPC 3D | ✅ 배치의 49.5%가 등신 모델로 선다 — 사람 그림만 보면 94.5%다 |
...
실측 **106벌에 143.18MiB**고, 클립을 하나도 안 실으면 87.72MiB · 여덟을 다
실으면 234.93MiB다. 배틀 클립 합계는 264개다 — 89벌 중 86벌이 셋을 다 갖고,
...
✅ **화면에 섰다.** 배치의 49.5%가 등신 모델이다 — 사람 그림만 보면 94.5%다 (§16.6). 컷신에서도 선다 —
