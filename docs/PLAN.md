...
⚠️ **걷기를 옮기는 도구는 있는데 굽는 차례에서 안 쓴다.** 구워 낸 등신 89벌에
실린 것은 배틀 클립 셋뿐이고 걷기는 하나도 없다 — 걷기는 그대로
`actor/locomotion`이 뼈를 돌려 만든다. 등신 몸에 붙은 여덟은 전부 제자리 배틀
동작이라 걷는 것이 없고, 치비 쪽에는 56개가 있다. 아래 대수는
`tools/extract/bdspRetarget.py`에서 실측으로 확인된 것이고, **브라우저
변환기에는 같은 것이 아직 없다** — 굽는 쪽이 둘이라 한쪽만 있으면 못 쓴다.
...
✅ **용량은 갈래를 갈라서 풀었다.** 걷기를 `actor/locomotion`이 뼈를 직접
돌려서 만드므로(주인공도 그렇다) 치비(`fc*`) 17벌은 클립을 하나도 안 싣고,
등신(`tr*`·`pc*`) 89벌은 **배틀에서 이어 붙는 셋만** 싣는다
(`engine/actor/npcModels`의 `TRAINER_CLIPS` — 굽는 쪽 둘이 이 규칙 하나를 같이
본다). 실측 **106벌에 141.04MiB**(등신 132.93 · 치비 8.11)고, 클립을 하나도 안
실으면 87.72MiB · 여덟을 다 실으면 234.93MiB다. 클립 합계는 264개다 — 89벌 중
86벌이 셋을 다 갖고, 주인공 둘(`pc0001_00`·`pc0002_00`)과 `tr2018_00`에는
**`lose01_b`가 아예 없다**(원작이 안 만들었다. 그 셋은 진 동작을 안 한다).
서 있는 사람은 여전히 매 프레임 돌려야 한다 — 안 돌리면 팔을 벌린 T 자세로
선다. 갈래별 값과 **어느 것을 안 굽기로 했는지**는
[3D_GAP_AUDIT.md](3D_GAP_AUDIT.md) §3.2에 있다.
