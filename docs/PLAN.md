...
- ⛔ **애니메이션.** 클립이 Mecanim의 `m_MuscleClip`이라 UnityPy가 커브를 안
  펴 준다. 원시 타입트리로 내려가면 `m_StreamedClip`(176,480워드)과
  `m_ConstantClip`(921개)이 실제로 들어 있으므로 길은 있다 — 스트림 포맷을
  손으로 푸는 일이 남았다. 지금 나오는 것은 번들에 저장된 **쉬는 자세**(A 포즈,
  팔이 어깨에서 36° 내려와 있다)다
- ⛔ **화면에 세우는 일.** 위 때문이다. 숨도 안 쉬는 사람을 길에 세우는 것보다는
  판때기가 낫다
- ⚠️ **용량.** 한 명이 평균 2.5MB다(텍스처가 대부분). §4.4의 KTX2 + Meshopt를
  안 태우면 42명은 못 싣는다
