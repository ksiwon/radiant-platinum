...
114벌 중 스물다섯이 필드 번들뿐이고 엄마·간호사처럼 대부분 이야기 인물이라,
판때기로 되돌리면 오프닝 바로 다음 장면이 종잇장이 된다. 그 스물다섯은 **머리뼈를
...
| 자산 | 44그룹 · 파일 11,249개 · 1001.6MB — **포켓몬 3D 672.8**(파일 3,933) · **NPC 모델 146.1**(114벌) · **배틀 무대 128.2**(30벌) · 청크 28.4 · 소리 7.5 · 대사 3.7 · 행렬 3.4 · 소품 2.8 · 주인공 1.5MB (전부 PNG, KTX2 0개). 설치가 **요구하는** 것은 그중 26그룹이다 (DEPLOY.md §5.3 ⑮) |
...
| **모델이 서는 배치** | **1,879개 (52.9%)** — 사람 그림만 보면 99.2% · 114벌 146.1MiB |
| 판때기로 남는 배치 | 1,676개 — 그중 **사람은 열여섯뿐**(핸섬·플루토, BDSP에 몸이 없다)이고 나머지는 물건·포켓몬·자리표시자다 (3D_GAP §3.1) |
...
남은 것은 **비율**이다. 사람 그림 기준 99.2%고, 남은 열여섯은 BDSP에 몸이 없는 핸섬·플루토다.
...
| ⑤ NPC 3D | ✅ 배치의 52.9%가 모델로 선다 — 사람 그림만 보면 99.2%다 |
...
실측 **114벌에 146.1MiB**다. 106벌로 잰 자리에서 클립을 하나도 안 실으면
...
✅ **화면에 섰다.** 배치의 52.9%가 모델이다 — 사람 그림만 보면 99.2%다 (§16.6). 컷신에서도 선다 —
