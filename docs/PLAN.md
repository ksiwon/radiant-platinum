...
~~**포켓몬 모델이 없다.**~~ ✅ 원작 배틀 그림 493종이 선다(DATA.md §2.17).
3D 모델을 지어내지 않았다 — 4세대 배틀은 3D가 아니고, 무대와 카메라만 3D고
포켓몬은 80×80 도트 한 장이다.

~~**기술 연출이 없다.**~~ ✅ 틀 다섯에 타입 색을 갈아 끼운다(§7.3,
`engine/battle/vfx`). 어느 틀인지는 **롬의 기술 데이터가 정한다**:
`target & 0x10`(자기에게) → `category === 'status'` → `contact` → `special`.
471개가 전부 다섯 틀에 떨어진다. ⚠️ 원작 연출을 한 컷씩 옮긴 것이 아니다 —
기술별 연출 파일 표를 아직 안 읽었고, 지금 보여 주는 것은 "어떤 종류의
사건인가"다.

**소리가 없다.** `engine/audio/unlock.ts` 21줄이 전부다 — 오디오 언락 게이트만
있고 SDAT 재생이 미착수다. 포맷은 전부 풀려 있다(DATA.md §2.18: 곡 1013 ·
악기 뱅크 521 · 파형 874, 모르는 명령 0). 붙이는 일만 남았다.

그 밖에: 자전거 없음 · 비전기술 필드 사용 없음 · PWA 0파일 · 긴 풀숲 인카운터율
70%가 미적용(타일 거동값 32종이 아직 안 갈렸다 — 다만 전 오버월드의 1.3%다).
...
### 16.8 ⑦ 소리 — 형식은 확인했고 재생이 남았다

⚠️ **여기만 끝을 못 봤다.** 정직하게 적어 둔다.

된 것: SDAT를 열어 **전곡 1013개를 걸었다**(`tools/extract/sound.js`).

| | 실측 |
|---|---|
| 곡 | 1,013 |
| 푼 명령 | 706,455 (같은 자리를 두 번 안 센다) |
| 음표 | 258,642 |
| **모르는 명령** | **0종** |
| **세기 127 넘는 음표** | **0** |
| 템포 | 6~512 |

셋이 동시에 맞는 것이 명령표를 아는 증거다. 명령 폭을 하나만 잘못 알면 악보가
한 칸씩 밀리는데, ⚠️ 그때 나오는 답이 **완전히 깨진 값이 아니라 조금 이상한
값**이라는 것이 이 형식의 함정이다(트랙 오프셋을 파일 처음에서 재던 때
"모르는 명령 46개 · 세기 초과 1414 · 템포 최대 5057"이 나왔다).

명령표는 `engine/audio/sseq.ts`가 정본이고 추출기가 한 벌 더 든다(CommonJS라
TS를 못 부른다). **시험이 256개 값을 맞대 본다** — 한쪽만 고치면 걸린다.

안 된 것:

- **표본을 못 뽑았다.** 부호화 개수는 맞다(PCM8 494 · ADPCM 380, DATA.md와 일치)
  인데 **SWAR 안에서 표본 헤더 위치를 잘못 짚어** 길이 계산이 터무니없이 나온다.
  여기를 먼저 바로잡아야 한다.
- SBNK 악기표 · ADSR · PSG 파형 · Web Audio 재생기.
- 맵 헤더의 BGM 번호를 곡에 잇는 일.

바로 다음에 할 일은 SWAR 표본 헤더를 다시 재는 것이다 — DATA.md §2.18이
"`(loopOffset + nonLoopLength) × 4`가 874개 전부 실제 길이와 맞는다"고 적어
뒀으니, 그 식이 맞는 자리를 찾으면 나머지가 풀린다.
