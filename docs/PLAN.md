...
| 배틀 씬(무대·카메라·조명·그림자) | ✅ ⚠️ 무대가 **하나뿐이다** — 탄갱에서도 체육관에서도 챔피언방에서도 같은 풀밭이 뜬다 (`arenas/ground/g001` 한 벌만 구웠다) |
...
| | 실측 |
|---|---|
| 코드 | 53,170줄 · 파일 297개 · 시험 파일 104개 / 시험 1,170개 (`src/**/*.ts{,x}`) |
...
| 자산 | 파일 6,577개 · 176.0MB — **배틀 무대 74.5**(17벌) · NPC 모델 43.6 · 청크 29.8 · 소리 7.9 · 주인공 5.1 · 행렬 3.5 · 대사 3.5 · 소품 3.0MB (전부 PNG, KTX2 0개) |
...
- **포켓몬만 아직 도트 한 장이다.** 4세대 배틀 자체가 그랬고(무대와 카메라만
  3D, 포켓몬은 80×80 한 장) 그래서 지어내지 않았다. 그런데 무대가 BDSP의 진짜
  3D가 된 지금은 **그 한 장만 화면에서 튄다** — 앞쪽에 선 내 포켓몬이 도트
  덩어리로 크게 뜬다. BDSP romfs에 포켓몬 모델이 `Pokemon Database/pokemons/
  battle/pm0001_00_00` 꼴로 전부 들어 있다 (§16.6).
...
남은 구멍: **포켓몬이 도트 한 장** · 바위 판때기 · 자전거 · 비전기술 필드 사용 ·
PWA 0파일 · 긴 풀숲 인카운터율 70% 미적용(타일 거동값 32종이 아직 안 갈렸다 —
전 오버월드의 1.3%다).

배틀 쪽에서 메운 것: **경험치가 상대 다음 마리보다 먼저 온다**(원작 순서였는데
컨트롤러가 상대 교체까지 삼키고 와서 뒤로 밀려 있었다) · **시합규칙 「교체」**
(§7.7.8) · **트레이너의 가방**(§7.7.7) · **교체 판단**(§7.7.6) ·
**AI 바닥 플래그**(§7.7.5).
...
**포켓몬은 아직 원작 배틀 그림 493종이다.** 3D 모델을 지어내지 않았다
(§16.5·DATA.md §2.17). ⚠️ **무대가 BDSP의 진짜 3D가 된 지금은 그 한 장만
화면에서 튄다** — 앞쪽에 선 내 포켓몬이 3.8m짜리 도트 덩어리로 뜬다.

지어낼 필요는 없다. BDSP romfs에 **포켓몬 모델이 통째로** 있다:

```
AssetAssistant/Pokemon Database/pokemons/battle/pm0001_00_00   종·판·성별
                               pokemons/battle/animations/     동작
                               pokemons/field/                 오버월드용(작다)
                               pokemons/statue/                지하 석상
```

번들 이름이 스스로를 말한다 — `pm{도감:04}_{판:02}_{성별·변형:02}`. 세 자리가
붙어 있어 도감 번호로 바로 찾는다. 배틀 쪽 자료도 이미 뽑아 두었다:
`BattleDataTable.MotionTimingData`에 **종족 1,008마리의 모션 타이밍**이,
`MotionReplaceData`에 종족별 예외 51건이 있다 (§4.3.1).

세우려면 넘어야 할 것 넷:

1. **용량.** 종 하나에 메시 + 뼈대 + 텍스처 + 동작 여러 벌이다. 493종을 다
   구우면 GB 단위가 된다 — 배틀에 나오는 종만, 그것도 배틀이 열릴 때 받아야 한다
   (무대와 같은 방식). 먼저 한 마리를 구워서 **한 마리당 몇 MB인지**를 재야 한다
2. **뼈대·동작.** `bdspGlb.py`가 `SkinnedMeshRenderer` + 뼈대를 이미 다룬다
   (주인공·NPC 45명이 그것으로 섰다). 포켓몬도 같은 길일 것이나 확인이 필요하다
3. **배치와 카메라.** 지금 자리는 원작 DS의 문법(내 것은 앞쪽 왼쪽에 뒷모습,
   상대는 뒤쪽 오른쪽에 작게)이다. 3D 모델은 뒷모습 그림이 따로 없으므로 **한
   모델을 돌려 세운다.** BDSP 자신의 배치(z축 대칭 · 카메라 (−2.7, 0.7, 5.0)
   화각 30)가 §4.3.1에 재어져 있고, 옮기면 샷 다섯을 다시 짜야 한다
4. **크기.** 종마다 실제 키가 다르다. 도트는 80×80 안에서 차지하는 넓이로
   크기를 옮겼는데(`spriteFit`), 모델은 롬의 종족 키를 봐야 한다
...
| ⑤ 포켓몬 | ✅ 원작 배틀 그림 493종. ⚠️ 3D는 아직 (§16.6) |
| 배틀 무대 | ✅ `battleBG`가 BDSP 무대 열일곱 벌을 고른다 (DATA.md §2.17.1) |
