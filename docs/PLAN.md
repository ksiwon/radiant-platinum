...
| | 실측 |
|---|---|
| 코드 | 54,586줄 · 파일 306개 · 시험 파일 107개 / 시험 1,232개 (`src/**/*.ts{,x}`) |
...
타일 거동값 32종이 아직 안 갈렸다(전 오버월드의 1.3%) · 모바일 터치 · KTX2 0개.
...
⚠️ **바위가 판때기다 — 진단만 끝냈다.** 무쇠시티 길바닥의 자갈이 그것이다.
찍어서 확대해 보고 광선으로 되물으니 소품이 아니라 **지형 면에 칠해진 그림**
(`c3_iwa_s`)이었다. 원작 DS는 부감이라 그래도 됐다. 전 오버월드를 재 보면
바위 무늬가 칠해진 바닥 삼각형이 **221개 · 189타일²**뿐이다
(`c3_iwa_s` 48타일² · `himitu_kiwa` 141타일²) — 작다. 그 위에 3D 바위를
세우는 것은 풀숲에 포기를 세우는 것과 같은 일이다.
