...
| | 실측 |
|---|---|
| 코드 | 171,168줄 · 파일 782개 · 시험 파일 263개 (`src/**/*.ts{,x}`) / 시험 **3,314개** (`pnpm test:strict` · 건너뛴 것 3개 — 시험 파일 266개 중 셋은 `tools/`에 있다) |
...
| 자산 | 44그룹 · 파일 11,251개 · 1002.6MB — **포켓몬 3D 672.8**(파일 3,933) · **NPC 모델 147.2**(116벌) · **배틀 무대 128.2**(30벌) · 청크 28.4 · 소리 7.5 · 대사 3.7 · 행렬 3.4 · 소품 2.8 · 주인공 1.5MB (전부 PNG, KTX2 0개). 설치가 **요구하는** 것은 그중 26그룹이다 (DEPLOY.md §5.3 ⑮) |
| 배포물 | `dist` 파일 67개 · **4.5MB**. 첫 화면이 받는 것 351.7KB · **gzip 125.1KB** (목표 135 · 예산 150). 한 번 열 때 실제로 나가는 바이트는 따로 잰다 — 첫 방문 **636kB** · 다시 열기 **0.8kB** (DEPLOY.md §3) |
| 번들 출처 | `battle-sim` 청크 6.55MB → **1.51MB**. 게임 데이터 8,881kB → **0** (`pnpm provenance`: 제3자 정적 게임 데이터 없음, DEPLOY.md §4) |
| 브라우저 실측 | `pnpm e2e` — **PASS 28 · FAIL 0 · BLOCKED 0 · 못 잰 것 0** (DEPLOY.md §5) · `pnpm story` **PASS 70 · FAIL 0** |
| 공개 배포 | `pnpm release:check` **exit 0 · 열린 blocker 0** · `https://radiant.siwon.it.kr/` (DEPLOY.md §1) |
