...
| `src/` 자료 표 심사 | ⚠️ allowlist·배포물 재검사는 됐다. `textBanks.json`의 롬 키 697개는 미해결 |
...
감사를 붙이고 나서야 `src/data/textBanks.json`(뱅크 697개 · 롬 헤더에서 읽은
u16 키 포함)이 보였다. 지금은 `tools/distribution/dataTables.mjs`가 목록을
들고 있고, 목록에 없으면 빌드가 서고, "배포물에 안 들어간다"는 빌드 뒤에
실제로 `dist/`를 뒤져 확인한다 (COPYRIGHT.md §6 · §9).
...
| 배포물 | `dist` 파일 29개 · 12.0MB. 첫 화면이 받는 것 437.2KB · **gzip 141.8KB** (목표 135 · 예산 150) |
| 번들 출처 | `battle-sim` 청크 6.55MB 중 **8,673kB가 `@pkmn/sim` 정적 게임 데이터** — 미해결 (DEPLOY.md §4) |
| 브라우저 실측 | `pnpm e2e` — PASS 14 · FAIL 0 · **BLOCKED 2** (DEPLOY.md §5) |

⚠️ **첫 화면 141.8KB는 부팅 게이트를 포함한 값이다.** `<App />`을 곧바로 그리던
