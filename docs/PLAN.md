...
**제일 큰 구멍은 연출이다.** 디컴프의 필드 스크립트 1,124파일에서 명령이
35,832번 쓰이는데 그중 **75.9%만 실제로 돈다**(서로 다른 명령 692개 중 74개).
안 도는 것을 세어 보면 대부분이 컷신 장치다:

| 명령 | 쓰임 | 없으면 |
|---|---|---|
| `LockAll` · `ReleaseAll` | 2,627 | 대화 중에 딴 사람이 계속 걸어다닌다 |
| `PlaySE` · `WaitSE` · `PlayFanfare` | 1,135 | 문·계단 말고는 효과음이 없다 |
| `WaitFadeScreen` | 430 | 장면이 안 끊긴다 |
| `AddObject` · `RemoveObject` | 430 | 사람이 나타나고 사라지는 연출이 통째로 없다 |
| `SetObjectEventPos` · `Dir` · `MovementType` | 335 | 컷신 자리 배치가 안 선다 |
| `Warp` | 68 | **스크립트가 옮기는 이동이 안 된다** |
...
목도리**다 — 뼈대는 하나도 안 빠진다. 클립 셋을 얹는 데 245KB가 든다.
