...
기획서(`.audit/hard-parts-plan.html`)의 일곱 중 넷이 남아 있었다. 아래는 그중
**지금 안 된 것**만이다. 된 것의 값과 근거는 각 문서에 있다 (NSBCA는
[DATA §2.31](DATA.md), 진화·부화는 [PARITY §3.1·§3.2](PARITY.md), 문은 §8.5,
미리 굽기는 [REPAIR §8.1](REPAIR.md)).

**① 소품 메시가 아직 원작 것이 아니다.** NSBCA 디코더와 소품 애니 목차는 섰고
문의 **길이·갈래**가 원작 클립에서 온다. 그런데 문짝 자체는 우리가 세운 상자다 —
`convertProps`의 `placeByNode`가 노드 변환을 **정점에 발라 버려서** 원작 메시의
노드를 따로 못 움직인다. 여는 자리: 애니 있는 소품 **112개**에 한해
`[노드 → 정점 구간]`을 같이 싣고, 씬에 노드별 행렬을 얹는 얇은 층을 붙인다.
그러면 자전거 비탈(303·304) · 배 컷신(`ship_demo` 모델 넷 + 클립 열여섯) ·
BTP0(텍스처 갈아 끼우기) · BTA0(UV 이동)이 한 번에 열린다.

**② 진짜 렌더에서 뼈 행렬이 남의 버퍼로 간다.** 미리 굽기와 **무관하게**
(끄고도 그대로다) three가 유니폼 그룹을 나눠 쓰는 바람에 뼈 132벌짜리 쓰기가
6·2·9·131벌짜리 버퍼 넷으로 간다. WebGPU가 그 쓰기를 버리므로 그 조각들은 **낡은
행렬로 그려진다.** 잰 값은 REPAIR §8.1에 있다. 공유 캐시 열쇠의 이어 붙이기는
**원인이 아니었다**(시험해 봤다) — 다른 자리를 좁혀야 한다.

**③ 조우 이펙트 · 폼 변화 입자.** 진화·부화와 같은 길이면 된다 — 마디를 재는
순수 모듈 하나와 `SplParticles` 배선. 조우 이펙트는 따로 있는 것이 아니라
`waza` 묶음 안이다(`battle_particles.order` 4~15째가
`encounter_grass_a/b`·`water`·`plain`·`building`·`mountain`). 조우 컷인의
검은 막 자리표시자(`battleScreen.css.ts`)가 그제야 빠진다.

**④ 4.9초짜리 프레임의 임자.** `warpCost.mjs`가 하던 **창 안만 세기**를
`warpGpu.mjs`에 옮기는 것이 첫 걸음이다 (REPAIR §8.1).
