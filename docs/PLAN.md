...
세 페이즈의 완료 조건은 전부 통과했고, 그 상태를 시험 1,238개가 붙들고 있다.
...
시험은 **소스 옆에 둔다**(`*.test.ts`, 107파일 / 1,238개). 따로 `tests/`를 두지
...
야생을 만나 배틀을 끝내고, 저장하고 껐다 켜면 그 자리다. 시험 1,238개가 그것을 묶고
...
| | 실측 |
|---|---|
| 코드 | 55,928줄 · 파일 308개 · 시험 파일 107개 / 시험 1,238개 (`src/**/*.ts{,x}`) |
...
| 자산 | 파일 7,084개 · 658.7MB — **포켓몬 3D 422.7**(493종) · **배틀 무대 134.4**(30벌) · NPC 모델 43.6 · 청크 29.8 · 소리 7.9 · 주인공 5.1 · 행렬 3.5 · 대사 3.5MB (전부 PNG, KTX2 0개) |

⚠️ **삼각형과 드로우콜은 헤드리스에서 재고, 프레임률은 못 잰다.** 헤드리스
크롬에는 WebGPU 어댑터가 없어 WebGL2 + SwiftShader(소프트웨어)로 떨어진다 —
무엇을 GPU에 올렸느냐는 우리가 정한 값이라 그대로지만 프레임 시간은 이 기계의
것이 아니다(같은 자리에서 5~8fps로 찍힌다). 프레임률은 실제 브라우저의
`PerfOverlay`로 본다.
...
남은 구멍: **자전거가 안 그려진다** · 무쇠시티 자갈 48타일²가 바닥 그림이다 ·
**숲 벽(`conttree`) 밑바닥은 아직 옆 타일을 베낀다**(떡잎마을 바닷가 숲이 하얀
모래인 것이 그것이다 — 한 그루짜리 판만 제 바닥 그림을 들고 있다) · 타일 거동값
32종이 아직 안 갈렸다(전 오버월드의 1.3%) · 모바일 터치 · KTX2 0개.
...
⚠️ **자전거가 안 그려진다.** 타면 빨라지고 조우가 잦아지지만(DATA.md §4.2.1)
몸 밑에 자전거가 없다. BDSP의 자전거는 주인공 프리팹 쪽에 붙어 있고 우리
주인공(`dawn.glb`)은 다른 길로 왔다.
...
때까지다. **그다음이 막혀 있다** — 잔모래마을 연구소에서 도감을 받는 대목이고,
거기에 `GivePokedex`가 있다. 자리로는 **딱 한 개**인데 그 뒤 이야기가 통째로
그 뒤에 있다(DATA.md §2.10의 표). 같이 필요한 것이 `OpenPokemonNamingScreen`(5) ·
지역도감 세기 · 무리 포켓몬이다. 라이벌이 뒤를 **따라다니는** 것도 그 뒤
이야기라 아직 없다 (`MOVEMENT_TYPE_FOLLOW_PLAYER`).
...
화면에 실제로 서는 삼각형은 그래서 자리를 크게 안 탄다 (`pnpm shot --perf`):
...
**포켓몬 493종이 BDSP 모델로 선다** (DATA.md §2.17.2). 422.7MB이고 배틀이 열릴
...
열여덟), 색이 다른 개체(`Rare`)의 텍스처, 그리고 종마다 다른 **모션 타이밍**
(`BattleDataTable.MotionTimingData`에 1,008마리분이 있다 — 지금은 우리 박자다).
...
**안 하는 것:** 바닥면(−Y) 채우기. 건물 밑은 볼 자리가 없다.
