...
**⑤ 뼈 행렬이 남의 버퍼로 가는 것의 나머지.** 임자 하나는 잡아 고쳤다 —
`BattleTrainers`가 모델 로더 넷 중 혼자 `unifySkeletons`를 안 불러서 주인공의
`wear` 재질을 뼈 **131벌짜리 몸통과 9벌짜리 신발**이 나눠 썼다(REPAIR §8.1).
배틀 콘솔 오류가 **45 → 9**로 줄었다. 남은 아홉은 **판마다 들쭉날쭉하고**(넷 중
둘은 0) 받는 버퍼의 크기(131·6·2·9벌)가 **씬의 어느 스켈레톤과도 안 맞는다** —
곧 우리 모델이 아니라 three가 바인딩을 나눠 주는 자리다. 재는 자 넷을 남겨
두었다 (`.audit/boneShare.mjs --battle` · `boneWho.mjs --battle` ·
`consoleCount.mjs` · `bufOverflow.mjs`).
