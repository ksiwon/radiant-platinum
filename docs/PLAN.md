...
| 번들 출처 검사 | ✅ 청크별 모듈 출처. ⚠️ 그 검사가 찾아낸 8,881kB를 **1,732kB로** 줄였다. 0이 아니라 blocker 유지 (DEPLOY.md §4) |
| 번들 안의 `eval(` | ✅ **0건.** `>eval` 디버그 명령을 빌드에서 지운다 — CSP를 열 이유가 없어졌다 |
| 앱 셸 그림 심사 | ❌ **새 blocker.** 타이틀 배경에 워드마크, 아이콘에 원작 캐릭터로 보이는 형상 (COPYRIGHT.md §11) |
| 한 번 설치하면 다시 안 묻기 | ⚠️ 구현 완료 · 진짜 바이트로 "다시 안 만든다"까지 실측. **진짜 12/12 완주는 아직** (IMPORT.md §15) |
...
- **번들 안의 제3자 게임 데이터 1.7MB** (DEPLOY.md §4). 8.9MB에서 줄었다.
  다음 조각은 `pokedex.mjs`를 롬에서 주입하는 것이고, 그러려면 몸무게가 필요해
  `text` 그룹 뒤에 온다 — `Low Kick`·`Grass Knot`의 위력이 몸무게로 정해진다.
  **근사해서 넣지 않는다**
...
그 검사가 `battle-sim` 청크의 8,881kB짜리 제3자 게임 데이터를 찾아냈다. 그중
7,149kB를 빌드에서 뺐고 1,732kB가 남았다 (DEPLOY.md §4). 공개 배포 판정은
...
5. ⚠️ Platinum은 그룹 둘(`moves`·`marts`)이 노드 산출물과 바이트로 같다.
   **BDSP는 컨테이너까지다** — UnityFS·LZ4·SerializedFile이 UnityPy와 오브젝트
   208개까지 일치하고, 그 위(타입 트리·텍스처 디코딩·GLB 쓰기)는 아직이다.
   필수 12개 중 2개가 만들어진다 — 최우선 기술 게이트다.
...
| 배포물 | `dist` 파일 30개 · **7.2MB**. 첫 화면이 받는 것 442.1KB · **gzip 144.5KB** (목표 135 · 예산 150) |
...
⚠️ **첫 화면 144.5KB는 부팅 게이트를 포함한 값이다.** `<App />`을 곧바로 그리던
때가 133.3KB였으니 `BootGate`+설치 기록 읽기가 11.2KB(gzip)다 — 이번에 1회 설치
계약(산출물 판 확인·읽을 때 검사)이 2.2KB 더 붙었다. 공개판은 부팅할 때
`install.json`을 읽어야 화면을 고를 수 있으므로 이 비용은 미룰 데가 없다.
이제 `boundary:post`가 `index.html`이 적어 둔 것을 gzip해서 **직접 잰다** —
150KB를 넘으면 빌드가 서고 135KB를 넘으면 경고한다. 지금은 경고 쪽이다.
