...
✅ **용량은 클립을 빼서 풀었다.** **걷기를 `actor/locomotion`이 뼈를 직접 돌려서
만들므로**(주인공도 그렇다) 클립을 안 싣는다 — 지금 106벌에 87.72MiB고, 클립까지
넣으면 234.93MiB다. 대신 서 있는 사람도 매 프레임 돌려야 한다. 안 돌리면 팔을
벌린 T 자세로 선다. 갈래별 값과 **어느 것을 안 굽기로 했는지**는
