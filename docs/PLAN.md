...
| | 실측 |
|---|---|
| 코드 | 189,279줄 · 파일 861개 · 시험 파일 299개 (`src/**/*.ts{,x}`) / 시험 **3,805개** (`pnpm test:strict` · 건너뛴 것 3개 — 시험 파일 305개 중 여섯은 `tools/`에 있다) |
...
| 자산 | 44그룹 · 파일 11,251개 · 1002.6MB — **포켓몬 3D 672.8**(파일 3,933) · **NPC 모델 147.2**(116벌) · **배틀 무대 128.2**(30벌) · 청크 28.4 · 소리 7.5 · 대사 3.7 · 행렬 3.4 · 소품 2.8 · 주인공 1.5MB (전부 PNG, KTX2 0개). 설치가 **요구하는** 것은 그중 28그룹이다 — Platinum 스물넷 · BDSP 넷 (`import/install/required.ts` · DEPLOY.md §5.3 ⑮) |
| 배포물 | `dist` 파일 90개 · **4.9MB**. 첫 화면이 받는 것 19개 · 351.1KB · **gzip 124.7KB** (목표 135 · 예산 150). 한 번 열 때 실제로 나가는 바이트는 따로 잰다 — 첫 방문 **636kB** · 다시 열기 **0.8kB** (DEPLOY.md §3) |
...
| **모델이 서는 배치** | **1,895개 (53.3%)** — 사람 그림만 보면 **100%** · 116벌 147.2MiB |
...
✅ **용량은 갈래를 갈라서 풀었다.** 치비(`fc*`) 17벌은 클립을 하나도 안 싣고,
등신(`tr*`·`pc*`) 89벌은 **배틀에서 이어 붙는 셋만** 싣는다
(`engine/actor/npcModels`의 `TRAINER_CLIPS` — 굽는 쪽 둘이 이 규칙 하나를 같이
본다). 주인공 둘만 거기에 필드 동작 열여섯을 얹는다.
실측 **116벌에 147.2MiB**다. 106벌로 잰 자리에서 클립을 하나도 안 실으면
87.72MiB · 여덟을 다 실으면 234.93MiB였다. 배틀 클립 합계는 264개다 — 89벌 중 86벌이 셋을 다 갖고,
...
| | 크기 | 다음에 할 일 |
|---|---|---|
| **포켓몬 이로치·암컷 외형** | 3,375개 · **231MB** | **2026-08-19 보류하기로 정했다.** 이로치 텍스처 3,186 · 암컷 프리팹 188 · 목차 1을 굽는 **브라우저 변환기가 없다** — 노드 추출기만 만든다. 그래서 설치본에서는 이로치를 잡아도 배틀에 **평범한 색으로 선다** (색 판정 자체는 원작대로 돈다 — `isShiny`). 굽기로 하면 설치 총량이 약 3분의 1 늘어 932MB가 된다 ([DEPLOY.md](DEPLOY.md) §5의 ⑮가 지금 값의 임자다). ⚠️ **조용하지는 않게 해 뒀다** — `monVariants`가 **변환기 없는 그룹**으로 목록에 올라 있어(`blockedBy`) 설치 화면의 「아직 안 옮긴 변환」에 이유와 함께 뜬다. 그래도 배틀 화면 자체는 아무 말도 안 한다 — `scene/battle/monModel.ts`의 `loadMonVariantIndex`가 `.catch`로 빈 목차를 쓰고 평범한 색으로 세운다. ⚠️ **되살리는 법이 바뀌었다** — 이제 그룹이 있으므로 `convert`를 달아 주면 되고, `npcModels`의 `GROUP_FORMAT`을 올릴 필요가 없다(그 그룹은 바이트가 안 바뀐다). 이미 깔린 사람이 자동으로 받게 하려면 `REQUIRED_BDSP_GROUPS`에 넣는다 — 그러면 `boot`이 「필수 그룹이 빈다」로 보고 설치기를 부르고, 온전한 그룹은 그대로 건너뛴다 (`import/install/required.ts` · `installer.ts`의 `resumableGroups`) |
...
| 아무도 안 부르는 `export` | **60** (값 54 · 타입 6) | 세는 자는 `.audit/deadExports.mjs`(타입스크립트 파서로 훑는다). 기계가 할 수 있는 자리는 끝났다 — 제 파일 안에서만 쓰는 `export` 481개는 낱말을 뗄고(`pnpm exports:check`가 다시 자라는 것을 막는다), 이름이 어디에도 안 나오는 89개 중 **근거가 선 29개를 지웠다**. ⚠️ **남은 60개는 쓸어 담으면 안 된다** — 일부러 둔 빈 껍데기(`FormatsData`, [DEPLOY.md](DEPLOY.md) §4) · **만들어 낸 파일**(`DECIBEL`·`POKETCH_MAP_STEP`) · 사흘 안에 들어온 손대는 중인 것 셋 · 아직 안 끝난 BDSP 브라우저 변환의 발판과 조사 도구용 여섯이 섞여 있다. **구현해 놓고 안 이어 붙인 다섯은 [REPAIR.md](REPAIR.md) §10으로 옮겼다.** ⚠️ **시험만 쓰는 `export` 557개는 흙이 아니다** — 순수 함수를 시험이 직접 부르라고 연 자리고, 검사가 시험을 소비자로 안 세면 그것이 다 걸려서 못 쓴다 |
