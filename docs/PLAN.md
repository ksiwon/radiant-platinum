...
세 페이즈의 완료 조건은 전부 통과했고, 그 상태를 시험 607개가 붙들고 있다.
...
| 배틀 HUD · 기술 선택 · 배속 | ✅ |
| **포켓몬 모델** | ❌ `capsuleGeometry`다 |
| **기술 연출 (VFX 아키타입 5종 · 카메라 샷 8종)** | ❌ 글만 찍는다 |

### Phase 4 — 시스템 완성

| | 상태 |
|---|---|
| 파티·박스·가방·도감·트레이너 카드·상점·설정 | ✅ X 키로 열린다 |
| 트레이너 배틀 · 배지 · 기술머신 | ✅ |
| **오디오 (`NitroSynth`, §4.5)** | ❌ 언락 게이트 21줄이 전부 |
| **낮/밤 사이클** | ⚠️ `gameHour`는 도는데 하늘·조명이 안 받는다 |
| 비전기술 필드 사용 (거합베기·괴력) | ❌ |
| 모바일 터치 컨트롤 · 키 리맵 UI | ❌ |
| PWA (`vite-plugin-pwa`) | ❌ |
| 접근성 최소 세트 (§12.1) | ⚠️ 일부 |
...
- 나머지 신오지방 · 종 추가 · 스토리 스크립트 (오프닝에 남은 명령 46개는
  DATA.md §5에 세어 뒀다)
- 도구화: 맵 에디터, 스크립트 에디터
...
시험은 **소스 옆에 둔다**(`*.test.ts`, 54파일 / 607개). 따로 `tests/`를 두지
않는 이유는 옮길 때 같이 안 옮겨져서 조용히 죽기 때문이다.

⚠️ `tools/preview/`는 **게임이 쓰는 코드가 아니다.** 브라우저 자동화가 없어서
`scene/`의 규칙을 옮겨 적고 소프트웨어로 래스터라이즈해 PPM을 뱉는다. 여기서
맞다고 게임에서 맞는 것은 아니다 — 모양과 크기를 눈으로 재는 용도다. 그래도
필요하다: 소품 뒷면을 처음 시도했을 때 수치는 맞고 화면은 틀렸는데, 그것을
잡은 것이 이 도구다 (DATA.md §2.2).
