...
| 배틀파크에서 한 칸도 못 간다 | 1칸 | 확인 지점 `battlepark`(맵 322 · 칸 20,33)에서 네 방향이 다 안 나간다. **입력은 닿는다** — 왼쪽을 누르면 바라보는 각이 −1.57로 돌고 x가 20.50 → 20.42로 8/100칸 밀리다 멎는다(`bumpDir`은 −1이라 부딪힘 판정도 아니다). 격자는 남쪽이 열려 있다고 말한다(`isBlocked` 거짓 · 높이 둘 다 1)고 사람도 스크립트도 없다. 재려면 `node .audit/parkProbe.mjs` |
