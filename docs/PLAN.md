...
⚠️ **밤에 빛을 줄이면 사람도 같이 사라진다.** 심야의 몸빛은 낮의 **26%**까지
떨어져 실루엣이 배경에 묻혔다(램버트 어림, `fx/sky`의 `bodyLight`). 그래서 인물을
따라다니는 점광원을 붙이고 사거리를 4m로 끊는다. 발밑에 얕게 번지는 몫은 원래
밝기의 22%라 손전등이 아니라 달빛 웅덩이로 읽힌다.
...
⚠️ **화면에 뜨는 이름은 전부 이것이지만 저장소 키는 아니다.** IndexedDB
데이터베이스 `pt-3d`와 localStorage 키 `pt3d.options`는 그대로 둔다 — 바꾸면 이미
저장된 리포트와 설정을 못 찾는다. 옮기려면 옛 키를 읽어 새 키로 나르는 절차가
따로 필요하다.
...
**다음 칸은 NPC 3D 하나다.** 그것이 BDSP 경로에 걸려 있고, 그 경로가 이제
막혀 있지 않다 — nsp가 `raw/roms/`에 키까지 있고 `nstool`과 `UnityPy`가 깔렸다.
남은 것은 romfs를 풀어 번들에서 모델·애니메이션을 꺼내는 일이다.
