...
포함:
- 떡잎마을 · 201~204번 도로 · 잔모래마을 · 축복시티 · 무쇠시티와 그 체육관 · 예지호수
...
**뱅크 순서는 지역판마다 다르다** (us 724 / ko 714 / ja 709개). 같은 인덱스에서 엔트리 수가 일치하는 비율은 us↔ko 10%, ko↔ja 63%에 불과하다. **인덱스로 텍스트를 참조하면 로케일을 바꾸는 순간 엉뚱한 데이터가 나온다.** 게임 코드는 반드시 의미 이름으로 참조하고, `src/import/platinum/textBanks.ts`가 로케일별 인덱스로 옮긴다 — 표를 들고 있는 것이 아니라 **이름 순서에서 자리를 계산하고 사용자의 롬으로 검산한다** (COPYRIGHT §6).
...
값은 `src/import/platinum/textBanks.test.ts`가 고정한다(뱅크 수 724·714·709). 판별 휴리스틱을 개선하다 이 값이 흔들리면 그건 개선이 아니라 회귀다.
...
**아트 방향 결정: 풀 프로포션(등신)으로 통일한다.** BDSP 혹평의 대상은 오버월드 치비 모델이고, 포켓몬 모델(Pokemon-3D-api, 본가 SM/SwSh 계열)·3인칭 추적 카메라(§6.2)와 정합하는 건 등신이다. BDSP에는 배틀(VS)용 풀 프로포션 모델이 전 트레이너 클래스에 별도로 존재한다 — DS 스프라이트를 충실히 3D화한 디자인이라 플래티넘 리메이크에 가장 정통성 있는 소스다.
...
| 일반 주민 · 엄마 | `persons/field/fc####_00` (치비) | ✅ 배틀 몸이 없는 **열일곱**만 굽고 비율을 맞춰 세운다 (아래) |
| 마박사 | BDSP의 `doctor00~02` 중 어느 것인지 못 짚었다 | ✅ 인트로는 신사(`tr0046_00`) 대역 (3D_GAP_AUDIT §3.1) |

`tr####`가 어떤 인물·직업군에 대응하는지는 **번들 안 텍스처 이름**으로 붙였다 — 156종 중 46종이 붙는다 (§16.9).

⚠️ **배틀 몸이 아예 없는 사람이 있어서 치비 번들을 그대로는 못 버린다.** 굽힌
116벌 중 스물일곱이 필드 번들뿐이고 엄마·간호사처럼 대부분 이야기 인물이라,
판때기로 되돌리면 오프닝 바로 다음 장면이 종잇장이 된다. 그 스물일곱은 **머리뼈를
0.2로 줄이고 늘어난 몫을 키에만 주어** 등신 계통에 맞춰 세운다 —
`engine/model/chibi`, 근거와 실측은 [DATA](DATA.md) §2.16. **치비 비율 그대로
서는 사람은 하나도 없다.**
...
- **`_SkinColor`는 피부 전용이 아니라 세 번째 범용 레이어 색이다.** 가방엔 노랑(#eedfa7)이 들어간다
- **레이어 색은 감마(sRGB)다.** 선형으로 보고 그대로 곱하면 색이 통째로 바랜다. 주인공 `pc0001`로 재면 모자 `_PrimaryColor` (0.750, 0.364, 0.274)가 선형으로는 살구색 (224, 162, 143) · 감마로 보면 붉은색 (191, 93, 70)이고, 머리 `_PrimaryColor` (0.240, 0.275, 0.370)는 회백색 (137, 145, 166) ↔ 남색 (61, 70, 94), 피부 `_SkinColor` (0.949, 0.769, 0.664)는 거의 흰색 (252, 227, 212) ↔ (242, 196, 169)로 갈린다. 셋 다 뒤쪽이 원작이다 — 주인공은 **붉은 모자에 남색 머리**다. 앞쪽으로 구운 화면에서는 살색 모자가 머리를 덮어 **민머리로 보였고**, 확인 지점 86곳 중 56곳에서 그렇게 찍혔다
...
- **얼굴이 유난히 하얀 사람이 있는데 굽는 쪽의 결함이 아니다.** 얼굴 앞면(법선 +Z) UV만 찍어 재면 흰 화소가 `tr1086_00` 43.4% · `tr1040_00` 38.0% · `tr1025_00` 32.3%로, 보통 사람(들국화 3.2 · 곤충소년 4.9 · 신사 0.0%)의 열 배쯤이다. **원본이 그렇게 칠해져 있다** — 앞의 둘은 face `_MaskTex`의 **60%가 검정**이라 그 자리에 틴트가 아예 안 들어가고 `_MainTex`의 회색 음영이 그대로 남고(다른 사람은 16~34%), 셋째는 마스크는 25%만 검지만 `_SkinColor`(0.851, 0.787, 0.749)가 거의 무채색이다. **이 수를 이상 징후로 다시 읽지 않는다** — 한 번 그렇게 읽고 원인을 찾다가 헛돌았다
...
지금 선 렌즈 셋 (`engine/actor/camera.ts`):

| 상황 | 프리셋 | 왜 |
|---|---|---|
| 필드 기본 | dist 8 · height 4 · fov 55 | 온 신오를 이 렌즈로 본다 |
| **방 안** (`mapType` 4·5 또는 `camera` 4) | 빗변 6.363 · 각은 **맵마다** · fov 55 | 원작 필드 카메라 갈래의 **내림각 그대로** (`FIELD_CAMERA_PITCH`) |
| 깨어진 세계 | dist 21.436 · height 35.748 · fov 8.0914 | 원작 필드 카메라 값 그대로 (PARITY §6.10) |
...
실내 정사영(4번) 50.0867도가 300개로 제일 많고, 검은겨울 체육관은 9번
40.5890도, 장막 체육관은 10번 60.8038도, 들판 체육관은 1번 68.3679도다.
거리·화각·투영은 안 따라간다(97칸 정사영은 우리 55도 원근과 안 섞인다) —
**빗변 6.363칸을 그 각으로 돌려** 거리와 높이를 낸다.

⚠️ **`mapType`만 보면 안 된다.** 구저택(맵 296)은 `mapType` 3(굴)인데
`camera` 4다 — 둘 중 하나라도 방이면 방으로 본다. 닿는 맵이 357개다
(둘 다 277 · `mapType`만 57 · `camera`만 23).

⚠️ **방 안에서는 겨눔점을 앞으로 민다** (`aimPitch`). 건물에 들어서면 주인공이
늘 앞벽에 붙어 서므로 카메라가 갈 자리는 그려진 바닥 밖이다 — 스무 곳 전부
...
(맵 209)에서 6×4 조각이 뽑혀 겨눔이 16칸 밀렸고 화면의 89.3%가 검었다.
물리면 41.6%다.
...
그려진 바닥을 통째로 감싸면 방에서 떨어진 바닥 한 칸이 상자를 부풀린다 —
연고시티 체육관 문 방이 475칸 대 100칸이었다. **걸어 다닐 수 있는 칸으로만**
번져 가르고(벽 밑에도 바닥이 깔려 있어서 「이어진 바닥」만으로는 안 갈라진다),
상자끼리 겹치므로 주인공이 든 것 중 **제일 작은 것**을 쓴다 (REPAIR §13).

⚠️ **주인공이 화면 가운데에서 벗어난다. 그것이 맞다** — 원작도 맵 경계에서는
가운데가 아니다 (`field_camera.c`). 가운데를 지키려면 방 밖을 보여 주는 수밖에 없다.
...
**마우스가 꼭 필요한 자리는 1인칭 시선뿐이다.** 나머지는 전부 키보드로 된다 —
그것이 규칙이고, 새 화면을 낼 때 지켜야 하는 선이다. 3인칭은 카메라가 북쪽에
고정이라 돌 것이 없고, 시점 전환도 `V`가 한다(휠은 덤이다). 클릭은 **덧붙임**이지
유일한 길이면 안 된다.

실측으로 두 자리가 막혀 있었다:

- **인트로의 몬스터볼.** 원작이 터치 화면이라 볼을 눌러야 열리고 키를 누르면
  "그 버튼이 아니고"라고 나무란다. 그 말은 그대로 두되, 한 번 나무란 뒤에는
  키로도 열리게 했다 — 여기서 막으면 마우스 없이 게임을 **시작조차** 못 한다.
- **별명 화면의 "그대로 두기".** 글을 치는 자리라 `useMenuKeys`를 못 쓴다
  (X·Backspace를 가로채면 이름에 x를 못 넣고 지우지도 못한다). Esc 하나만
  따로 듣는다 — Enter로 결정, Esc로 통과.

`.audit/probe/introKeys.mjs`가 타이틀 → 인트로 → 오버월드를 **클릭 0번**으로
지나간다. 시작 메뉴·가방·파티·상자·상점·배틀은 애초에 `<div>`라 클릭할 것이
없고 커서가 전부다.
...
#### 7.2.4b 등판은 하나씩 선다 — 프레임 수도 차례도 원작 것이다

등판 박자의 쉼이 **0**이었다. 재생기는 「글도 쉼도 없는 박자」를 같은 프레임에
이어 붙이므로, 앞의 글이 하나라도 비면 두 마리가 **한 프레임에** 섰다 — 무대가
다 서고 열리게 바꾸고 나서(§7.4의 `sceneReady`) 그 겹침이 눈에 드러났다.

원작이 자리마다 프레임을 적어 두었다:

| 자리 | 프레임 | 원작 |
|---|---:|---|
| 글 하나 | 30 | `WaitButtonABTime 30` |
| 야생이 서 있는 채로 도는 조우 연출 | 122 | `PlayEncounterAnimation` → `WaitTime 122` |
| 여는 등판 — 우리 | 96 | `ThrowPokeball PLAYER` → `PokemonSlideIn` → `WaitTime 96` |
| 여는 등판 — 상대 | 112 | 트레이너전의 `PokemonSlideIn ENEMY` → `WaitTime 112` |
| 판 도중 교체 | 72 | `PokemonSendOut` → `WaitTime 72` (다섯 subscript가 같다) |

차례도 원작이 정한다 — **상대가 먼저다.** 야생은 `SetPokemonEncounter ENEMY`가
글보다 앞이라 화면이 열릴 때 **이미 서 있고**(그 자리만 몸이 글보다 먼저다),
트레이너전은 `PrintFirstSendOutMessage ENEMY` → 던지기 → 그 다음이 우리 차례다.
sim은 `|switch|`를 **우리 쪽부터** 내놓으므로 여는 등판 묶음만 뒤집어 준다
(`battleStore`의 `foeFirst`). ⚠️ **등판이 0번이 아니다** — 줄기는 `start`로
열린다(실측 `start | switch:p1a | switch:p2a | turn`). 0번부터 세면 그 함수가
아무것도 안 하고 조용히 지나간다.

#### 7.2.5 배틀은 **무작위 1,500판**으로 담근다

손으로 고른 판으로는 못 잡는 고장이 있다. `controller.test`가 보는 것은 우리가 미리 아는 상황(모부기와 찌르꼬, 3대3, 볼 하나)인데, 실제로 사용자가 만나는 배틀은 493종 × 레벨 × 특성 × 도구 × AI 조합이라 한 조합에서만 서는 버그는 그 목록에 영영 안 걸린다.

그래서 `sim/soak.test`가 **무작위로 짜고 무작위로 두고 불변식을 잰다.** 무엇이 나올지는 모르지만 "이런 일은 절대 없어야 한다"는 안다:

① 예외가 안 난다 · ② 고를 것이 없는데 안 끝난 상태(교착)가 0 · ③ 돌려준 사건만 다시 접어도 같은 화면이 나온다 · ④ 박자로 펴도 안 터진다 · ⑤ 체력·PP가 범위를 안 벗어난다 · ⑥ 빈 턴 칸이 화면에 안 샌다 · ⑦ 끝났으면 `finish`가 정해진다 · ⑧ 화면에 영어가 안 뜬다

기본은 24판이라 `pnpm check`에 12초를 더한다. `BATTLE_SOAK=1500`으로 늘려 돌린다 — 10분이다. 실패는 **씨앗과 함께** 적으므로 숫자 하나로 같은 판을 다시 굴릴 수 있다.

**첫 실행에서 다섯 갈래가 나왔다.** 전부 화면에서 배틀이 통째로 서거나 없는 기술이 뜨는 고장이고, 모두 제 자리의 시험으로 옮겼다(`controller.test`의 「담금질이 잡은 자리」):

| | 무엇이 보였나 | 왜 |
|---|---|---|
| 빈 턴 칸 누출 | 교체 화면의 잉어킹이 `물장구·몸통박치기·물장구` 세 칸 | 빈 턴 칸은 **우리 팀 전원**에 붙는데 나와 있는 한 마리만 가렸다 |
| 교착 ① | PP가 다 떨어진 상대가 굳어 명령 창이 영영 안 뜬다 | 빈 턴 칸에 PP가 남아 sim이 **발버둥을 안 준다** — 그 칸은 화면·AI 양쪽에서 걸러지므로 고를 것이 0이 된다 |
| 교착 ② | 쓰러진 턴에 볼을 던지면 배틀이 선다 | 그 턴의 요청은 `forceSwitch`라 sim이 기술 명령을 거절하는데, 우리는 이미 요청을 비운 뒤다 |
| 교착 ③ | 참기·역린 다음 턴에 도망치면 배틀이 선다 | 묶인 턴의 요청에는 기술이 **하나뿐**이라 `move 5`가 "그런 기술 없다"로 거절된다 |
| 재생 불일치 | 잡거나 도망쳐도 재생기가 접은 화면은 안 끝난 상태 | `ball`·`escape`를 뷰가 안 접었다 |

고친 방향은 하나다: **빈 턴 칸의 PP를 늘 0으로 눕혀 두고 쓸 때만 한 칸 세운다**(`session.lowerIdle`·`useIdle`). 그러면 sim이 그 칸을 처음부터 없는 것처럼 다뤄서 PP가 떨어진 순간 원작처럼 발버둥이 나온다. 못 세우는 턴(강제 교체·묶임·도발)은 `canSpendTurn`이 미리 닫고, 화면의 가방·도망 칸도 그 값을 보고 함께 잠근다 — 눌러도 아무 일이 없는 칸을 남기지 않는다.
...
**한 자리에 선다.** 무대 전체가 늘 보이고, 움직이는 것은 포켓몬과 기술 연출뿐이다.

⚠️ **한동안 샷 여섯을 컷으로 이었다.** 기술을 쓰면 어깨 너머(`oncoming`), 맞으면
클로즈업(`impact`), 쓰러지면 로우앵글(`faint`), 교체하면 회전(`switchIn`) —
6세대 이후 본가의 문법이다. 배율이 샷마다 3.1배로 갈리던 것을 `SHOT_REACH`
하나로 못 박아 고친 뒤에도 **한 턴에 컷이 서넛**이라, 플레이해 보면 무대가
아니라 카메라가 먼저 보였다. 원작 DS는 카메라가 아예 안 움직이고 BDSP도
배율까지는 안 흔든다. 그래서 컷을 통째로 걷어냈다 (`engine/battle/shots.ts`의
`CAMERA` 하나가 남았다).

남은 움직임은 둘이다 — **기술 대본이 시키는 화면 흔들림**(`Func_ShakeBg`가 적힌
기술 서른 개)과, 좁은 무대에서 카메라를 당기는 `cameraFit`이다.

**자리와 렌즈는 BDSP 값이다** (`BattleDefaultPlacementData`, §4.3.1): 카메라
(−2.7, 0.7, 5.0) · 화각 30 · 포켓몬 (0, 0, ±2.2). 우리가 바꾼 것은 **높이
하나**(0.7 → 1.5, 겨누는 곳 0.4)다. BDSP는 눈높이에서 수평으로 보는데 그러면
뒤에 선 상대가 화면 한가운데(960×640에서 y 398)에 떨어져 **내 체력판에 가린다**
— 우리 체력판은 원작 DS 배치(상대 왼쪽 위 · 나 오른쪽 아래)라서다. 조금
내려다보면 내 것 (158, 433) · 상대 (643, 293)으로 둘 다 판을 안 문다. 2.2까지
올리면 이번에는 하늘과 나무가 화면 밖으로 밀린다.

설정의 **배틀 애니메이션**은 기술 연출과 조우 입자에 걸린다. 카메라는 이제 켜든
끄든 같은 자리다.
...
#### 7.7.5 BDSP가 제 강자에게 주는 비트를 바닥으로 깐다
...
것이 이 프로젝트의 선택이고, 기준은 지어내지 않고 **자료가 제일 센 상대에게
실제로 켜 주는 값**을 쓴다.

그 값이 플래티넘 롬에서는 난천의 7이고, **같은 신오를 다시 만든 BDSP에서는
111**이다. BDSP 덤프(`Dpr/masterdatas`)의 `TrainerTable.TrainerData` 707줄에
`AIBit` 칸이 있고 분포가 이렇다:

| AIBit | 명 | 누구 |
|---:|---:|---|
| **111** = `0b110_1111` | **77** | 관장 여덟 · 사천왕 넷 · 챔피언 · 갤럭시단 · 라이벌 · 타워타이쿤 다섯 · 재대결판 · 모리모토 |
| 107 | 6 | 위에서 EXPERT만 빠진 값 |
| 25 · 11 | 1 · 3 | |
| **9** = `BASIC\|SETUP_FIRST_TURN` | **619** | 길에서 만나는 전부 |
| 8 | 1 | |
...
일을 다 말하지 않는다 — 본체는 「데미지를 안 내는 기술 전부에 92%로 +3」이고,
배턴터치를 아는지는 그 앞의 문지기일 뿐이다(모르면 31.25%로 그냥 끝낸다).
그래서 이 비트가 켜진 상대는 셋업·상태이상·방어를 훨씬 자주 고른다.

바닥을 까는 자리는 `sim/brain.ts` 하나뿐이다 (`ai/score.BDSP_TOP_FLAGS`).
자료에 더 켜져 있으면 그쪽을 남긴다 — 플래티넘에만 있는 `RISKY`가 그렇다.
...
- **더블의 `TAG_STRATEGY`(0x80).** 자료는 아무도 안 켜는데 **원작이 켠다** —
  더블 배틀이면 `TrainerAI_Init`이 마스크에 얹는다(`trainer_ai.c` 254줄).
  루틴이 1,000줄이고 짝을 돕는 갈래(도우미·짝을 비켜 때리기)가 거기 다 있다.
  안 옮긴 채로 짝을 후보에 남기면 **자기편을 때리므로**, 지금은 짝을 겨누는
  후보를 AI에서 뺀다 (`sim/brain.choices`).
...
- **`CHECK_HP`(0x100) · `WEATHER`(0x200) · `HARASSMENT`(0x400).** 두 롬 어느 쪽도
  한 번도 안 켠다. 플래티넘 928명의 `ai` 값은 0·1·3·5·7·15·17·33·35 **아홉
  가지뿐**이고(제일 큰 것이 35라 0x40 위가 통째로 없다), BDSP 707명은
  111·107·25·11·9·8 여섯뿐이다(§7.7.5). 옮길 자리가 없다.

#### 7.7.10 검증은 승률로 한다

점수 단위 테스트가 다 맞아도 배선이 어긋나 있으면 AI는 아무 일도 안 한다 — 배틀은 멀쩡히 돌아가고 아무도 모른다. 그래서 **같은 파티·같은 씨앗으로 붙여서 승률을 잰다.** 무작위로 두는 쪽이 5할(기준선 18/40), AI가 8할(31/40)이다. 이 차이가 사라지면 실패다.

**바닥을 올릴 근거도 여기서 나온다.** BDSP가 111을 켠다는 것은 실측이지만, 그 값이 **우리 엔진에서도** 더 잘 두는지는 다른 질문이다. 같은 잣대(탐욕)를 상대로 나란히 돌린 실측이 **111 → 39/40(97.5%) · 7 → 40/40(100%)**이고, 무작위를 상대로는 **31/40 대 32/40**이다. 판 하나 차이 — **더 세지지도 더 약해지지도 않았다.** 얹은 셋이 하는 일은 세기가 아니라 **성격**이다(변화기·셋업을 훨씬 자주 고른다). 이 두 수가 떨어지면 바닥을 되돌려야 한다는 뜻이고, 그때 고칠 것은 시험의 수가 아니라 바닥이다.
...
슬롯은 **리포트 한 벌**이다. `report()`를 부른 시점만
...
타이틀과 설정에도 수동 “세이브 파일 내보내기”와 “세이브 파일 불러오기”를 항상 둔다.
...
개발 서버에서 상시 오버레이(`ui/hud/PerfOverlay`, 배포본에는 안 실린다): FPS · 프레임 타임 p50/p99 · 드로우콜 · 삼각형 · **구간마다의 최장 프레임**. 재는 자는 `engine/loop/frameStats`고, 게임 루프가 프레임마다 **자르기 전의 간격**을 민다.

⚠️ **평균 FPS는 여기서 답이 아니다.** 예순 프레임 중 쉰아홉이 16.7ms고 하나가 300ms면 평균은 21.4ms(「FPS 47」)로 읽혀 별일 아닌 것처럼 보이는데, 사람이 느끼는 것은 그 하나뿐이다. 그래서 임자 값이 **p99**다 — 백 프레임 중 제일 느린 하나가 얼마나 느린가.

⚠️ **「1% low」라는 이름에 셈이 둘 있다** — 백분위와 「제일 느린 1%의 평균」이다. 두 값이 다르므로 밝히지 않으면 남의 수와 못 견준다. **우리는 백분위다** (`frameStats.percentile`).

⚠️ **루프 시간이 아니라 프레임 간격이다.** `gameLoop.stats.frameMs`는 우리 코드가 쓴 시간이고, 드라이버가 셰이더를 굽느라 멎은 3.3초는 거기 안 잡히고 간격에만 잡힌다 (REPAIR §8).

#### 실기 기준선 — **아직 안 받았다**

⚠️ **헤드리스로는 못 잰다.** 헤드리스 크로미움이 WebGPU 장치를 못 만들어 렌더러가 WebGL2/SwiftShader(소프트웨어 래스터라이저)로 내려앉는다 — `pnpm shot`이 FPS 4~5를 찍는 것이 그 까닭이고, 그 수는 사용자 기계의 수가 아니다 (DEPLOY.md). 그래서 **사람이 제 기계에서 읽어 주기 전까지 이 표는 빈다.**

받는 법: 개발 서버에서 얼마쯤 걸어 다닌 뒤 콘솔에서 `pt.perf()`. 켠 직후의 로딩을 빼고 재려면 `pt.perfReset()` 먼저.

| 무엇 | 어디서 나오나 | 예산 |
|---|---|---|
| 프레임 시간 p50 | `pt.perf().p50` | *받으면 적는다* |
| 프레임 시간 **p99** | `pt.perf().p99` | *받으면 적는다* |
| 드로우콜 | `pt.perf().calls` | *받으면 적는다* |
| 삼각형 | `pt.perf().tri` | *받으면 적는다* |
| 맵 전환 최장 프레임 | `pt.perf().spans['맵 전환'].worst` | *받으면 적는다* |
| 첫 배틀 진입 히칭 | `pt.perf().spans['첫 배틀'].first` | *받으면 적는다* |

⚠️ **첫 배틀은 「처음 한 번」이 임자다** — `@pkmn/sim`이 그때 처음 들어오고(§7.5.1) 무대·모델도 그 판에 처음 굽는다. 두 번째부터는 다 캐시라 **다른 것을 재는 값**이다. 맵 전환은 반대로 **제일 나쁜 번**이 임자다(어느 맵이 아픈가).

**성능 회귀는 CI에서 못 잡는다** — 위와 같은 까닭이다. 잡으려면 실기가 도는 자리가 있어야 하고, 그 전까지는 **사람이 읽어 주는 수**가 유일한 기준선이다.
...
| | 상태 |
|---|---|
| 프로덕션 금지 산출물 검사 | ✅ 빌드 앞뒤로 선다. 642.0MB · 7,110개가 나가던 것을 액수 목록으로 줄였다 — 지금 크기는 [DEPLOY.md](DEPLOY.md) §3이 임자다 |
...
| Platinum 브라우저 변환 | ✅ **그룹 스물아홉이 다 돈다.** 그중 **스물셋**이 노드 산출물과 바이트(그림은 픽셀)로 맞대어져 있다 — 남은 여섯은 `text`·`species`·`sound`·`encounters`·`trainers`·`spawns` |
...
| 설치 상태 구분 | ✅ `installing`/`partial`/`ready`. 필수 29그룹이 다 있어야 `ready` |
...
| Import Wizard | ✅ Worker로 돈다. 진짜 롬과 `AssetAssistant`만으로 필수 그룹을 전부 옮기고 `ready`가 된다. **잰 수는 [DEPLOY.md](DEPLOY.md) §5의 ⑮가 임자다** |
...
지금은 **스물아홉이 다 구현돼 있어 막힌 줄이 없다** — 남으면 그 자리에 왜
막혔는지를 적고, 시험이 그 이유가 비어 있지 않은지 센다(`convert.test.ts`).
...
| 비전기술 필드 사용 | ✅ **아홉이 다 나간다** — 풀베기·공중날기·파도타기·괴력·안개제거·바위깨기·폭포오르기·락클라임·플래시. 이름은 롬에서 읽은 것이다(`names/moves.ko.json`). 조건만 우리 것이고 그 뒤는 원작 `FIELD_MOVES` 스크립트가 한다 (PARITY §1.8) |
| 키 리맵 UI | ❌ |
...
### 15.1 시험용 손잡이 — **개발 서버에서만**

만든 것을 눈으로 확인하려고 매번 처음부터 걸어갈 수는 없다. 세 가지를 열어 둔다.

| 손잡이 | 여는 법 | 하는 일 |
|---|---|---|
| **확인 지점** | **` (백틱)** — 타이틀·게임 중 어디서나 | 이야기 순서로 늘어놓은 86개 지점. 고른 자리로 순간이동 + 그 자리의 파티·가방·소지금·배지·도감까지 채움 |
| 계기판 | 왼쪽 위에 저절로 | FPS·프레임 시간·드로우콜·삼각형·백엔드 |
| 개발 콘솔 | `window.pt` | 트레이너 찾기·배틀 열기·파티 넣기·회복 · `probe()`로 지금 자리·시점·프레임 읽기 · `view()`·`look()`으로 시점 돌리기 |

인트로 건너뛰기는 따로 두지 않는다. 타이틀에서 확인 지점을 열면 마박사 대목(45줄)을
**같은 이름으로** 건너뛰고 고른 자리에서 시작하므로, 손잡이 하나가 그 일을 겸한다.

**켜는 법이 없다.** 조건이 `import.meta.env.DEV` 하나고, 개발 서버(`pnpm dev`)에서는
늘 참이다. 빌드 상수라 배포 빌드에서는 rollup이 이 가지를 통째로 흔들어 내서
**조각이 아예 안 나온다** — 안 받는 것과 안 실리는 것은 다르다. 실측으로
`dist/assets/`의 조각 62개에 `확인 지점`·`CHECKPOINTS`·`installDevConsole`·
`devWarp`·`PerfOverlay`가 **한 건도 없다**.

⚠️ **주소로 켜는 손잡이를 두지 않는다.** 켜는 값이 런타임 값이 되는 순간 rollup이
가지를 못 흔들어서, 확인 지점 표도 `window.pt`도 **아무도 안 받는 채로 배포물에
남는다.** 그것이 곧 뒷문이다. e2e ㉓이 `?dev=1`을 실제로 눌러 보고 `pt`도 없고
확인 지점 0줄이고 개발 조각 요청 0건인 것을 잰다. 소스 쪽은
`app/devBackdoor.test.ts`가 지킨다 — 주소·저장소로 켜는 길이 없는지, 개발 조각을
받는 자리가 다 빌드 상수 뒤인지.

⚠️ **그래도 배포본은 봐야 한다.** 굽힌 것과 개발 서버는 같은 화면을 안 낸다 —
실측으로 같은 떡잎마을이 `--mode development` 번들에서는 콘솔 2,440줄·배틀타워 최저
48fps인데 프로덕션으로 구우면 2줄·60fps다. **그 일을 세이브 파일이 한다** (§15.1b).
...
### 15.1b 확인 지점 세이브 — 배포본을 상대로 보는 길 (`pnpm saves`)

확인 지점 86자리마다 `.rpsave` 한 벌이 `saves/`에 있다. 타이틀의 「세이브 파일
불러오기」로 고르고 「이 리포트로 이어하기」를 누르면 그 자리에서 시작한다 —
**개발 손잡이가 하나도 없는 배포본에서도 같은 길이다.**

**파일을 조립하지 않는다.** 굽는 자(`tools/saves/bake.mjs`)는 자리마다

1. **새 화면**을 열고 (확인 지점은 가방에 도구를 *더하므로* 한 화면에서 몰면
   뒤로 갈수록 앞자리의 도구·플래그·도감이 쌓인다)
2. 리포트를 지우고 (화면의 「처음부터」와 같은 길)
3. 백틱 표에서 그 줄을 눌러 뛰어들고
4. 삼각형이 멎고 **화면이 실제로 돌기 시작할** 때까지 기다린 뒤
5. **도착하며 걸린 장면을 끝까지 밀고** (세이브는 도는 스크립트를 못 담는다)
6. 화면의 「리포트에 적기」가 내보내는 그 파일을 그대로 받아 적는다

⚠️ **굽는 동안은 배틀을 안 연다.** 확인 지점 열다섯은 도착하자마자 배틀을 여는데
세이브가 배틀을 못 담는다. 담는 것은 **그 배틀이 열리기 직전의 자리**고, 파일로
열면 사람이 풀에 들어가거나 눈을 마주쳐서 연다 — 원작에 오히려 가깝다.

**재는 자가 따로 있다** (`pnpm saves:check`). 파일마다 새 화면을 열어 진짜
`<input type="file">`에 얹고 「이 리포트로 이어하기」를 눌러서 넷을 본다:
① 리포트가 가리키는 맵에 실제로 섰는가 ② 삼각형이 붙고 **화면이 도는가**
③ 한 칸이라도 걸어지는가 ④ 콘솔이 조용한가. 「열리더라」로 넘어가지 않는다.

세이브에 **담게 만든 칸이 둘**이다 (CODEMAP §2.2대로 맨 뒤에 붙였다):

| 칸 | 왜 |
|---|---|
| `position.y` | 깨어진 세계는 맵 격자가 아니라 **떠 있는 판**이 통행과 높이를 정한다. 판을 고르는 `findPlatform`이 (x, y, z) 셋을 다 보는데 격자에 물으면 0이 와서 엉뚱한 판이 걸린다 (PARITY §6.10). 보통 맵에서는 **안 쓴다** — 격자가 다시 내주게 두어야 자료가 바뀌어도 자리가 따라간다 |
| `hourPin` | 게임 시각은 켤 때 기계 시계에서 한 번 받아 굳히는 값이라(`worldState`의 `startHour`), 파일이 들고 다니지 않으면 **밤 자리를 담은 세이브가 낮에 열린다**. 사람이 실제로 논 리포트는 늘 null이다 |

⚠️ **이 파일들은 낡는다.** 확인 지점은 좌표를 안 적지만 세이브는 **적어야만**
한다 — 자료가 바뀌면 `pnpm saves`로 다시 굽고 `pnpm saves:check`로 잰다.

⚠️ **봉투에 콘텐츠 계약이 찍힌다.** 개발 서버에서 구우면 `platinumLocale`이 `dev`라
설치본에서 열 때 「설치본과 콘텐츠 계약이 다릅니다」가 한 줄 뜬다 — 막지는 않는다
(`compareContract`). 지역판까지 맞추려면 설치본에서 구워야 한다.
...
| | 실측 |
|---|---|
| 코드 | 198,386줄 · 파일 922개 · 시험 파일 312개 (`src/**/*.ts{,x}`) / 시험 **3,932개** (`pnpm test:strict` · 건너뛴 것 3개 — 시험 파일 318개 중 여섯은 `tools/`에 있다) |
...
| 자산 | 46그룹 · 파일 11,278개 · 1069.4MB — **포켓몬 3D 705.5**(파일 3,933) · **NPC 모델 167.5**(116벌 + 목차) · **배틀 무대 134.5**(30벌) · 청크 29.8 · 소리 7.9 · 대사 3.9 · 행렬 3.5 · 소품 3.0 · 주인공 1.6MB (전부 PNG, KTX2 0개). 설치가 **요구하는** 것은 그중 29그룹이다 — Platinum 스물다섯 · BDSP 넷 (`import/install/required.ts` · DEPLOY.md §5.3 ⑮) |
| 배포물 | `dist` 파일 108개 · **6.3MB**. 첫 화면이 받는 것 19개 · 358.7KB · **gzip 126.6KB** (목표 135 · 예산 150). 한 번 열 때 실제로 나가는 바이트는 따로 잰다 — 첫 방문 **636kB** · 다시 열기 **0.8kB** (DEPLOY.md §3) |
| 번들 출처 | `battle-sim` 청크 6.55MB → **1.51MB**. 게임 데이터 8,881kB → **0** (`pnpm provenance`: 제3자 정적 게임 데이터 없음, DEPLOY.md §4) |
| 브라우저 실측 | `pnpm e2e` — **PASS 28 · FAIL 0 · BLOCKED 1 · 못 잰 것 0** (막힌 것은 ⑯ 하나고 올려야 풀린다, DEPLOY.md §5) · `pnpm story` **PASS 88 · FAIL 0** |
| 공개 배포 | `pnpm release:check` **열린 blocker 2** — ⑥(⑯이 옛 빌드를 쟀다)과 ⑧(dist가 커밋 전 나무에서 나왔다). 둘 다 올리고 다시 빌드하면 닫힌다 · `https://radiant.siwon.it.kr/` (DEPLOY.md §1) |

⚠️ **첫 화면 125.1KB는 부팅 게이트를 포함한 값이다.** 공개판은 부팅할 때
...
(`Characters/objects/ob1004_00`)에 있었다 — 오래 `ob1003_00`을 굽고 있었는데
원작이 주인공과 한 리그로 묶어 든 것은 그쪽이다. 붙이는 자리를 번들이
(0, 0, 0)으로 적어 두어 발밑에 그대로 놓으면 되고, 앉은 자세는 안장·페달·
손잡이 자리로 **역기구학**이 푼다 — 발이 페달에서 5cm 안이다(시험).

⚠️ **숲 밑에 파란 마름모와 갈색 턱 띠가 깔려 있었다 — 고쳤다.** 잎을 걷어낸
자리를 메우는 판(`floorPatch`)이 **제일 가까운** 바닥을 베끼는데, 그 제일 가까운
것이 웅덩이거나 방지턱 그림이었다. 오버월드 176청크에서 메운 삼각형 8,296개가
`puddlep`이고(잔디 44,310 다음이다) 정작 웅덩이의 바닥 넓이는 전체의 0.1%다.
떡잎마을 나무줄 밑은 통째로 눈밭(`s_snow`, 넓이 몫 0.1%인데 22배를 칠했다)이라
숲 앞에 **흰 선반**이 깔려 보였다. 세 가지를 고쳤다: 턱도 이름으로 거르고,
거르고 남은 것이 없으면 **이웃에서 빌려 오게** 하고(예전에는 안 거른 것을 도로
써서 거르기가 헛일이었다), 비긴 칸은 **그 청크를 실제로 이루는 넓은 땅**이
가져간다. 화면에 광선을 쏘아 같은 픽셀이 `s_snow04`에서 `ngrass`로 바뀐 것을
확인했다 (DATA.md §2.2).
...
| `SetWarpEventPos` | 예지호수 입구에서 **문 하나에 목적지가 둘** 겹쳐 있었다 |
...
⚠️ **화면 덮개를 이 화면이 걷는다.** 원작 스크립트가
`FadeScreenOut → StartChooseStarterScene` 순서라(`scripts_route_201.s`) 필드
화면은 **덮인 채로** 프로그램이 뜨는데, 원작은 그 프로그램이 제 화면을 따로
세운다. 우리는 화면이 하나고 덮개가 DOM 판 한 장이라(`ui/field/FadeOverlay`,
zIndex 300) 그 판이 3D 무대를 덮은 채 남았다 — 글창은 이 화면 것이라 그
위(400)에 떠서 **글은 보이고 고를 수도 있는데 가방만 안 보이는** 화면이 됐다.
그래서 `ChooseStarter`가 뜨는 동안만 덮개를 걷고, 나갈 때 **그대로 되돌려
놓는다**: 다음 명령이 `FadeScreenIn`인데 `startFade`가 지금 덮인 만큼에서
시작하므로, 걷어 버린 채로 나가면 밝아질 것이 없어 필드가 튀어나온다.
...
남은 자리는 거의 전부 **우리에게 그 계통이 없는** 것들이다: 배틀타워 · 통신 ·
게임 기록. 콘테스트와 지하통로는 **범위 밖으로 정했다** (PARITY §9).
...
#### 타이틀 — 넷이 앞에 서고 둘이 뒤에 붙는다

제목은 **Pokémon Radiant Platinum**이고, 아래에 넷이 한 줄로 선다:
"시작" · "이어하기" · "세이브 파일 내보내기" · "세이브 파일 불러오기".
그 뒤에 "설정"이 작게 붙는다 — **빼지 않는다.** 빼면 게임을 열기 전에 설정을
볼 길이 이 화면에서 사라진다.

⚠️ **설치 화면으로 가는 길은 평소에 안 세운다.** 여기까지 왔다는 것이 곧 설치가
온전하다는 뜻이라(부팅이 `installReady`를 통과해야 타이틀이 열린다) 다시 설치할
일이 없다. 늘 세워 두면 쓸 일 없는 단추가 하나 서 있고 하필 그 끝이 에셋을 지우는
화면이다. **딱 한 자리에서만 필요하다** — `watchIntegrity`가 뒤에서 훑다가 어긋난
파일을 만나는 때다. persist가 거부된 채 공간이 모자라면 브라우저가 설치본을 조용히
되찾아 가는데(e2e ㉔), 그때는 기록이 `ready`라 부팅은 통과하고 실제 파일만 깨져
있다. 이 길이 없으면 사이트 데이터를 통째로 지우는 수밖에 없고 그러면 리포트까지
같이 날아간다. 그래서 **경고가 떴을 때만** 그 경고를 고치는 길로 선다.

⚠️ **하네스가 타이틀을 「비공식·비제휴」로 찾고 있었다.** 그 글이 타이틀에만
있었기 때문인데, 설치 화면에도 같은 고지가 서면서 표식이 유일하지 않게 됐다.
그러자 e2e ⑮가 「설치 시작」을 누른 **직후에** 완주했다고 보고, 아무것도 안 깔린
채로 OPFS를 읽어 `NotFoundError`로 죽었다 (㉕·㉖은 그 설치본이 없어 따라 죽었다).
지금은 **타이틀에만 있는 것**으로 잰다 — 「시작」 단추, `exact: true`
(안 그러면 설치 화면의 「설치 시작」에 걸린다).

⚠️ **없는 것을 말없이 흐리게 두지 않는다.** 리포트가 없으면 "이어하기"가 안
눌리는데, 흐리기만 하면 눌러 보고 나서야 없다는 걸 안다. 그래서 **왜 못 누르는지**를
차림표 아래에 글로 적는다.

⚠️ **"시작"은 되돌릴 수 없다** — 하던 리포트를 지운다. 한동안 이것을 버튼이 아니라
요약 아래 작은 글자로 두었는데, 이어하기 옆에 같은 크기로 두면 잘못 눌리기
때문이었다. 넷이 나란히 서는 지금은 **확인을 한 번 받아** 막는다 — 크기로 막던 것을
걸음으로 막는다. 지우기 전에 파일로 받아 두는 것은 그대로다.

⚠️ **"이어하기"만 롬 글을 안 쓴다.** 나란히 선 넷 중 하나만 롬에서 오면 그 칸만
롬 지역판을 따라가서, 일본어 롬으로 설치한 사람의 차림표에 「冒険をつづける」
하나가 섞인다. 요약 넷(주인공·시간·도감·배지)은 그대로 롬 글이다 — 그쪽은 게임
안의 말이고 이쪽은 앱 껍데기의 말이다.

#### 설치 화면이 곧 첫 화면이다

설치 전 사용자는 **타이틀에 못 간다** — `BootGate`가 설치 전에는 `<App/>`을 아예
안 그린다(그리면 타이틀 곡·UI 글·맵 미리받기가 그 자리에서 요청으로 나가고, 그건
`/data`가 없는 상태라 전부 404다. e2e ①이 그것을 잰다). 그래서 그 사람이 처음
만나는 화면은 설치 화면이고, **거기가 타이틀처럼 보여야 한다**: 같은 배경 그림
(앱 셸이라 설치 전에도 있다) · 제목 · 그리고 **준비물 둘을 맨 앞에**.

⚠️ **비공식 고지가 거기에도 있어야 한다** (COPYRIGHT.md §11). 한동안 타이틀에만
있었는데, 정작 처음 오는 사람은 타이틀을 못 보므로 **아무도 안 보는 고지**였다.
아래쪽 끝이 아니라 제목 바로 아래다 — 끝에 두었더니 세 화면쯤 스크롤해야 나왔다.

⚠️ **`AssetAssistant`가 무엇이고 어디 있는지는 물음표로 편다** (`ui/common/Hint`).
`title` 속성으로는 안 된다 — 여러 줄이 안 들어가고, 터치에서는 안 뜨며, 담아야
하는 것이 폴더 나무다. **어디 있는지는 적고 어떻게 얻는지는 안 적는다**
(COPYRIGHT.md §4).
...
### 16.5 ④ 캐릭터 모션 — ✅ 원작 동작으로 걷는다

**걷기·뛰기·서 있기는 원작 클립이 몬다** (`engine/actor/clipGait`). 절차형
(`gait.ts` + `locomotion.ts`)은 그 아래 폴백으로 남는다 — 자전거와 턱 넘기는
클립이 아예 없어 여전히 절차형이고, 클립이 없는 옛 설치본도 그쪽으로 간다.

**어느 몸이 무엇으로 걷는가.** 원작 인물 번들을 다 열어 센 값이다
(`.audit/probe/walkDonors.mjs`):

| 몸 | 걷기 | 어떻게 |
|---|---|---|
| 주인공 `pc0001`·`pc0002` | `walk_b`·`run_b`가 제 몸에 있다 | 그대로 돈다 |
| 트레이너 `tr*` 91벌 | **없다** (`walk_b`를 가진 등신은 주인공 옷차림 28벌뿐) | 주인공 걷기를 **화면에서** 그 몸으로 옮긴다 |
| 치비 `fc*` 24벌 | `walk_f`·`run_f`·`wait_f`가 제 몸에 있다 | 그대로 돈다 |
| 모두 | 서 있기 `wait_b`는 등신 124벌 전부에 있다 | 제 몸의 것을 쓴다 |

오버월드에서 원작이 세우는 몸은 치비고 등신은 배틀 몸이라, 등신에는 걷기가
없다. 우리는 등신으로 세우므로(§4.3) 주인공 것을 옮긴다 — 몸마다 옮겨 **구우면**
한 명당 360KB, 아흔한 벌이면 33MB라 굽지 않고 세울 때 옮긴다.

⚠️ **클립을 제 빠르기로 돌리지 않는다. 걸은 거리로 돌린다.** 우리 속도는 이어져
있고(걷기 4.5·달리기 8m/s) 클립은 한 빠르기로 짜여 있다. 몸마다 **한 바퀴에
나아가는 거리**를 재 두고 움직인 거리만큼 위상을 민다. 그 거리는 **발이 땅에 닿은
구간에서** 잰다 — 발이 쓸린 폭으로 재면 뛸 때 두 발이 다 뜨는 구간이 섞여
길게 잡힌다. 한 번 디디는 동안 그 발이 땅에서 밀린 거리로 견줬다
(`.audit/probe/gait/view.mjs`가 화면에서 잰다):

| 한 바퀴 거리를 재는 방법 | 걷기 4.5m/s | 뛰기 8m/s |
|---|---|---|
| 발이 쓸린 폭 | 0.6~6.8cm | 28cm |
| 디딘 발 빠르기의 75% 분위 | 5~15cm | 4~7cm |
| **닿은 구간만** ← 이것 | 0~4cm | 1.8~5.6cm |
| (견줌) 절차형 | 26~30cm | 31~38cm |

⚠️ **쉬는 자세는 몸을 세우는 그 순간에 뜬다.** 옮기는 수식이 「쉬는 자세에서
얼마나 돌았나」라(`import/bdsp/retarget`) 절차형이 한 번이라도 뼈를 돌린 뒤에
뜨면 그 자세가 기준이 된다 — 실제로 그렇게 해 놓고 화면을 찍었더니 사람들이
팔을 벌린 채 서 있었다.

⚠️ **회전과 골반 자리만 싣는다.** 원작 클립에는 뼈 **크기** 트랙이 있는데,
치비는 우리가 머리·손·다리 뼈 크기를 줄여서 등신 옆에 세운다(`model/chibi`) —
크기 트랙을 두면 클립이 도는 순간 그 값이 매 프레임 덮여 머리가 도로 커진다.

아직 절차형이 맡는 자리: 자전거·턱 넘기·록클라임·파도타기. 본 로컬 축은 여전히
하드코딩하지 않는다 — 주인공 번들은 본이 로컬 +X로 뻗고 Hips가 180° 돌아 있다.
...
| **모델이 서는 배치** | **1,895개 (53.3%)** — 사람 그림만 보면 **100%** · 116벌 159.8MiB |
| 판때기로 남는 배치 | 1,660개 — **사람은 하나도 없다.** 물건·포켓몬·자리표시자뿐이다 (3D_GAP §3.1) |
...
사람 그림 기준 **100%**다 — BDSP에 몸이 없던 핸섬·플루토는 남의 몸을 다시 칠해 세웠다.
...
**남은 것:** 스윕(`0xE3`, 204회)과, 효과음 781개 중 걸 자리가 게임에 없는
것들. ⚠️ **그중 콘테스트·지하통로 몫은 영영 안 걸린다** — 범위 밖이다
(PARITY §9).
...
| ⑤ NPC 3D | ✅ 배치의 53.3%가 모델로 선다 — 사람 그림만 보면 **100%**다 |
...
표를 만들었는데 그건 정해 둔 아트 방향과 반대였다.** 배틀 몸이 없는 열일곱만
필드 번들에서 오고, 그것도 머리를 줄여 등신 계통에 맞춰 세운다 (§4.3).
...
⚠️ **그래서 `.dae` 길을 은퇴시킬 수 있었다.** 두 길이 같은 모델을 주고
**단위만 달랐다**(바인드 키 2.5095 대 1.4725). 지금은 주인공 둘 다 번들에서
굽는다 — 구운 파일을 다시 재도 메시 6 · 삼각형 18,544 그대로다 (DATA.md §4.2.1).
...
등신 몸에 붙은 클립은 전부 제자리 배틀 동작이라 걷기도 낚시도 없고, 치비 쪽에
56개가 있다. **주인공 둘에는 그중 필드 동작 열여섯을 옮겨 싣는다**
(`HERO_FIELD_CLIPS` — 낚시 일곱 · 폭포 셋 · 록클라임 하나 · 공중날기 둘 · 물주기
셋). 걷기는 안 옮긴다 — `actor/locomotion`이 뼈를 직접 돌려 만드는 것이고,
걸음이 속도에 묶여야 발이 안 미끄러진다.
...
`fc0001_00`은 Transform 276개에 이름이 159종이고, 겹치는 117쌍 중 116쌍은 쉬는
자세가 같은데 `RItem1` 한 쌍이 1.0000 다르다. 클립 56개의 바인딩 16,305건이
전부 `Origin` 갈래에 떨어지고 그 안에서는 이름이 126종에 126개라, 그 갈래로
좁히면 짝짓기가 확정된다.

⚠️ **자전거 클립 일곱만 다른 갈래다.** 그것들은 `fc0001_00`이 아니라 옷 번들
`fc0001_11`에 들어 있는데, **바인딩은 `fc0001_00`의 계층을 가리킨다** — 실측으로
198개가 그쪽에 100% 맞고 제 번들에는 0% 맞는다. 그리고 그 갈래가 `Origin`이
아니라 `Bicycle/Origin/…`이다: 사람과 자전거(`ob1004_00`)를 한 벌로 묶어 둔
리그가 따로 있고 `bike_wait/walk/run_f`가 그 둘을 함께 돌린다. `Origin`으로만
좁히면 자전거 클립은 한 채널도 안 걸린다 (DATA §4.2.1).
...
⚠️ **치비 번들은 메시가 자급자족이 아니다.** `fc0001_00`을 열면 메시가 다른
번들에 있다고 나온다 — 폴더(644MB)를 통째로 열어야 풀린다. **동작을 꿔 오는
데는 상관없다**: 필요한 것이 Transform 계층과 AnimationClip뿐이고 그 둘은 번들
하나에 다 들어 있다(실측 Transform 276 · 클립 56). 등신 번들은 메시까지 하나에
다 들어 있다.

✅ **용량은 갈래를 갈라서 풀었다.** 치비(`fc*`) 27벌은 클립을 하나도 안 싣고,
등신(`tr*`·`pc*`) 89벌은 **배틀에서 이어 붙는 넷만** 싣는다
(`engine/actor/npcModels`의 `TRAINER_CLIPS` — 굽는 쪽 둘이 이 규칙 하나를 같이
본다). 주인공 둘만 거기에 필드 동작 열여섯을 얹는다.
실측 **116벌에 159.8MiB**다(치비 27벌 12.94 · `tr` 87벌 140.81 · 주인공 둘 6.02).
106벌로 잰 자리에서 클립을 하나도 안 실으면
87.72MiB · 여덟을 다 실으면 234.93MiB였다. 배틀 클립 합계는 353개다 — 89벌 중 86벌이 넷을 다 갖고,
주인공 둘(`pc0001_00`·`pc0002_00`)과 `tr2018_00`에는 **`lose01_b`가 아예
없다**(원작이 안 만들었다. 그 셋은 진 동작을 안 한다). 서 있는 사람은 여전히
매 프레임 돌려야 한다 — 안 돌리면 팔을 벌린 T 자세로 선다. 갈래별 값과
**어느 것을 안 굽기로 했는지**는 [3D_GAP_AUDIT.md](3D_GAP_AUDIT.md) §3.2에 있다.

⚠️ **`wait_b`가 그 넷에 든 것은 크기가 아니라 화면이 정했다.** 없으면 트레이너가
지시를 내린 뒤 `order_b`의 마지막 프레임에 **굳은 채로** 배틀이 끝날 때까지 서
있다 — 돌아갈 자리가 없어서다. 이 클립 하나가 89벌에 1.33초짜리 쉼을 준다
(위 표의 무릎 회전폭 0.019가 그것이다 — 서 있는 자세라 거의 안 움직인다).

✅ **화면에 섰다.** 배치의 53.3%가 모델이다 — 사람 그림만 보면 100%다 (§16.6). 컷신에서도 선다 —
...
**① 배 컷신 (`ship_demo`).** 소품 노드 파이프라인이 섰으므로 남은 것은 자료
쪽이다 — `arc/ship_demo.narc`는 멤버 스무 개(모델 넷 + BCA0 4 · BTP0 4 ·
BTA0 4 · **BMA0 4**)고 `bm_anime`과 아카이브가 다르다. BMA0(재질 색 애니)는
아직 디코더가 없다. 부르는 자리는 `canalave_ship.c`다.

**② DOM 위에 입자를 얹을 층.** 폼 변화(기라티나·쉐이미)의 마디는 원작 값으로
돌지만 **입자를 못 그린다** — 원작은 파티 목록 위에 3D 입자를 얹는데
(`DrawParticleSystem`) 우리 파티 화면은 DOM 창이고 공유 캔버스가 그 뒤에 있다.
창에 구멍을 내면 갈아 끼우는 아이콘이 같이 사라진다. 자료는 이미 실려 있다
(`splPack`의 `formChange` 묶음, 멤버 둘).

**③ 자전거 4단과 거기 매인 둘.** 램프(24칸)와 진흙 비탈(34칸)은 **4단일 때만**
동작한다. 비탈 클립은 이제 풀리고 원작대로 멈춰 서 있다(`paused` ·
`loopCount = 1`) — 올라타는 방아쇠가 4단에 매여 있어 같이 남았다 (PARITY §1.9).

**④ 워프 멈춤의 남은 값.** 이제 **멈춘 창 안만** 센다(REPAIR §8.1). 그 창에서
`(idle)`이 3분의 1이다 — 자바스크립트가 도는 것이 아니라 **기다리는** 시간이라,
다음 물음은 「우리 함수를 빠르게」가 아니라 「무엇을 기다리는가」다.

⚠️ **뼈 행렬이 남의 버퍼로 가던 것은 닫혔다** (REPAIR §8.1). 임자가 둘이었다 —
`BattleTrainers`가 로더 다섯 중 혼자 `unifySkeletons`를 안 불렀고,
`PlayerModel`이 손질을 **`useEffect`에서** 불러 한 프레임 늦었다. 이야기 훑기로
장면마다 수백~수천 줄이던 것이 **0**이 된다.

### 16.11 아직 안 한 것 — 아는 채로 남겨 둔 것들

**원작과의 차이는 [PARITY.md](PARITY.md)가 정본이다.** 거기에 플래티넘의 기능
표면을 갈래로 갈라 하나씩 대조해 두었다 — 무엇이 되고, 무엇이 반쯤이고,
무엇이 없고, 무엇을 안 하기로 했는지까지. 여기 표는 그중 **맵·화면 쪽**만
남긴다.
...
| 아무도 안 부르는 `export` | **59** (값 52 · 타입 7) | 세는 자는 `.audit/probe/deadExports.mjs`(타입스크립트 파서로 훑는다). 기계가 할 수 있는 자리는 끝났다 — 제 파일 안에서만 쓰는 `export`는 낱말을 떼서 **지금 0개**고(`pnpm exports:check`가 다시 자라는 것을 막는다 — 실측 2026-09-13), 이름이 어디에도 안 나오는 것 중 **근거가 선 29개를 지웠다**. ⚠️ **남은 52개는 쓸어 담으면 안 된다** — **만들어 낸 파일**(`DECIBEL`·`POKETCH_MAP_STEP`) 둘 · 아직 안 끝난 BDSP 브라우저 변환의 발판과 조사 도구용 여섯(`readIseAt`·`iseInfo`·`weightTable`·`meshesOf`·`texturesOf`·`textureObjects`) · **구현해 놓고 안 이어 붙인 둘**(`berryWateringCancel`·`journalWarpedByMove` — [REPAIR.md](REPAIR.md) §10이 임자다)이 그 안에 있다. ⚠️ **`readBackup`은 거기 있었는데 지웠다** — 백업 되찾기를 이으면서 판정을 붙인 `readBackupDetailed` 하나로 합쳤다. 날것을 돌려주는 문을 따로 두면 되찾는 쪽이 스키마를 안 거치고 쓰는 길이 생긴다. ⚠️ **「누가 쓴다」고 적힌 주석을 믿지 않는다** — REPAIR §10이 여섯 중 다섯을 거짓으로 잡았고, `scene/stepSystem`의 `resetDayCache`도 「시험이 쓴다」고 적힌 채 그런 시험이 없어서 지웠다. ⚠️ **시험만 쓰는 `export` 562개는 흙이 아니다** (값 534 · 타입 28) — 순수 함수를 시험이 직접 부르라고 연 자리고, 검사가 시험을 소비자로 안 세면 그것이 다 걸려서 못 쓴다 |
| 그래픽 프리셋 ↔ 후처리 | — | 헤드리스로는 이득을 못 재므로 실기에서 모자랄 때 (§10.1) |

#### 맵 270개를 한 번에 훑는다 (`.audit/probe/mapAudit.mjs`)

확인 지점 하나를 찍는 데 2분이고 열 번에 한두 번은 엉뚱한 데서 찍힌다 —
그것으로는 행렬 270개를 못 덮는다. 그래서 **돌고 있는 앱 안에서 실제 모듈을
불러** 자료를 통째로 잰다. 재는 것이 곧 화면이 쓰는 그 코드다.

| 세는 것 | 실측 | 읽는 법 |
|---|---|---|
| 건물이 지면과 만나는가 | 640 / 2,967이 0.05타일 넘게 어긋난다 | 제일 큰 넷(±3~4타일)은 백화점 옥상 간판처럼 **일부러 띄운 것**이다. +0.25타일짜리 106개가 무엇인지는 아직 안 갈랐다 |
| 턱에 단차가 있는가 | 720 / 720이 **평평하다** | 고장이 아니라 원작이 그렇다 — BDHC는 턱 양쪽이 같은 높이고 턱은 그림(`allpeak`)이다. 3인칭에서는 잔디에 그은 갈색 선으로 읽히므로 나무·바위처럼 **우리가 세운다** — `scene/Ledges`가 칸마다 높이 0.18짜리 판과 앞면을 얹는다 (§16.6과 같은 갈래) |
| 사람이 벽 속에 서 있는가 | 174 / 3,555 | **174개가 174개 다 등장 플래그가 걸려 있다** — 플래그가 설 때까지 안 보이는 자리다. 고장이 아니다 |
...
⚠️ **`src/scene` 최상위를 하위 폴더로 가르는 것도 안 한다.** 평평한 것은 맞다 —
125개고(시험 37 · `.tsx` 34 · 나머지 `.ts` 54) 그 안에 R3F 컴포넌트와 체육관별
로직과 엔진 어댑터와 순수 기하 헬퍼가 섞여 있다. **그런데도 안 한다:** 파일을
옮기는 것뿐이라 기능 이득이 0인데, 이름이 앞을 맞추고 있어 실제로는 한 덩이로
읽힌다 (`distortion*` 열 · 체육관 여섯). 「이미 있는 본을 따르면 된다」도 반만
맞다 — `battle/`은 25개로 자리를 잡았지만 `field/`와 `fx/`는 각각 넷뿐이다.
**길을 못 찾아 실제로 걸릴 때 다시 본다.**
