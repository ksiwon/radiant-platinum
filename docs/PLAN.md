...
| 체육관 장치를 **걸어서** 확인하는 자리 | 8자리 중 0 | 확인 지점의 체육관 여덟이 전부 문간(일곱은 `warp` 0, 연고만 3번)에 서서 `battle`로 관장전을 곧바로 연다 — 꽃시계·문 고르기·샌드백·물바닥·뜨는 판·톱니를 **실제로 밟는 확인 지점이 하나도 없다.** 연고는 그 위에 `gym3`의 맵이 88(입구 방)이라 문 방 89·90에 아예 안 들어간다. 장치 자체는 단위 시험이 지킨다 (`scene/*Gym.test.ts` 여섯 벌 109개). 화면까지 보려면 `battle` 없는 지점을 장치 앞에 세워야 한다 |
