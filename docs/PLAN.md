...
~~**소리가 없다.**~~ ✅ 울린다(§16.8). 맵 헤더의 BGM 1186개가 전부 실재하는 곡을
가리키고, 낮/밤이 원작 `rtc.c` 표대로 갈린다. 배틀에 들어가면 곡이 바뀐다.
포락선은 지어내지 않고 ARM7에서 뜯어 왔다. **남은 것은 효과음 781개와
울음소리다.**
...
⚠️ **기억으로 적었던 초안이 세 군데 틀렸고 그중 둘은 시험이 못 잡았다.**
...
**남은 것:** 효과음 781개(형식은 같고 거는 일만 남았다), 모듈레이션·포르타멘토·
스윕(명령은 밟지만 소리에 반영 안 한다 — 전곡에서 `0xCA` 3333회 · `0xE3` 204회),
포켓몬 울음소리.
...
- **효과음 781개.** 형식이 BGM과 같아서 거는 일만 남았다. 문 여닫기·메뉴·
  풀숲처럼 이미 사건이 있는 자리부터.
