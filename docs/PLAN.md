...
`SAVE_VERSION`은 21이고 슬롯은 **리포트 한 벌**이다. `report()`를 부른 시점만
...
개발 빌드에 상시 오버레이(`ui/hud/PerfOverlay`, `?dev=1`일 때만): FPS · 프레임 타임 p50/p99 · 드로우콜 · 삼각형 · **구간마다의 최장 프레임**. 재는 자는 `engine/loop/frameStats`고, 게임 루프가 프레임마다 **자르기 전의 간격**을 민다.
...
받는 법: `?dev=1`로 켜고 얼마쯤 걸어 다닌 뒤 콘솔에서 `pt.perf()`. 켠 직후의 로딩을 빼고 재려면 `pt.perfReset()` 먼저.
...
**켜는 법은 주소다** (`app/devTools.ts`).

```
…/?dev=1     켠다 (기억한다 — 다음부터는 안 붙여도 켜져 있다)
…/?dev=0     끈다
```

개발 서버(`pnpm dev`)에서는 늘 켜져 있다. **안 켠 사람은 이 조각들을 한 바이트도 안
받는다** — 닿는 길이 전부 동적 import 하나뿐이라, 켜야 그때 청크를 받는다
(실측: 안 켠 배포본에서 `DevWarpScreen`·`devConsole`·`PerfOverlay` 요청 0건, 켜면 5건).
`lazy()`를 모듈 꼭대기에 두면 이렇게 안 되므로 쓰지 않는다.

⚠️ **에셋 갈래는 안 건드린다.** 이 손잡이는 화면에 개발 UI를 붙일 뿐이고, 무엇을
읽고 어디서 뜰지(`app/boot.ts`)는 그대로다 — `?dev=1`로 열어도 설치가 없으면 설치
화면이고, `?dev=1&assets=opfs`도 안 먹는다. e2e ㉓이 그 둘을 같이 누른다.

⚠️ **왜 배포본에도 두는가.** 굽힌 것과 개발 서버는 같은 화면을 안 낸다 — 실측으로
같은 떡잎마을이 `--mode development` 번들에서는 콘솔 2,440줄·배틀타워 최저 48fps인데,
프로덕션으로 구우면 2줄·60fps다. 「사람이 받는 것」을 보려면 배포본에서 봐야 한다.
...
### 16.10 다음 긴 작업 — 남은 넷의 차례

기획서(`.audit/hard-parts-plan.html`)의 일곱 중 셋이 닫혔고 넷이 남았다. 여기
적는 것은 **다음에 무엇을 어느 차례로 하는가**다. 값의 근거는 각 문서에 있다
(NSBCA는 [DATA §2.31](DATA.md), 진화는 [PARITY §3.1](PARITY.md), 미리 굽기는
[REPAIR §8.1](REPAIR.md)).

**① 진화 연출 — 정본은 A로 정했다** (사람이 고른 자리)

원작이 **입자로 돈다**는 것이 이번에 확인됐으므로, A는 「원작 박자 + 우리
그림」이 아니라 **「원작 그대로」**다. 자료는 이미 굽고 있고(`evolve` 묶음)
그리는 기계도 이미 있다(`scene/battle/splDraw`의 `SplShow`).

1. `engine/battle/evolutionBeat.ts` — 순수 마디 기계. 이미터 열셋의 차례,
   교대의 크기 곡선(`delta` 8→16→32→64), 취소 창, 레터박스, 흰 막.
   **길이는 상수가 아니라 `.spa`에서 나온다** — `splLifeFrames`로 재면
   교대 242프레임 · 전체 378프레임이다.
2. `SplParticles`에 `basis`를 받는 길을 낸다 — 배틀은 두 배틀러로 축을 세우지만
   연출 무대는 카메라가 정면이라 항등 기저다. **그리는 길은 하나로 둔다.**
3. `scene/CinematicStage`가 60Hz 고정 걸음으로 마디를 돌리고 입자를 얹는다.
   `cinematicMotion`의 사인파를 원작 기계로 바꾼다.
4. `ui/menu/EvolutionScreen`의 `CHANGE_MS`(2,200ms)를 마디 기계가 주는 값으로.

**② 미리 굽기 오류** — 확인 한 번(REPAIR §8.1의 갈래가 맞는지). 싸고, 맞으면
미리 굽기가 그제야 실제로 돈다.

**③ NSBCA · BTP0 · BTA0** — 디코더는 끝났다(DATA §2.31). 남은 것은 배선이다:
`import/platinum/nsbca.ts` → 소품 번호 → 애니 번호 표를 생성기로 굽고(**두
베이커가 같은 표를 본다**) → `convertProps`가 애니 있는 112개만 노드 구간을
남긴다 → 문(200ms 경첩을 클립의 진짜 프레임으로) → 비탈 303·304 · 배 컷신 ·
BTP0 · BTA0. 검증은 apicula와 **관절 행렬을 프레임마다** 견주는 것이다.

**④ 조우·부화·폼 변화 입자** — 형식 일이 아니다. 조우 이펙트는 따로 있는 것이
아니라 `waza` 묶음 안이고(`battle_particles.order` 4~15째가
`encounter_grass_a/b`·`water`·`plain`·`building`·`mountain`), 알·폼 변화도 이미
구워져 있다. 없는 것은 **큐 표와 부르는 자리**뿐이다.
