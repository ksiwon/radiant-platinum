...
효과음과 **울음소리 493종**도 붙었다. 포락선은 지어내지 않고 ARM7에서 뜯어 왔다.
...
명령 넷에는 색을 줬다(싸운다 붉은색 · 가방 노란색 · 포켓몬 초록 · 도망친다 파랑).
원작 배틀 메뉴가 그렇게 갈라져 있어서 글자를 안 읽어도 손이 먼저 간다.
...
⚠️ **눈으로 확인하지 못했다.** 이 환경에 브라우저 자동화가 없어서 화면을 찍을 수
없다 — 규칙과 수치는 디컴프에서 가져왔고 빌드·시험은 통과하지만, 보기 좋은지는
직접 봐야 한다.
