...
| 초기 청크 | **≤ 150KB gz** | 122KB ✅ — 타이틀에 three.js를 싣지 않는다 (§10.4) |
| 게임 진입 시 추가 JS | ≤ 450KB gz | 468KB ⚠️ — `three/webgpu`가 혼자 427KB다 (§10.4) |
...
| 지표 | 목표 | 실측 | 근거 |
|---|---|---|---|
| 초기 청크 (타이틀 인터랙티브) | **≤ 150 KB gz** | **122 KB** ✅ | three.js를 타이틀에 싣지 않는다. 라우트 분할 |
| 게임 진입 시 추가 JS | **≤ 450 KB gz** | **468 KB** ⚠️ | 4% 초과. 앱 코드는 41KB고 나머지가 three다 |
...
**분할 실측** (`vite build`, gzip):

```
초기 (타이틀)                        게임 (/play)                    배틀 (첫 조우)
  react           75.39 KB            three          427.45 KB        battle-sim  1033.81 KB
  optionsStore    26.79               Stage           21.70           controller    13.48
  index (앱 셸)   15.99               field            7.95         ──────────────────────
  css              3.32               PlayRoute        6.11           합계        1047.29
  rolldown-runtime 0.62               OptionsScreen    2.96
──────────────────────────           css              1.68
  합계           122.11 KB gz         genericNames     0.23
                                    ──────────────────────
                                      합계           468.08 KB gz
```
...
**18KB(4%) 넘는다.** 앱 코드가 41KB뿐이라 여기서 더 깎을 것은 three밖에 없다.
...
세 페이즈의 완료 조건은 전부 통과했고, 그 상태를 시험 717개가 붙들고 있다.
...
시험은 **소스 옆에 둔다**(`*.test.ts`, 66파일 / 717개). 따로 `tests/`를 두지
...
야생을 만나 배틀을 끝내고, 저장하고 껐다 켜면 그 자리다. 시험 717개가 그것을 묶고
...
| | 실측 |
|---|---|
| 코드 | 33,699줄 · 파일 222개 · 시험 파일 66개 / 시험 717개 |
...
붙이고 사거리를 4m로 끊는다. 발밑에 얕게 번지는 몫은 원래 밝기의 12%라 손전등이
아니라 달빛 웅덩이로 읽힌다.

밤 자체도 한 번 올렸다. 지금 비율은 이렇다(램버트 어림, `fx/sky`):
...
세기는 고정 상수가 아니라 **모자란 만큼**이다 — `0.6 × 낮의 몸빛 − 지금 몸빛`을
키 라이트의 이득으로 나눈다. 그래서 낮·아침·해질녘에는 정확히 0이 되어 꺼지고,
나중에 시간대 프리셋을 다시 손보면 이 값도 따라 움직인다.
...
⚠️ 이 화면들은 **눈으로 확인할 수 없다.** 이 환경에 브라우저 자동화가 없다.
규칙과 수치는 디컴프·계산에서 가져오고 빌드·시험으로 묶지만, **보기 좋은지는
사람이 봐야 한다.**
...
⚠️ 이 화면들은 **눈으로 확인하지 못했다.** 이 환경에 브라우저 자동화가 없다.
...
| 길 | 얻는 것 | 드는 것 |
|---|---|---|
| BDSP 덤프 | 원작 그대로의 트레이너·포켓몬 | nsp·키·nstool·UnityPy가 **전부 갖춰졌다**. romfs를 풀고 번들에서 꺼내는 일이 남았다 |
...
| ⑤ NPC 3D | ⛔ 2D 그림 그대로. BDSP 추출에 걸려 있다 |

**BDSP 경로는 열렸다.** nsp → NCA → romfs를 풀어 `Characters/persons/field`의
번들 161개를 꺼냈고(644MB), UnityPy로 열면 메시·재질·텍스처·애니메이션이 그대로
들어 있다. 절차와 함정은 `tools/extract/bdspNpc.py` 머리말에 적어 뒀다.

**누가 누구인지도 절반 붙였다.** 번들 이름은 `fc1006_00`처럼 번호지만 그 안의
텍스처 이름에 갈래가 적혀 있다(`fc1006_00_bugcatcher_body_col`) — 낱말 121개가
나온다. 플래티넘 그림 이름(`BUG_CATCHER`)과 **글자가 같은 것** 32쌍은 두 자료가
독립적으로 같은 낱말을 고른 것이라 더 볼 것이 없고, 손으로 적은 짝 13개를 더해
**156 중 45**가 붙는다.

⚠️ **나머지 111은 일부러 비워 뒀다.** 느슨하게 맞춰 보면 48쌍이 붙는데 그중
...
**아직 안 한 것:** 메시·뼈대를 glb로 굽는 일과 그것을 화면에 세우는 일. 지금
NPC는 여전히 판때기 그림이다.
