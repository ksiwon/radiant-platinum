...
지금 걷는 것은 **절차적 보행**이다(`gait.ts` + `locomotion.ts`). 주인공 번들
(`pc0002_00`)의 165조인트를 그대로 쓰고 위상 속도를 보폭에서 유도한다. 현재 개발 raw에는 BDSP의
...
본 로컬 축은 하드코딩하지 않는다. 주인공 번들은 본이 로컬 +X로 뻗고 Hips가
180° 돌아 있어서 월드 회전을 본의 로컬로 켤레변환해야 한다. 새 브라우저 변환기도 같은 rig
