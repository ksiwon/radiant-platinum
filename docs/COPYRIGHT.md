...
| 위험 | 이 구조의 효과 | 남는 위험 |
|---|---|---|
| 원본 ROM·추출 에셋 직접 배포 | **없다.** 이 리포의 `raw/`·`public/data/`·`public/models/` 산출물이 `dist/`에 없고 `@pkmn/sim`의 게임 데이터도 0바이트다 — 매 빌드 재검사 + 브라우저에서 요청 0건 실측 | ⚠️ 과거 Git 히스토리는 그대로다 (§9 · HISTORY.md) |
...
| 상표·출처 혼동 | 제목·manifest에서 "Pokémon"을 앞세우지 않고, 타이틀 화면에 비공식·비제휴 고지를 **눈에 보이게** 둔다 | ⚠️ **앱 셸 그림에 워드마크와 원작 캐릭터로 보이는 형상이 남아 있다** (§11). 이건 바이트가 아니라 보이는 것의 문제라 BYOR가 안 줄여 준다 |
| 테이크다운·호스팅 중단 | 원본 유래 CDN이 없어 영향 범위가 줄어듦. 접수·중단 절차를 미리 정해 뒀다 (TAKEDOWN.md) | 권리자는 앱 셸·저장소·도메인에도 삭제를 요구할 수 있음 |
...
**공개 사용자가 고르는 진짜 구조에서는 안 도는 코드**가 남는다. 개발도 공개도
같은 것을 읽는다. 어댑터(`tools/raw/sources.cjs`)가 자리를 정하므로 raw를
정리해도 코드는 안 바뀐다.
...
통과하면서 6.55MB였고, 그 안의 8,881kB가 `@pkmn/sim`의 종족·기술·습득기술 표였다.
MIT 패키지라는 사실과 거기 담긴 게임 데이터를 우리가 배포해도 되는지는 다른
질문이다 — **오픈소스 라이선스가 Pokémon 데이터에 대한 권리를 주지 않는다.**

빌드에서 표를 껍데기로 갈아 끼워 **8,881kB를 전부 뺐다**(청크 483kB). 수치는
사용자의 롬에서 오고 효과 구현은 우리 소스로 옮겼다 — 어떻게 갈랐고 그것이
원작과 같다는 것을 무엇으로 재는지는 `docs/DEPLOY.md` §4.
...
`dist/`를 뒤져 확인한다. 지금 남은 것은 `supported.json` 하나다 — `textBanks.json`은 지웠다 (§9).
...
— `src/` 안이고 이름도 `.json`이다. 안에 든 것은 뱅크 724개의 이름·상수·**롬 뱅크
헤더 +2에서 읽은 u16 복호화 키**·지역별 인덱스·엔트리 수였다.

**지웠다.** 배포물에는 안 들어가고 있었지만, 자리나 개수가 아니라 롬에서 읽은
값이라 §2의 "비표현적 메타데이터"에 들지 않는다.

지운 뒤에 무엇이 대신하는가:

- `src/import/platinum/textBanks.ts`가 **이름 순서에서 자리를 계산한다.** 남은
  것은 셋 다 이름이다 — 뱅크 순서(디컴프 `generated/text_banks.txt`) · CJK에
  없는 뱅크 · 끼워 넣는 자리. 숫자가 하나도 없다
- 계산이 맞는지는 **사용자의 롬이 답한다.** `readBankLayout`이 narc의 실제 뱅크
  수(us 724 / ko 714 / ja 709)와 (키, 엔트리) 쌍의 유일성을 그 자리에서 확인하고
  어긋나면 던진다. 값을 저장하지 않으므로 표가 낡을 일도 없다
- 개발용 표는 `pnpm extract:textbanks`가 **`raw/work/`** 에만 굽는다 (gitignore).
  세 로케일 724개가 옛 표와 전부 일치하는 것을 확인하고 지웠다
  (`textBanks.local.test.ts`, `PT_REQUIRE_DATA=1`)
- 일반 시험은 **인공 NARC와 지어낸 키**만 쓴다. 실패 메시지에도 값을 안 적는다 —
  어긋난 뱅크의 이름과 개수까지만 말한다. 그러지 않으면 CI 로그가 곧 그 표가 된다
- **히스토리에는 그대로 있다.** 블롭 3개 · 커밋 4개. 재작성 대상에 넣었다
  (HISTORY.md §1)

경로 규칙이 이름을 바꿔 옮긴 것을 못 본다는 것이 이 발견의 교훈이라,
`boundary:pre`가 **tracked 파일 전부**를, `boundary:post`가 **배포물 전부**를
열어 머리 바이트와 지운 값의 모양을 직접 본다. 실측: tracked 493개 · 배포물
30개 — 원본 유래 0개.
...
3. `git filter-repo --invert-paths --path public/data --path assets-manifest.json --path src/data/textBanks.json`
...
BYOR가 줄이는 것은 **원본 바이트 배포**다. 공개 앱 셸이 공식처럼 보이는가는
그것과 **별개의 문제**고, 사용자가 자기 데이터를 가져오는 구조는 여기에
아무 도움도 안 준다.

### 바이트 검사가 못 잡은 것

셸 그림 셋은 전부 우리가 만든 PNG다. 출처 검사도 매직바이트 검사도 히스토리
감사도 전부 통과한다. 그런데 **화면을 열어 보면**(`node tools/shot/title.mjs`)
타이틀 배경에 금속 질감의 `POKEMON RADIANT PLATINUM` 로고가 그려져 있고,
아이콘과 배경의 주인공은 기라티나(오리진 폼)로 보이는 형상이다.

정본은 [`tools/distribution/shellArt.mjs`](../tools/distribution/shellArt.mjs)다.
그림마다 **무엇을 그렸는지** 적고, 목록에 없는 그림이 셸에 들어오면 빌드가 선다.
...
이것이 `brand-art` release blocker였다. **만든 사람이 알고 감수하기로 했다.**

| | |
|---|---|
| 누가 | 만든 사람 |
| 근거 | 원작을 리디자인한 타이틀 표지를 쓰는 팬게임이 널리 있다 |
| 알고 있는 것 | 관행이 넓다는 것은 허가도 면책도 아니다. 상표·2차적 저작물 위험과 takedown 가능성은 그대로 남는다 (§3) |

⚠️ **"문제 없다"고는 적지 않는다.** 관행은 위험의 **크기**를 짐작하게 할 뿐
유무를 정해 주지 않는다. 리디자인한 그림만 쓴 팬게임도 내려간 적이 있다 —
Pokémon Uranium(2016)은 페이크몬과 아트를 **전부 새로 그렸는데도** DMCA로
내려갔고, Pokémon Prism(2016)은 발매 직전에 중지 통지를 받았다. 원본 바이트를
안 싣는 것은 이 축을 안 줄여 준다. 그리고 이 문서는 처음부터 "안전"·"합법"을
쓰지 않는다 (§3 · DEPLOY.md §6).

**대장은 안 고쳤다.** [`shellArt.mjs`](../tools/distribution/shellArt.mjs)는 지금도
워드마크가 있다고, 형상이 무엇으로 보인다고 그대로 적는다. 바뀐 것은 사실이
아니라 **그 사실을 두고 공개를 막을 것인가**다. 층이 갈라져 있어서 판단이 다시
뒤집혀도 사실은 다시 잴 필요가 없다 — 그림을 안 바꾸고 통과시키려면 대장이
거짓말을 해야 하는데, 그 길은 만들지 않았다.

**조용히 사라지지도 않는다.** blocker에서 뺀 뒤에도 `pnpm release:check`는
무엇을 안고 가는지 매번 찍는다:
...
워드마크를 지우고 캐릭터가 아닌 것으로 바꾸면 대장이 스스로 비고, 그때는
감수할 것도 없어진다. 신고를 받으면 [TAKEDOWN.md](TAKEDOWN.md) 순서로 간다.
...
고지 문구가 실제로 보이는지는 스크린샷으로 확인했다. 처음엔 메뉴 위에 뒀는데
배경 로고와 정확히 겹쳐 글자가 사라졌다 — `crest`와 `foot`이 `display: none`이라
거기 넣었으면 문서에만 있고 화면에는 없었을 것이다. **그건 "표시했다"가 아니다.**

### 앱 셸이 말하는 것

타이틀 화면과 manifest `description`이 다음을 적는다:

- 비공식·비제휴 팬 프로젝트다
- 관련 상표와 저작물은 각 권리자의 것이다
- 자신이 적법하게 보유한 게임 데이터만 고르라
- 서버는 원본도 변환 결과도 받거나 저장하지 않는다
- **무료·비영리·BYOR가 권리자의 허가를 뜻하지 않는다**
...
| 분석·telemetry·외부 CDN·광고 SDK | 없다. CSP `connect-src`의 바깥 오리진은 **버그 제보의 EmailJS 하나**뿐이고, 그것은 사람이 적고 사람이 누를 때만 간다 (아래) |
| 다운로드·ROM·키·추출 도구 링크 | 없다 (§4) |
| 공개 마케팅 스크린샷 | 없다. 생길 때 사용자가 로컬 설치한 자산을 안 넣는다 |
| README | **아직 없다.** 만들 때 위 고지를 같은 문구로 넣는다 |

### 나가는 것 — 딱 하나

**버그 제보**(`src/ui/screens/BugReport.tsx`)뿐이다. 사람이 제목과 내용을 치고
「보내기」를 눌러야 나가고, 그때 나가는 것은 이것이 전부다:

| 나간다 | 안 나간다 |
|---|---|
| 사람이 친 제목·내용 | 롬 바이트 · 변환 결과 · 리포트 |
| 게임 판 (`patchLog.ts`의 `VERSION`) | 롬 지역판 · 파일 이름 · 해시 |
| 브라우저 종류(`userAgent`)·창 크기 | 계정 · 주소 · 위치 · 식별자 |

받는 곳은 EmailJS를 거친 **만든 사람의 메일함**이다. 저장소도 통계판도 없다.

⚠️ **한때 이것도 없었다.** 2026-09-04 전에는 CSP `connect-src`에 바깥 오리진이
하나도 없어서, 「안 나간다」가 **구조적으로** 참이었다. 지금은 **정책적으로**
참이다 — 코드가 저 하나 말고는 안 부르고, `rules.test.mjs`가 오리진이 둘로
늘어나는 것을 막는다. 그 등급 차이를 이 문서가 감추지 않는 것이 중요하다
([DEPLOY.md](DEPLOY.md) §3).

### 코드로 해결되지 않는 것

**프로젝트 이름 자체의 상표·혼동 위험은 코드로 못 없앤다.** `Radiant Platinum`이
안전한지는 이 문서도, 검사 스크립트도, LLM도 판정할 수 없다. 독립적인 법률
검토 또는 권리자의 허가가 필요한 항목으로 남긴다.

같은 층에 있는 것들:

- 캐릭터·세계관·게임 구조의 2차적 저작물성
- 원작 대사·이름·설명문을 사용자의 롬에서 읽어 화면에 그대로 쓰는 것
- 도메인 이름

`pnpm release:check`가 전부 초록이 되더라도 **"takedown 위험이 없어졌다"고
말하지 않는다.** 검사가 재는 것은 기술적 경계뿐이다.
